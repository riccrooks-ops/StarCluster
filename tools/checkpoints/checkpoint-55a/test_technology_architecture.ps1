...
foreach ($study in @($profileStudy, $auxStudy, $isolationStudy, $powerStudy)) {
    Assert-True ([string]$study.schemaVersion -eq $canonicalIntegratedSchema) "Checkpoint 55a integrated study schema mismatch: $($study.id)"
}
...
$checkpoint = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-55a.json')
...
Assert-True ([string]$checkpoint.checkpointId -eq '55a') 'Checkpoint 55a definition ID mismatch.'
...
Write-Host '       Checkpoint 55a hotfix definition: unchanged 39 stages, 10,696 Monte Carlo variants, 106.96 million default trials.'
