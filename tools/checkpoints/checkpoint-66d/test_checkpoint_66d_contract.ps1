...
$guardedDefinitionPaths = @(
    'tools/calibration/checkpoints/checkpoint-66d.json',
    'tools/calibration/checkpoints/checkpoint-66d-deep-calibration.json'
)
...
$active = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-66d.json')
$deep = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-66d-deep-calibration.json')
...
$numericBaselinePath = Join-Path $repositoryRoot 'docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_2.csv'
$numericBaselineHash = (Get-FileHash -LiteralPath $numericBaselinePath -Algorithm SHA256).Hash.ToLowerInvariant()
Assert-True (([string]$study.baselineSha256).ToLowerInvariant() -eq $numericBaselineHash) 'CP66 study baselineSha256 must match the exact v0.2 numerical baseline used by the active stage.'
$technologyCatalogProperty = $study.PSObject.Properties['technologyProfileCatalog']
Assert-True ($null -eq $technologyCatalogProperty -or [string]::IsNullOrWhiteSpace([string]$technologyCatalogProperty.Value)) 'CP66 TL1-only study must not bind a frozen historical technology-profile catalog to the newer v0.2 baseline.'
Assert-True (@($study.variants | Where-Object { [string]$_.sideAProfileId -ne 'tl1-production' -or [string]$_.sideBProfileId -ne 'tl1-production' }).Count -eq 0) 'CP66 catalog-free study is valid only while every variant uses tl1-production on both sides.'
$auxCatalogRelative = [string]$study.auxiliaryProfileCatalog
Assert-True (-not [string]::IsNullOrWhiteSpace($auxCatalogRelative)) 'CP66 study must retain its explicit auxiliary-profile catalog.'
$auxCatalogPath = Join-Path $repositoryRoot $auxCatalogRelative
$auxCatalog = Read-Json $auxCatalogPath
$auxProfileIds = @($auxCatalog.profiles | ForEach-Object { [string]$_.id })
$referencedAuxIds = @($study.variants | ForEach-Object { [string]$_.sideAAuxiliaryProfileId; [string]$_.sideBAuxiliaryProfileId } | Select-Object -Unique)
foreach ($auxId in $referencedAuxIds) { Assert-True ($auxProfileIds -contains $auxId) "CP66 variant references missing auxiliary profile '$auxId'." }
$activeStudyStage = Find-ById $active.stages 'tl1-scripted-overload-tactics'
$deepStudyStage = Find-ById $deep.stages 'tl1-scripted-overload-tactics'
Assert-True ($null -ne $activeStudyStage -and $null -ne $deepStudyStage) 'CP66 checkpoint definitions must include the scripted overload stage.'
foreach ($stage in @($activeStudyStage,$deepStudyStage)) {
    $args = @($stage.arguments)
    $baselineArgIndex = [Array]::IndexOf($args, '--baseline-file')
    Assert-True ($baselineArgIndex -ge 0 -and ($baselineArgIndex + 1) -lt $args.Count) 'CP66 scripted overload stage must declare --baseline-file.'
    Assert-True ([string]$args[$baselineArgIndex + 1] -eq 'docs/archive/player_technology/pre-cp165-active/tl1_core_combat_numerical_baseline_v0_2.csv') 'CP66 scripted overload stage must execute against numerical baseline v0.2.'
}
...
Assert-True ([string]$study.id -eq 'tl1-itc09-scripted-overload-tactics') 'Unexpected Checkpoint 66/66d study ID.'
Assert-True (@($study.variants).Count -eq 80) 'Checkpoint 66/66d study must define exactly 80 variants.'
...
Assert-True ([string]$active.checkpointId -eq '66d' -and [string]$deep.checkpointId -eq '66d') 'Checkpoint definitions must identify 66d.'
Assert-True ([string]$active.manifestFile -eq 'CHECKPOINT_66D_SHA256SUMS.txt' -and [string]$deep.manifestFile -eq 'CHECKPOINT_66D_SHA256SUMS.txt') 'Checkpoint definitions must bind the CP66d manifest.'
Assert-True ([string]$active.outputRoot -eq 'out/checkpoint-66d' -and [string]$deep.outputRoot -eq 'out/checkpoint-66d-deep-calibration') 'Checkpoint 66d output roots mismatch.'
...
foreach ($definition in @($active,$deep)) {
    ...
    Assert-True ([bool]$pre.required -and [string]$pre.script -eq 'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1') 'CP66d definitions must require the shared native dependency precheck.'
    Assert-True (@($pre.powerShellPaths) -contains 'tools/calibration/run_calibration_checkpoint.ps1' -and @($pre.powerShellPaths) -contains 'tools/checkpoints/checkpoint-66d/apply_checkpoint_66d.ps1' -and @($pre.powerShellPaths) -contains 'tools/checkpoints/checkpoint-66d/test_checkpoint_66d_contract.ps1') 'CP66d dependency precheck must inspect apply/contract/shared harness.'
    Assert-True (@($pre.checkpointDefinitionPaths) -contains 'tools/calibration/checkpoints/checkpoint-66d.json' -and @($pre.checkpointDefinitionPaths) -contains 'tools/calibration/checkpoints/checkpoint-66d-deep-calibration.json') 'CP66d dependency precheck must inspect normal and deep definitions.'
}

$applySource = Get-Content -LiteralPath (Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-66d\apply_checkpoint_66d.ps1') -Raw
...
Assert-True ($guardCallIndex -ge 0 -and $guardCallIndex -lt $contractCallIndex -and $contractCallIndex -lt $harnessCallIndex) 'CP66d apply script must run the native dependency precheck before contract and harness.'
Assert-True ($applySource -notmatch '\$LASTEXITCODE') 'CP66d apply script must not read LASTEXITCODE.'
...
$contractSource = Get-Content -LiteralPath (Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-66d\test_checkpoint_66d_contract.ps1') -Raw
Assert-True ($contractSource -notmatch '\$_\.key') 'CP66d contract must not read the nonexistent numerical-baseline .key property.'
Assert-True ($contractSource -match 'parameter_id' -and $contractSource -match 'numericColumns') 'CP66d contract must validate the numerical-baseline parameter_id/value schema before row lookup.'
...
Assert-True ($runner -match 'string\.IsNullOrWhiteSpace\(study\.TechnologyProfileCatalog\)' -and $runner -match 'TechnologyCombatProfileCatalog\.BuildTl1\(baseline\)') 'Integrated runner must support catalog-free tl1-production construction from the active baseline.'
...
Assert-True ($validationFiles.Count -eq 1 -and $validationFiles[0].Name -eq 'Checkpoint_66d_Policy_Telemetry_Classification_Hotfix.md') 'Exactly one CP66d active validation runbook must remain.'
$rootTxt = @(Get-ChildItem -LiteralPath $repositoryRoot -File -Filter '*.txt')
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_66D_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_66D_SHA256SUMS.txt as .txt.'
...
Write-Host '       Checkpoint 66d native dependency contract: shared harness + checkpoint guard reject Python runtime dependencies for CP66+.'
...
Write-Host '       Hotfix stack: numerical-baseline schema, gate scope, v0.2 catalog binding, and policy-telemetry study classification are all preflighted before native acceptance.'


# Cross-study integration audit: the CP66 study ID must be registered in every
# shared runner location that materially affects execution or release gating.
...
foreach ($fragment in $requiredRunnerFragments) {
    Assert-True ($runnerText.Contains($fragment)) "CP66 cross-study integration is missing runner fragment: $fragment"
}
$policyGateStart = $runnerText.IndexOf('"policy-telemetry"', [StringComparison]::Ordinal)
$policyGateEnd = $runnerText.IndexOf('"attack-layer-telemetry"', [StringComparison]::Ordinal)
Assert-True ($policyGateStart -ge 0 -and $policyGateEnd -gt $policyGateStart) 'Unable to isolate the shared policy-telemetry gate source span.'
$policyGateText = $runnerText.Substring($policyGateStart, $policyGateEnd - $policyGateStart)
$policyStudyMentions = ([regex]::Matches($policyGateText, 'Tl1ScriptedOverloadTacticsStudyId')).Count
Assert-True ($policyStudyMentions -ge 2) 'CP66 must be classified in both the policy-telemetry predicate and its diagnostic branch; otherwise the 80-variant run can fail only after Monte Carlo completes.'
Assert-True ($policyGateText.Contains('results.All(result => result.MeanOrderRequests > 0.0)')) 'Shared policy telemetry must continue requiring requested-order telemetry.'

Write-Host 'Checkpoint 66d contract validation passed.'
