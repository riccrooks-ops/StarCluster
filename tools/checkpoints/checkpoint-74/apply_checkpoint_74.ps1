...
$contractCheck = Join-Path $PSScriptRoot 'test_checkpoint_74_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-74-deep-calibration.json' } else { 'checkpoint-74.json' }
$definition = Join-Path $repositoryRoot (Join-Path 'tools\calibration\checkpoints' $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-74.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-74-deep-calibration.json'
$guardedPowerShellPaths = @(
    'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1',
    'tools/checkpoints/checkpoint-74/apply_checkpoint_74.ps1',
    'tools/checkpoints/checkpoint-74/test_checkpoint_74_contract.ps1',
...
$staleRootPatterns = @('^CHECKPOINT_(?!74(?:_|$)).*_SHA256SUMS\.txt$','^Checkpoint_(?!74(?:_|$)).*_Readme\.txt$','^checkpoint-(?!74(?:-|$)).*-static-preflight\.txt$')
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_74_Accepted_EW_Doctrine_And_Degraded_Fire_Foundation.md') { Move-Item -LiteralPath $file.FullName -Destination $validationArchive -Force }
    }
}
& $contractCheck
