...
$checkpoint = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-58e.json')
...
Assert-True ([int]$output.weapons.kinetic.damage -eq 5 -and [int](Get-OptionalProperty $output.weapons.kinetic 'powerCost' -1) -eq 2) 'TL4 Kinetic output/power candidate mismatch.'
Assert-True ([int]$output.weapons.energy.damage -eq 4 -and [int](Get-OptionalProperty $output.weapons.energy 'powerCost' -1) -eq 3) 'TL4 Energy output/power candidate mismatch.'
Assert-True ([int]$output.weapons.missile.damage -eq 6 -and [int](Get-OptionalProperty $output.weapons.missile 'powerCost' -1) -eq 1) 'TL4 Missile output/power candidate mismatch.'
...
Assert-True ([string]$checkpoint.checkpointId -eq '58e') 'Checkpoint 58e definition ID mismatch.'
Assert-True (@($checkpoint.stages).Count -eq 56 -and [int]$checkpoint.checkpointMetrics.stageCount -eq 56) 'Checkpoint 58e must contain 56 stages.'
Assert-True ([int]$checkpoint.checkpointMetrics.monteCarloVariantCount -eq 14746 -and [int]$checkpoint.checkpointMetrics.trialsAtDefault -eq 147460000) 'Checkpoint 58e workload metrics mismatch.'
Assert-True ([string]$checkpoint.primaryStudy.id -eq 'tl4-itc04-single-main-axis-screening' -and [int]$checkpoint.primaryStudy.variantCount -eq 144) 'Checkpoint 58e primary study metadata mismatch.'
...
# Checkpoint 58e release-manifest contract: retained repository ZIPs are repository-owned files,
# not packaging artifacts, and therefore must be present exactly once in the top-level manifest.
$manifestPath = Join-Path $repositoryRoot 'CHECKPOINT_58E_SHA256SUMS.txt'
Assert-True (Test-Path -LiteralPath $manifestPath -PathType Leaf) 'Checkpoint 58e repository manifest is missing.'
...
foreach ($relative in $retainedZipPaths) {
    $matches = @(
        $manifestLines | ForEach-Object {
            $entryMatch = [regex]::Match($_, '^([0-9a-fA-F]{64})  (.+)$')
            if ($entryMatch.Success -and $entryMatch.Groups[2].Value.Replace('\','/') -eq $relative) {
                $_
            }
        }
    )
    Assert-True ($matches.Count -eq 1) "Retained repository ZIP must appear exactly once in Checkpoint 58e manifest: $relative"
    $expectedHash = ([regex]::Match($matches[0], '^([0-9a-fA-F]{64})  ')).Groups[1].Value.ToLowerInvariant()
    $actualHash = (Get-FileHash -LiteralPath (Join-Path $repositoryRoot $relative) -Algorithm SHA256).Hash.ToLowerInvariant()
    Assert-True ($actualHash -eq $expectedHash) "Retained repository ZIP manifest hash mismatch: $relative"
}
...
Assert-True ($runnerSource.Contains('study.Variants.All(v => v.SideASecondaryFamily is null && v.SideBSecondaryFamily is null)')) 'Checkpoint 58e single-main gate must read secondary-family state from study variants.'
Assert-True (-not $runnerSource.Contains('results.All(r => r.SideASecondaryFamily is null && r.SideBSecondaryFamily is null)')) 'Checkpoint 58e must not read non-existent secondary-family members from variant summaries.'
Assert-True ($runnerSource.Contains('v.ProfileLabel?.StartsWith("r58-defpair-none-", StringComparison.Ordinal) == true')) 'Checkpoint 58e powered-defense none-label check must be nullable-safe.'
Assert-True ($runnerSource.Contains('v.ProfileLabel?.StartsWith("r58-defpair-base-", StringComparison.Ordinal) == true')) 'Checkpoint 58e powered-defense base-label check must be nullable-safe.'
Assert-True ($runnerSource.Contains('private static int RequiredVariantCountForStudy(string studyId)')) 'Checkpoint 58e must centralize integrated tactical-combat required variant counts.'
Assert-True (([regex]::Matches($runnerSource, 'RequiredVariantCountForStudy\(study\.Id\)')).Count -eq 2) 'Checkpoint 58e Validate and BuildGates must both use the shared required-count helper.'
Assert-True (-not $runnerSource.Contains('int requiredCount = study.Id switch')) 'Checkpoint 58e must not retain duplicate study-ID required-count switches.'
foreach ($mapping in @(
    ...
    Assert-True ($runnerSource.Contains($mapping)) "Checkpoint 58e required-count helper missing mapping token: $mapping"
}
Assert-True ($runnerSource.Contains('$"RESULT {result.Id}: mean {result.MeanTurns:F2}, " +')) 'Checkpoint 58e per-variant console output must be labeled RESULT, not PASS.'
Assert-True (-not $runnerSource.Contains('$"PASS {result.Id}: mean {result.MeanTurns:F2}, " +')) 'Checkpoint 58e must not claim per-variant PASS before aggregate gates run.'
Assert-True ($runnerSource.Contains('$"FAILED GATE {gate.Id}: {gate.Detail}"')) 'Checkpoint 58e must print failed aggregate gate ID and detail directly.'

$applySource = Get-Content -LiteralPath (Join-Path $PSScriptRoot 'apply_checkpoint_58e.ps1') -Raw
Assert-True (-not ($applySource -match '(?im)^\s*&\s*(python|python3|py)(\.exe)?\b')) 'Checkpoint 58e launcher must not require Python.'
Assert-True ($applySource.Contains('checkpoint-58e.json')) 'Checkpoint 58e launcher must use the Checkpoint 58e definition.'
...
Write-Host '       Checkpoint 58e gate hotfix: one required-variant-count map feeds validation and aggregate gates; failed gates print directly.'
Write-Host '       Checkpoint 58e hotfix: 56 stages; 14,746 Monte Carlo variants; 147.46 million default trials.'
