...
$normalRel = 'tools/calibration/checkpoints/checkpoint-69d.json'
$deepRel = 'tools/calibration/checkpoints/checkpoint-69d-deep-calibration.json'
$guardedPs = @(
    'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1',
    'tools/checkpoints/checkpoint-69d/apply_checkpoint_69d.ps1',
    'tools/checkpoints/checkpoint-69d/test_checkpoint_69d_contract.ps1',
...
Assert-True ([string]$normal.checkpointId -eq '69d') 'Checkpoint 69d normal definition ID mismatch.'
Assert-True ([string]$deep.checkpointId -eq '69d') 'Checkpoint 69d deep definition ID mismatch.'
Assert-True ([string]$normal.manifestFile -eq 'CHECKPOINT_69D_SHA256SUMS.txt') 'Checkpoint 69d normal manifest binding mismatch.'
Assert-True ([string]$deep.manifestFile -eq 'CHECKPOINT_69D_SHA256SUMS.txt') 'Checkpoint 69d deep manifest binding mismatch.'
Assert-True ([string]$normal.outputRoot -eq 'out/checkpoint-69d') 'Checkpoint 69d normal output root mismatch.'
Assert-True ([string]$deep.outputRoot -eq 'out/checkpoint-69d-deep-calibration') 'Checkpoint 69d deep output root mismatch.'
$expectedGuardedPs = @(
    'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1',
    'tools/checkpoints/checkpoint-69d/apply_checkpoint_69d.ps1',
    'tools/checkpoints/checkpoint-69d/test_checkpoint_69d_contract.ps1',
...
foreach ($definitionDocument in @($normal, $deep)) {
    Assert-True ([bool]$definitionDocument.nativeDependencyPrecheck.required) 'Checkpoint 69d definitions must require the native dependency precheck.'
    Assert-True ((@($definitionDocument.nativeDependencyPrecheck.powerShellPaths) -join '|') -eq ($expectedGuardedPs -join '|')) 'Checkpoint 69d native dependency PowerShell path binding mismatch.'
    Assert-True ((@($definitionDocument.nativeDependencyPrecheck.checkpointDefinitionPaths) -join '|') -eq ($expectedGuardedDefs -join '|')) 'Checkpoint 69d native dependency definition binding mismatch.'
}
Assert-True (@($normal.stages).Count -eq 11) 'Checkpoint 69d normal suite must contain 11 stages.'
Assert-True (@($deep.stages).Count -eq 30) 'Checkpoint 69d Deep Calibration must contain 30 stages.'
Assert-True ([int]$normal.checkpointMetrics.monteCarloVariantCount -eq 72 -and [int]$normal.checkpointMetrics.trialsAtDefault -eq 720000 -and [int]$normal.checkpointMetrics.smokeTrialsAtDefault -eq 72 -and [int]$normal.checkpointMetrics.totalTrialExecutionsAtDefault -eq 720072) 'Checkpoint 69d normal Monte Carlo/smoke workload mismatch.'
Assert-True ([int]$deep.checkpointMetrics.monteCarloVariantCount -eq 1616 -and [int]$deep.checkpointMetrics.trialsAtDefault -eq 16160000 -and [int]$deep.checkpointMetrics.smokeTrialsAtDefault -eq 72 -and [int]$deep.checkpointMetrics.totalTrialExecutionsAtDefault -eq 16160072) 'Checkpoint 69d Deep Calibration workload mismatch.'

$policyPath = Join-Path $repositoryRoot 'docs\design\testing\checkpoint_69_validation_suite_policy_v0_1.json'
$policy = Get-Content -LiteralPath $policyPath -Raw | ConvertFrom-Json
Assert-True ([int]$policy.normal.stageCount -eq 11 -and [int]$policy.normal.smokeTrials -eq 72 -and [int]$policy.normal.totalTrialExecutions -eq 720072) 'Checkpoint 69d validation policy normal-stage/smoke metadata mismatch.'
Assert-True ([int]$policy.deepCalibration.stageCount -eq 30 -and [int]$policy.deepCalibration.smokeTrials -eq 72 -and [int]$policy.deepCalibration.totalTrialExecutions -eq 16160072) 'Checkpoint 69d validation policy Deep Calibration metadata mismatch.'
$mustAlways = @($policy.mustAlwaysRunStageIds)
Assert-True ($mustAlways -contains 'tl1-sensor-ew-candidate-operational-preflight' -and $mustAlways -contains 'tl1-sensor-ew-candidate-operational-smoke' -and $mustAlways -contains 'tl1-sensor-ew-candidate-operational-combat') 'Checkpoint 69d validation policy must require candidate preflight, smoke, and substantive combat stages.'
...
# CP69d gate-semantics regression learned from the native CP69c Stage 12 smoke failure.
Assert-True (-not $runnerSource.Contains('result.MeanActiveSensorPowerCommittedA <= 1.0 + tolerance')) 'Checkpoint 69d gate regression: cumulative per-trial Active Sensor power must not be compared directly with the 1-TP per-use normal-mode cost.'
Assert-True (-not $runnerSource.Contains('result.MeanActiveSensorPowerCommittedB <= 1.0 + tolerance')) 'Checkpoint 69d gate regression: Side B cumulative per-trial Active Sensor power must not be compared directly with the 1-TP per-use normal-mode cost.'
Assert-True ($runnerSource.Contains('Math.Abs(result.MeanActiveSensorPowerCommittedA -') -and $runnerSource.Contains('result.MeanActiveSensorPoweredTurnsA) <= tolerance')) 'Checkpoint 69d gate regression: Side A clear-normal power must be validated as cumulative TP equal to powered sensor evaluations for the 1-TP normal mode.'
Assert-True ($runnerSource.Contains('Math.Abs(result.MeanActiveSensorPowerCommittedB -') -and $runnerSource.Contains('result.MeanActiveSensorPoweredTurnsB) <= tolerance')) 'Checkpoint 69d gate regression: Side B clear-normal power must be validated as cumulative TP equal to powered sensor evaluations for the 1-TP normal mode.'

# CP69d runtime-binding regression learned from the native CP69b Stage 11 failure.
$preflightStage = @($normal.stages | Where-Object { [string]$_.id -eq 'tl1-sensor-ew-candidate-operational-preflight' })
Assert-True ($preflightStage.Count -eq 1) 'Checkpoint 69d normal suite must contain exactly one executable Sensor/EW candidate preflight stage.'
Assert-True ([string]$preflightStage[0].command -eq 'tl1-integrated-tactical-combat-preflight') 'Checkpoint 69d candidate preflight must invoke the actual integrated tactical-combat consumer in preflight mode.'
Assert-True (-not [bool]$preflightStage[0].metrics.usesTrials -and [bool]$preflightStage[0].metrics.actualConsumerDeserializer -and [bool]$preflightStage[0].metrics.catalogBindingRequired) 'Checkpoint 69d candidate preflight metrics must bind actual consumer deserialization without Monte Carlo trials.'
$deepPreflightStage = @($deep.stages | Where-Object { [string]$_.id -eq 'tl1-sensor-ew-candidate-operational-preflight' })
Assert-True ($deepPreflightStage.Count -eq 1) 'Checkpoint 69d Deep Calibration must also contain the executable Sensor/EW candidate preflight stage.'
$smokeStage = @($normal.stages | Where-Object { [string]$_.id -eq 'tl1-sensor-ew-candidate-operational-smoke' })
Assert-True ($smokeStage.Count -eq 1) 'Checkpoint 69d normal suite must contain exactly one 72-variant execution smoke stage.'
Assert-True ([string]$smokeStage[0].command -eq 'tl1-integrated-tactical-combat') 'Checkpoint 69d smoke stage must invoke the full integrated tactical-combat runner.'
Assert-True ((@($smokeStage[0].arguments) -join '|').Contains('--trials|1')) 'Checkpoint 69d smoke stage must execute exactly one trial per variant.'
Assert-True ([int]$smokeStage[0].metrics.variantCount -eq 72 -and [int]$smokeStage[0].metrics.totalSmokeTrials -eq 72 -and [bool]$smokeStage[0].metrics.fullPipelineExecution) 'Checkpoint 69d smoke stage must execute the full 72-variant pipeline for 72 total smoke trials.'
$deepSmokeStage = @($deep.stages | Where-Object { [string]$_.id -eq 'tl1-sensor-ew-candidate-operational-smoke' })
Assert-True ($deepSmokeStage.Count -eq 1) 'Checkpoint 69d Deep Calibration must also contain the 72-variant execution smoke stage.'
...
Assert-True ($loadSensorStart -ge 0 -and $loadSensorEnd -gt $loadSensorStart) 'Checkpoint 69d catalog-binding regression: unable to isolate LoadSensorEwProfiles.'
$loadSensorText = $runnerSource.Substring($loadSensorStart, $loadSensorEnd - $loadSensorStart)
Assert-True ($loadSensorText.Contains('PropertyNameCaseInsensitive = true')) 'Checkpoint 69d catalog-binding regression: integrated Sensor/EW catalog deserialization must accept the authoritative camelCase catalog.'
Assert-True ($loadSensorText.Contains('if (catalog.Candidates is null)')) 'Checkpoint 69d catalog-binding regression: integrated Sensor/EW catalog loader must reject an unbound Candidates collection explicitly.'
Assert-True ($loadSensorText.Contains('catalog.Candidates.ToDictionary(')) 'Checkpoint 69d catalog-binding regression: candidate dictionary construction must occur only after the binding guard.'
...
Assert-True ($programSource.Contains('Console.Error.WriteLine($"ERROR: {exception}");')) 'Checkpoint 69d diagnostics must print the full top-level exception so native-only failures retain stack traces.'
...
Assert-True (-not $runnerSource.Contains('v.ProtectedCompartmentation || !v.BaseShieldRechargeEnabled ||')) 'CP69d compile regression: nullable BaseShieldRechargeEnabled may not be negated directly inside a non-nullable || chain.'
Assert-True ($runnerSource.Contains('v.ProtectedCompartmentation || !(v.BaseShieldRechargeEnabled ?? true) ||')) 'CP69d compile regression: fixed-control shield check must coalesce nullable BaseShieldRechargeEnabled to its existing default true value before negation.'
Assert-True (-not $runnerSource.Contains('v.EvasiveManeuversEnabled || !v.PdsEnabled || v.EscapeDisengagementEnabled ||')) 'CP69d compile regression: nullable Evasive/PDS/Escape controls may not be used directly inside a non-nullable || chain.'
Assert-True ($runnerSource.Contains('v.EvasiveManeuversEnabled != false || v.PdsEnabled != true ||')) 'CP69d compile regression: fixed-control validation must require explicit Evasive=false and PDS=true values.'
Assert-True ($runnerSource.Contains('v.EscapeDisengagementEnabled != false ||')) 'CP69d compile regression: fixed-control validation must require explicit EscapeDisengagement=false.'
...
Assert-True ($reviewStart -ge 0 -and $reviewEnd -gt $reviewStart) 'CP69d compile regression: unable to isolate Sensor/EW candidate review writer.'
$reviewText = $runnerSource.Substring($reviewStart, $reviewEnd - $reviewStart)
Assert-True ($reviewText.Contains('static double Conditional(Tl1IntegratedTacticalCombatVariantSummary result)')) 'CP69d compile regression: Sensor/EW candidate review writer must define its own method-local Conditional helper.'
Assert-True ($reviewText.Contains('F(Conditional(result))')) 'CP69d compile regression: candidate review output must continue using the local conditional-win helper.'

# CP69d is a gate-semantics-only successor hotfix: pin all non-runner Sensor/EW authority and study inputs to the original CP69 package.
$cp69FrozenHashes = @{
...
foreach ($entry in $cp69FrozenHashes.GetEnumerator()) {
    ...
    Assert-True ($actual -eq [string]$entry.Value) "CP69d runtime-binding hotfix changed frozen CP69 authority/input: $($entry.Key)."
}

$validationFiles = @(Get-ChildItem -LiteralPath (Join-Path $repositoryRoot 'docs\validation') -File -Filter 'Checkpoint_*.md')
Assert-True ($validationFiles.Count -eq 1 -and $validationFiles[0].Name -eq 'Checkpoint_69d_Sensor_EW_Candidate_Active_Power_Gate_Hotfix.md') 'Exactly one Checkpoint 69d active validation runbook must remain.'
$rootTxt = @(Get-ChildItem -LiteralPath $repositoryRoot -File -Filter '*.txt')
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_69D_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_69D_SHA256SUMS.txt as .txt.'
...
Write-Host '       Checkpoint 69d native dependency contract: PowerShell + pinned .NET only; active Python runtime dependencies are rejected before native work.'
...
Write-Host 'Checkpoint 69d active-power gate hotfix contract validation passed.'
