...
$contractCheck = Join-Path $PSScriptRoot 'test_checkpoint_69d_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-69d-deep-calibration.json' } else { 'checkpoint-69d.json' }
$definition = Join-Path $repositoryRoot (Join-Path 'tools\calibration\checkpoints' $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-69d.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-69d-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!69D(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!69d(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!69d(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_69d_Sensor_EW_Candidate_Active_Power_Gate_Hotfix.md') {
