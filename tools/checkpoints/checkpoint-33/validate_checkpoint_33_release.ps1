...
$stagingRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-33-release-{0}" -f ([guid]::NewGuid().ToString('N')))
...
function Assert-ArchiveContract {
    ...
    try {
        ...
        $manifestEntry = $zip.GetEntry('CHECKPOINT_33_SHA256SUMS.txt')
        ...
        foreach ($line in ($manifestText -split '\r?\n')) {
            ...
            if ($name -eq 'CHECKPOINT_33_SHA256SUMS.txt') { throw 'Archive manifest must not contain itself.' }
        ...
        if ($manifest.Count -ne 673) { throw "Checkpoint 33 archive manifest must contain exactly 673 files; found $($manifest.Count)." }

        $actualNames = @($entries | ForEach-Object { $_.FullName.Replace('\','/') } | Where-Object { $_ -ne 'CHECKPOINT_33_SHA256SUMS.txt' })
...
try {
    ...
    $checkpointScript = Join-Path $stagingRoot 'tools\checkpoints\checkpoint-33\apply_checkpoint_33.ps1'
    $staticPreflight = Join-Path $stagingRoot 'tools\checkpoints\checkpoint-33\static_preflight_checkpoint_33.ps1'
    $runtimeRegistry = Join-Path $stagingRoot 'tools\checkpoints\checkpoint-33\checkpoint_runtime_registry.ps1'
    if (-not (Test-Path -LiteralPath $checkpointScript -PathType Leaf)) { throw "Checkpoint 33 application script was not found in $resolvedArchive." }
    if (-not (Test-Path -LiteralPath $staticPreflight -PathType Leaf)) { throw "Checkpoint 33 static preflight was not found in $resolvedArchive." }
    if (-not (Test-Path -LiteralPath $runtimeRegistry -PathType Leaf)) { throw "Checkpoint 33 runtime registry was not found in $resolvedArchive." }
    ...
    if (-not $RepositoryContractOnly) {
        Write-Host '[4/5] Running clean build, 674 tests, retained lanes, 294 correction variants, and self-tests...'
        & $checkpointScript
    }
    else {
    ...
    Write-Host ''
    Write-Host 'Checkpoint 33 release validation completed successfully.'
