...
function Test-NormalizedRelativePathCompatibility {
    $testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-33-relative-path-test-{0}" -f ([guid]::NewGuid().ToString('N')))
...
function Ensure-ActiveConceptDocument {
    ...
    if ($activeConcepts.Count -ne 1 -or $activeConcepts[0].Name -ne $ExpectedFileName) {
        ...
        throw "Exactly the Checkpoint 33 concept document must remain active. Found: $activeNames"
    }
    Write-Host "       Active concept document: $ExpectedFileName"
}

function Test-ActiveConceptDocumentNormalization {
    $testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-33-concept-test-{0}" -f ([guid]::NewGuid().ToString('N')))
    $docsDirectory = Join-Path $testRoot 'docs'
    $archiveDirectory = Join-Path $docsDirectory 'archive'
    $expectedName = 'Star_Cluster_Game_Concept_v0.4e.docx'
...
function Assert-RepositoryManifest {
    ...
    foreach ($line in Get-Content -LiteralPath $ManifestPath) {
        ...
        if ($relativePath -eq 'CHECKPOINT_33_SHA256SUMS.txt') {
    ...
    if ($verified -ne 673) { throw "Repository manifest verified $verified files; the Checkpoint 33 archive requires exactly 673 files." }
...
function Ensure-ActiveValidationRunbook {
    ...
    if ($activeRunbooks.Count -ne 1 -or $activeRunbooks[0].Name -ne $ExpectedFileName) {
        ...
        throw "Exactly the Checkpoint 33 validation runbook must remain active. Found: $activeNames"
    }
    Write-Host "       Active validation runbook: $ExpectedFileName"
}

function Test-ActiveValidationRunbookNormalization {
    $testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-33-runbook-test-{0}" -f ([guid]::NewGuid().ToString('N')))
    $validationDirectory = Join-Path $testRoot 'validation'
    $archiveDirectory = Join-Path $validationDirectory 'archive'
    $expectedName = 'Checkpoint_33_TL1_Main_Weapon_Power_And_Interception_Order_Correction.md'
...
function Assert-TechnologyCsvContracts {
    ...
    $loadoutPath = '.\docs\design\player_technology\tl1_core_combat_loadouts_v0_2.csv'
    $scenarioPath = '.\docs\design\player_technology\tl1_core_combat_test_scenarios_v0_3.csv'
    ...
    if ($baselineHash -ne '11f533483d502ffa6b8dd2df5c0659e07d2281f0c2fd0b8b1360340a2cf06991') { throw "TL1 baseline SHA-256 is $baselineHash, not the accepted TL1 baseline hash." }
    $requiredValues = @{
        ...
        'kinetic_ammo'='100'; 'kinetic_power'='1'; 'missile_ammo'='25'; 'missile_launch_power'='0'; 'ammunition_ready_package'='1'; 'kinetic_pds_ammo'='50'; 'amm_pds_ammo'='25'; 'shield_overcapacity_amount'='1'; 'aux_reactor_output'='1'; 'aux_reactor_degraded_output'='0'
    }
    ...
    if (($scenarios[0].PSObject.Properties.Name -join ',') -ne $expectedHeader) { throw 'TL1 scenario matrix v0.3 header is invalid.' }
...
function Assert-Tl1ScenarioCorpusContracts {
    ...
    $matrixPath = '.\docs\design\player_technology\tl1_core_combat_test_scenarios_v0_3.csv'
    ...
    $implemented = @($matrix | Where-Object { $_.implementation_status -like 'implemented_checkpoint_*' })
...
function Assert-PdsStudyContracts {
    ...
    $baselinePath = '.\docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_1.csv'
    $expectedBaselineHash = '11f533483d502ffa6b8dd2df5c0659e07d2281f0c2fd0b8b1360340a2cf06991'
...
function Assert-WorkbookContract {
    $path = '.\docs\design\player_technology\StarCluster_Player_TL_Framework_Draft_v0_13.xlsx'
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw 'Technology workbook v0.13 was not found.' }
    Add-Type -AssemblyName System.IO.Compression.FileSystem
    $zip = [System.IO.Compression.ZipFile]::OpenRead((Resolve-Path -LiteralPath $path).Path)
    try {
        ...
        $requiredSheets = @('Overview','TL1 Baseline','TL1 Loadouts','TL1 Test Matrix','Phase A Runtime','TL1 Phase B','TL1 Calibration','Checkpoint 28 Energy','Checkpoint 29 Matrix','Checkpoint 30 PDS','Checkpoint 31 Defense','Checkpoint 33 Correction','Component Schema','Checkpoint 25 Plan','TL Matrix','Components','Compatibility Profiles','Adaptation Rules','Reference Library','Reference Insights','Design Reconciliation','Level Themes','Design Decisions','Sources Used')
        ...
        foreach ($marker in @('D-257','Checkpoint 29 - Complete TL1 Weapon Matrix','48 variants x 10,000 trials','D-261','Checkpoint 30 PDS Control - Corrected by Checkpoint 31','59 variants x 10,000 trials','D-269','Checkpoint 31 - TL1 Layered Defensive Systems Calibration','171 variants x 10,000 trials','Checkpoint 33 - TL1 Main-Weapon Power and Interception-Order Correction','294 variants x 10,000 trials','Kinetic Cannon','Held Main then PDS','Operational +1 / Degraded +0')) {
            if (-not $allXml.Contains($marker)) { throw "Workbook does not contain required marker $marker." }
        }
    }
    finally { $zip.Dispose() }
    Write-Host '       Workbook OOXML: retained Checkpoint 29 Matrix, corrected Checkpoint 30 PDS, retained Checkpoint 31 Defense, new Checkpoint 33 Correction, Decisions through D-285, and no structured table parts verified.'
}
...
function Assert-NewUnitTestContracts {
    ...
    if ($directFacts -ne 10 -or $directTheories -ne 3 -or $kineticFacts -ne 8 -or $energyFacts -ne 10 -or $matrixFacts -ne 8 -or $pdsFacts -ne 15 -or $defenseFacts -ne 10 -or $weaponFacts -ne 14) {
        throw "Unit-test source counts differ from the Checkpoint 33 contract: direct $directFacts/$directTheories, kinetic $kineticFacts, energy $energyFacts, matrix $matrixFacts, PDS $pdsFacts, defense $defenseFacts, weapon $weaponFacts."
    }
    Assert-FileContains $pdsPath @('Operational_targeting_computer_assists_pds_by_ten_points','Degraded_targeting_computer_assists_pds_by_five_points','Own_evm_does_not_reduce_amm_interception_chance','Kinetic_pds_consumes_and_reloads_one_ready_package_on_an_intercept') 'Checkpoint 33 PDS tests'
    ...
    Assert-FileContains $weaponPath @('AmmunitionFedWeaponStartsWithOneReadyPackage','AutomaticLoaderKeepsOnePackageReadyUntilTheLastShot') 'Checkpoint 33 Ready Package tests'
    Write-Host '       Checkpoint 33 unit-test sources: 15 PDS facts, 10 defensive facts, Ready Package coverage, and retained direct-fire/kinetic/energy/matrix contracts present.'
}
...
try {
    Write-Host '[1/15] Verifying complete Checkpoint 33 repository and parser contracts...'
    foreach ($requiredFile in @(
        '.\StarCluster.sln',
        '.\global.json',
        '.\CHECKPOINT_33_SHA256SUMS.txt',
        '.\Checkpoint_33_Readme.txt',
        '.\README.md',
        '.\docs\Star_Cluster_Game_Concept_v0.4e.docx',
        '.\docs\archive\Star_Cluster_Game_Concept_v0.4c.docx',
        '.\docs\checkpoints\Checkpoint_33_TL1_Main_Weapon_Power_And_Interception_Order_Correction.md',
        '.\docs\validation\Checkpoint_33_TL1_Main_Weapon_Power_And_Interception_Order_Correction.md',
        '.\docs\validation\archive\Checkpoint_31_TL1_Layered_Defensive_Systems_Calibration.md',
        '.\docs\design\player_technology\StarCluster_Player_TL_Framework_Draft_v0_13.xlsx',
        '.\docs\design\player_technology\TL1_Main_Weapon_Power_And_Interception_Correction_Plan_v0_1.md',
        '.\docs\design\player_technology\tl1_power_envelope_calibration_schema_v0_2.json',
        '.\src\StarCluster.ScenarioRunner\Scenarios\TL1Calibration\tl1-pe02-main-power-interception-correction-study.json',
        ...
        '.\tools\checkpoints\checkpoint-33\static_preflight_checkpoint_33.ps1',
        '.\tools\checkpoints\checkpoint-33\checkpoint_runtime_registry.ps1'
    )) {
        if (-not (Test-Path -LiteralPath $requiredFile -PathType Leaf)) { throw "Required Checkpoint 33 file $requiredFile was not found." }
    }
    ...
    Assert-RepositoryManifest -ManifestPath '.\CHECKPOINT_33_SHA256SUMS.txt'
    Assert-PowerShellScriptsParse -RootPath '.'
    Ensure-ActiveValidationRunbook -ValidationDirectory '.\docs\validation' -ExpectedFileName 'Checkpoint_33_TL1_Main_Weapon_Power_And_Interception_Order_Correction.md' -ArchiveDirectory '.\docs\validation\archive'
    Test-ActiveValidationRunbookNormalization
    Ensure-ActiveConceptDocument -DocsDirectory '.\docs' -ExpectedFileName 'Star_Cluster_Game_Concept_v0.4e.docx' -ArchiveDirectory '.\docs\archive'
    ...
    if (($staticOutput | Out-String) -notmatch 'Checkpoint 33 static preflight completed successfully\.') { throw 'Checkpoint 33 static preflight did not report successful completion.' }

    Write-Host ''
    Write-Host 'Checkpoint 33 repository-contract preflight completed successfully.'
    ...
    if ($godotProcesses) {
        ...
        throw "Close Godot before applying Checkpoint 33. Running process(es): $processNames"
    }
    ...
    Write-Host '[5/15] Running 674 engine-independent tests...'
    ...
    if ($testText -notmatch 'Passed:\s+674') { throw 'The complete suite did not report the expected 674 passed tests.' }

    Write-Host '[6/15] Running seven accepted deterministic moving-missile scenarios...'
    $legacyOutput = '.\out\checkpoint-33-deterministic'
    Remove-Item $legacyOutput -Recurse -Force -ErrorAction SilentlyContinue
    $legacyText = Invoke-Runner -Arguments @('run-all','--scenario-dir','.\src\StarCluster.ScenarioRunner\Scenarios','--output-dir',$legacyOutput) -Description 'Checkpoint 33 legacy deterministic corpus'
    ...
    $phaseAOutput = '.\out\checkpoint-33-tl1-phase-a'
    Remove-Item $phaseAOutput -Recurse -Force -ErrorAction SilentlyContinue
    $phaseAText = Invoke-Runner -Arguments @('tl1-phase-a','--scenario-dir','.\src\StarCluster.ScenarioRunner\Scenarios\TL1PhaseA','--baseline-file','.\docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_1.csv','--output-dir',$phaseAOutput) -Description 'Checkpoint 33 TL1 Phase A corpus'
    ...
    $phaseBOutput = '.\out\checkpoint-33-tl1-phase-b'
    Remove-Item $phaseBOutput -Recurse -Force -ErrorAction SilentlyContinue
    $phaseBText = Invoke-Runner -Arguments @('tl1-phase-b','--output-dir',$phaseBOutput) -Description 'Checkpoint 33 TL1 Phase B corpus'
    ...
    $phaseCOutput = '.\out\checkpoint-33-tl1-kinetic-calibration'
    Remove-Item $phaseCOutput -Recurse -Force -ErrorAction SilentlyContinue
    $phaseCText = Invoke-Runner -Arguments @('tl1-kinetic-calibration','--study-file','.\src\StarCluster.ScenarioRunner\Scenarios\TL1Calibration\tl1-kc01-kinetic-interaction-study.json','--baseline-file','.\docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_1.csv','--trials','10000','--jobs','24','--output-dir',$phaseCOutput) -Description 'Checkpoint 33 TL1 kinetic calibration study'
    ...
    $phaseGOutput = '.\out\checkpoint-33-tl1-defensive-calibration'
    Remove-Item $phaseGOutput -Recurse -Force -ErrorAction SilentlyContinue
    $phaseGText = Invoke-Runner -Arguments @('tl1-defensive-calibration','--study-file','.\src\StarCluster.ScenarioRunner\Scenarios\TL1Calibration\tl1-ds01-layered-defensive-systems-study.json','--baseline-file','.\docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_1.csv','--trials','10000','--jobs','24','--output-dir',$phaseGOutput) -Description 'Checkpoint 33 TL1 layered defensive calibration study'
    ...
    Write-Host '[14/15] Running 294 TL1 main-power/interception correction variants at 10,000 trials each...'
    $phaseHOutput = '.\out\checkpoint-33-tl1-power-envelope-calibration'
    Remove-Item $phaseHOutput -Recurse -Force -ErrorAction SilentlyContinue
    $phaseHText = Invoke-Runner -Arguments @('tl1-power-envelope-calibration','--study-file','.\src\StarCluster.ScenarioRunner\Scenarios\TL1Calibration\tl1-pe02-main-power-interception-correction-study.json','--baseline-file','.\docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_1.csv','--trials','10000','--jobs','24','--output-dir',$phaseHOutput) -Description 'Checkpoint 33 TL1 main-power/interception correction study'
    if ($phaseHText -notmatch 'TL1 Power Correction preflight:\s+294 variants, focused reactor outputs 3-6, 1-TP Kinetic fire, zero-TP Missile launches, 1-TP Auxiliary Reactors, revised Shield overcapacity, Held Main before PDS, and exact reciprocal pairs verified; passed\.' -or $phaseHText -notmatch 'TL1 Power Correction Calibration:\s+294 variants, 10000 trials each, 0 failed gates\.') { throw 'The TL1 main-power/interception correction study did not report 294 variants and zero failed gates.' }

    Write-Host '[15/15] Running forty-six ScenarioRunner self-tests...'
    $selfTestText = Invoke-Runner -Arguments @('self-test','--scenario-file','.\src\StarCluster.ScenarioRunner\Scenarios\terminal-two-window-hit.json') -Description 'Checkpoint 33 runner self-tests'
    ...
    Write-Host ''
    Write-Host 'Checkpoint 33 completed successfully.'
    Write-Host 'Engine-independent tests passed: 674.'
    ...
    Write-Host 'TL1 main-power/interception correction calibration passed: 294 variants / 10,000 trials each / 0 failed gates.'
    Write-Host 'Runner self-tests passed: 46.'
    Write-Host 'TL1 main-weapon power and interception-order correction completed; no mechanical Godot validation is required.'
}
