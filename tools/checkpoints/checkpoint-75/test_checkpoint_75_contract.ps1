...
$normalRel = 'tools/calibration/checkpoints/checkpoint-75.json'
$deepRel = 'tools/calibration/checkpoints/checkpoint-75-deep-calibration.json'
$guardedPs = @(
...
Write-Host '       Validating Checkpoint 75 definitions and workload accounting...'
$normal = Read-Json $normalRel
$deep = Read-Json $deepRel
Assert-True ([string]$normal.checkpointId -eq '75' -and [string]$deep.checkpointId -eq '75') 'Checkpoint 75 definition ID mismatch.'
Assert-True ([string]$normal.manifestFile -eq 'CHECKPOINT_75_SHA256SUMS.txt' -and [string]$deep.manifestFile -eq 'CHECKPOINT_75_SHA256SUMS.txt') 'Checkpoint 75 manifest binding mismatch.'
Assert-True ([string]$normal.outputRoot -eq 'out/checkpoint-75' -and [string]$deep.outputRoot -eq 'out/checkpoint-75-deep-calibration') 'Checkpoint 75 output-root binding mismatch.'
Assert-True ((@($normal.nativeDependencyPrecheck.powerShellPaths) -join '|') -eq ($guardedPs -join '|')) 'Checkpoint 75 normal native-dependency PowerShell path binding mismatch.'
Assert-True ((@($deep.nativeDependencyPrecheck.powerShellPaths) -join '|') -eq ($guardedPs -join '|')) 'Checkpoint 75 deep native-dependency PowerShell path binding mismatch.'
Assert-True ((@($normal.nativeDependencyPrecheck.checkpointDefinitionPaths) -join '|') -eq ($guardedDefs -join '|')) 'Checkpoint 75 normal native-dependency definition binding mismatch.'
Assert-True ((@($deep.nativeDependencyPrecheck.checkpointDefinitionPaths) -join '|') -eq ($guardedDefs -join '|')) 'Checkpoint 75 deep native-dependency definition binding mismatch.'
Assert-True (@($normal.stages).Count -eq 11 -and @($deep.stages).Count -eq 30) 'Checkpoint 75 stage-count mismatch.'
Assert-True ([int]$normal.checkpointMetrics.stageCount -eq 11 -and [int]$normal.checkpointMetrics.monteCarloVariantCount -eq 40 -and [int]$normal.checkpointMetrics.trialsAtDefault -eq 400000 -and [int]$normal.checkpointMetrics.smokeVariantExecutions -eq 40 -and [int]$normal.checkpointMetrics.smokeTrialsAtDefault -eq 40 -and [int]$normal.checkpointMetrics.totalTrialExecutionsAtDefault -eq 400040) 'Checkpoint 75 normal workload mismatch.'
Assert-True ([int]$deep.checkpointMetrics.stageCount -eq 30 -and [int]$deep.checkpointMetrics.monteCarloVariantCount -eq 1584 -and [int]$deep.checkpointMetrics.trialsAtDefault -eq 15840000 -and [int]$deep.checkpointMetrics.smokeVariantExecutions -eq 40 -and [int]$deep.checkpointMetrics.smokeTrialsAtDefault -eq 40 -and [int]$deep.checkpointMetrics.totalTrialExecutionsAtDefault -eq 15840040) 'Checkpoint 75 Deep Calibration workload mismatch.'
Assert-True ([string]$normal.primaryStudy.id -eq 'tl1-itc17-applied-degraded-fire-family-candidates' -and [int]$normal.primaryStudy.variantCount -eq 40) 'Checkpoint 75 normal primary-study binding mismatch.'
Assert-True ([string]$deep.primaryStudy.id -eq 'tl1-itc17-applied-degraded-fire-family-candidates' -and [int]$deep.primaryStudy.variantCount -eq 40) 'Checkpoint 75 deep primary-study binding mismatch.'
...
Assert-True (($normalStageIds -join '|') -eq ($expectedNormalStageIds -join '|')) 'Checkpoint 75 normal stage ordering mismatch.'
...
Assert-True ($normalSelfTests.Count -eq 1 -and [int]$normalSelfTests[0].metrics.selfTestCount -eq 47) 'Checkpoint 75 normal ScenarioRunner self-test count mismatch.'
Assert-True ($deepSelfTests.Count -eq 1 -and [int]$deepSelfTests[0].metrics.selfTestCount -eq 47) 'Checkpoint 75 deep ScenarioRunner self-test count mismatch.'
...
$gateStart = $runner.IndexOf('if (study.Id == Tl1AppliedDegradedFireFamilyCandidateStudyId)')
$gateEnd = $runner.IndexOf('if (IsCheckpoint57Study', $gateStart)
Assert-True ($gateStart -ge 0 -and $gateEnd -gt $gateStart) 'Checkpoint 75 could not isolate its release-gate block.'
...
foreach ($gateName in $requiredGates) {
    Assert-True (([regex]::Matches($gateBlock, [regex]::Escape($gateName))).Count -eq 1) "Checkpoint 75 release-gate block must contain exactly one '$gateName' gate."
}
Assert-True ($gateBlock.Contains('Tl1ApproximateTrackFirmReferenceIsClean(') -and $gateBlock.Contains('new[] { 0, 20, 25 }') -and $gateBlock.Contains('MeanDirectShotsFired') -and $gateBlock.Contains('MeanPreventedTrackUnavailable')) 'Checkpoint 75 gate block telemetry/wiring mismatch.'
Assert-True ($gateBlock.Contains('Family assignment, -20 versus -25 preference') -and $gateBlock.Contains('missile and torpedo terminal requirements are independent and remain Firm-gated')) 'Checkpoint 75 release gates must keep outcome choice review-only and missiles separate.'
...
Assert-True ($concepts.Count -eq 1 -and $concepts[0].Name -eq 'Star_Cluster_Game_Concept_v0.6n.docx') 'Checkpoint 75 must expose exactly one active Concept v0.6n.'
Assert-True (Test-Path -LiteralPath (Join-Path $repositoryRoot 'docs/archive/Star_Cluster_Game_Concept_v0.6m.docx') -PathType Leaf) 'Checkpoint 75 must archive Concept v0.6m.'
...
Assert-True ($conceptHash -eq '9e60cc5e8d7998d341dd84fdf393828226e2bcd83baa1f4723c8ed56c754b665') 'Checkpoint 75 active Concept v0.6n content hash mismatch.'
Assert-True (Test-Path -LiteralPath (Join-Path $repositoryRoot 'docs/validation/archive/Checkpoint_74d_Degraded_Fire_Firm_Reference_Gate_Semantics_Hotfix.md') -PathType Leaf) 'Checkpoint 75 must archive the accepted CP74d validation runbook.'
$validationFiles = @(Get-ChildItem -LiteralPath (Join-Path $repositoryRoot 'docs\validation') -File -Filter 'Checkpoint_*.md')
Assert-True ($validationFiles.Count -eq 1 -and $validationFiles[0].Name -eq 'Checkpoint_75_Applied_Degraded_Fire_Family_Candidates_And_Missile_Terminal_Guardrails.md') 'Checkpoint 75 must expose exactly one active validation runbook.'
$rootTxt = @(Get-ChildItem -LiteralPath $repositoryRoot -File -Filter '*.txt')
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_75_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_75_SHA256SUMS.txt as .txt.'
...
Assert-True ($rootReadme.Contains('Checkpoint 75') -and $rootReadme.Contains('Concept v0.6n') -and $rootReadme.Contains('checkpoint-75/apply_checkpoint_75.ps1')) 'Checkpoint 75 root README is stale.'
Assert-True ($docsReadme.Contains('Checkpoint 75') -and $docsReadme.Contains('Star_Cluster_Game_Concept_v0.6n.docx') -and $docsReadme.Contains('Checkpoint_75_Applied_Degraded_Fire_Family_Candidates_And_Missile_Terminal_Guardrails.md')) 'Checkpoint 75 documentation README is stale.'
Assert-True ($todo.Contains('Checkpoint 75') -and $todo.Contains('400,000') -and $todo.Contains('degraded-fire-applied-review.csv')) 'Checkpoint 75 prototype TODO is stale.'

Write-Host '       CP75 applied direct-fire study: 40 Kinetic/Energy variants, -20/-25 only, outcomes review-only.'
Write-Host '       CP75 missile guardrails: live Firm command guidance, capability-gated peer authority, local-track seeker refinement.'
Write-Host '       CP74 degraded-fire foundation and accepted CP73 AI doctrine remain frozen.'
Write-Host 'Checkpoint 75 repository and cross-study contract validation passed.'
