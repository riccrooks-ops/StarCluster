...
$normalRel='tools/calibration/checkpoints/checkpoint-86.json'
$deepRel='tools/calibration/checkpoints/checkpoint-86-deep-calibration.json'
$guardedPs=@(
...
Write-Host '       Validating Checkpoint 86 definitions and workload accounting...'
$normal=Read-Json $normalRel
$deep=Read-Json $deepRel
Assert-True ([string]$normal.checkpointId -eq '86' -and [string]$deep.checkpointId -eq '86') 'Checkpoint 86 definition ID mismatch.'
Assert-True ([string]$normal.manifestFile -eq 'CHECKPOINT_86_SHA256SUMS.txt' -and [string]$deep.manifestFile -eq 'CHECKPOINT_86_SHA256SUMS.txt') 'Checkpoint 86 manifest binding mismatch.'
Assert-True ([string]$normal.outputRoot -eq 'out/checkpoint-86' -and [string]$deep.outputRoot -eq 'out/checkpoint-86-deep-calibration') 'Checkpoint 86 output-root binding mismatch.'
Assert-True (@($normal.stages).Count -eq 11 -and [int]$normal.checkpointMetrics.stageCount -eq 11) 'Checkpoint 86 normal stage-count mismatch.'
Assert-True ([int]$normal.checkpointMetrics.monteCarloVariantCount -eq 288 -and [int]$normal.checkpointMetrics.trialsAtDefault -eq 2880000 -and [int]$normal.checkpointMetrics.smokeVariantExecutions -eq 288 -and [int]$normal.checkpointMetrics.smokeTrialsAtDefault -eq 288 -and [int]$normal.checkpointMetrics.totalTrialExecutionsAtDefault -eq 2880288) 'Checkpoint 86 normal workload mismatch.'
Assert-True (@($deep.stages).Count -eq 36 -and [int]$deep.checkpointMetrics.stageCount -eq 36 -and [int]$deep.checkpointMetrics.monteCarloVariantCount -eq 1982 -and [int]$deep.checkpointMetrics.trialsAtDefault -eq 19820000 -and [int]$deep.checkpointMetrics.smokeVariantExecutions -eq 438 -and [int]$deep.checkpointMetrics.smokeTrialsAtDefault -eq 438 -and [int]$deep.checkpointMetrics.totalTrialExecutionsAtDefault -eq 19820438) 'Checkpoint 86 Deep Calibration workload mismatch.'
$expectedNormal=@(
...
Assert-True ((@($normal.stages | ForEach-Object {[string]$_.id}) -join '|') -eq ($expectedNormal -join '|')) 'Checkpoint 86 normal stage ordering mismatch.'
$self=@($normal.stages | Where-Object {[string]$_.id -eq 'runner-self-tests'})
Assert-True ($self.Count -eq 1 -and [int]$self[0].metrics.selfTestCount -eq 51) 'Checkpoint 86 must expose 51 ScenarioRunner self-tests.'
Assert-True ([string]$normal.primaryStudy.id -eq 'tl2-itc12-weapon-penetration-layered-defense-permutations' -and [int]$normal.primaryStudy.variantCount -eq 288) 'Checkpoint 86 primary-study binding mismatch.'
Assert-True ((@($normal.nativeDependencyPrecheck.powerShellPaths) -join '|') -eq ($guardedPs -join '|')) 'Checkpoint 86 native-dependency PowerShell binding mismatch.'
Assert-True ((@($normal.nativeDependencyPrecheck.checkpointDefinitionPaths) -join '|') -eq ($guardedDefs -join '|')) 'Checkpoint 86 native-dependency definition binding mismatch.'
...
Assert-True ($suiteJsonText.IndexOf('automaticSymmetricWeaponPromotion',[System.StringComparison]::OrdinalIgnoreCase) -ge 0 -and $suiteJsonText.IndexOf('false',[System.StringComparison]::OrdinalIgnoreCase) -ge 0) 'Standing suite JSON must state that symmetric weapon promotion is false.'
...
Assert-True ($rootReadme.Contains('Checkpoint 86') -and $rootReadme.Contains('CHAT_README.md')) 'Root README is not pointing to Checkpoint 86 and the mandatory chat bootstrap.'
...
Assert-True ($todo.Contains('Prototype TODO - Checkpoint 86') -and $todo.Contains('current-action list') -and $todo.Contains('family-specific weapon-penetration permutations')) 'Prototype TODO is stale or has drifted from CP86.'
...
Assert-True ($activeValidation.Count -eq 1 -and $activeValidation[0].Name -eq 'Checkpoint_86_TL2_Weapon_Penetration_And_Development_Continuity.md') 'Exactly one CP86 active validation runbook must remain.'
...
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_86_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_86_SHA256SUMS.txt as .txt.'
...
Write-Host 'Checkpoint 86 contract validation passed.'
