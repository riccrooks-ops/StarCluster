...
$contractCheck = Join-Path $PSScriptRoot 'test_checkpoint_69_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-69-deep-calibration.json' } else { 'checkpoint-69.json' }
$definition = Join-Path $repositoryRoot (Join-Path 'tools\calibration\checkpoints' $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-69.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-69-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!69(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!69(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!69(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_69_TL1_Sensor_EW_Candidate_Operational_Combat.md') {
            Remove-Item -LiteralPath $file.FullName -Force
        }
    }
}
