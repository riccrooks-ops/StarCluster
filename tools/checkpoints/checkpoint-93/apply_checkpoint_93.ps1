...
$contractCheck = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-93\test_checkpoint_93_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-93-deep-calibration.json' } else { 'checkpoint-93.json' }
$definition = Join-Path $repositoryRoot ('tools\calibration\checkpoints\' + $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-93.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-93-deep-calibration.json'
...
foreach ($file in @(Get-ChildItem -LiteralPath $repositoryRoot -File -Filter '*.txt')) {
    if ($file.Name -match '^CHECKPOINT_(?!93(?:_|$)).*_SHA256SUMS\.txt$' -or
        $file.Name -match '^Checkpoint_(?!93(?:_|$)).*_Readme\.txt$' -or
        $file.Name -match '^checkpoint-(?!93(?:-|$)).*-static-preflight\.txt$') {
...
foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
    if ($file.Name -ne 'Checkpoint_93_Matched_Readiness_Space_And_Population_Weighted_Cross_TL_Screening.md') {
...
if (-not (Test-Path -LiteralPath $testingArchive -PathType Container)) {
    New-Item -ItemType Directory -Path $testingArchive | Out-Null
}
$currentTestingFiles = @(
    'README.md',
    'Checkpoint_93_Validation_Tiers.md',
    'checkpoint_93_validation_suite_policy_v0_1.json',
    'Technology_Integration_Permutation_Suite_Architecture_v0_10.md',
    'technology_integration_permutation_suite_v0_10.json'
)
foreach ($file in @(Get-ChildItem -LiteralPath $testingRoot -File)) {
    if (-not ($currentTestingFiles -contains $file.Name)) {
...
if (-not (Test-Path -LiteralPath $conceptArchive -PathType Container)) {
    New-Item -ItemType Directory -Path $conceptArchive -Force | Out-Null
}
...
Write-Host '[3/4] Verifying Checkpoint 93 matched/readiness/Space/population contracts...'
