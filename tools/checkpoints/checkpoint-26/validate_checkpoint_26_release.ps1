...
$stagingRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-26-release-{0}" -f ([guid]::NewGuid().ToString('N')))
...
function Assert-ArchiveContract {
    ...
    try {
        ...
        $manifestEntry = $zip.GetEntry('CHECKPOINT_26_SHA256SUMS.txt')
        ...
        foreach ($line in ($manifestText -split '\r?\n')) {
            ...
            if ($name -eq 'CHECKPOINT_26_SHA256SUMS.txt') { throw 'Archive manifest must not contain itself.' }
            if ($manifest.ContainsKey($name)) { throw "Duplicate archive manifest path: $name" }
            $manifest[$name] = $m.Groups[1].Value.ToLowerInvariant()
        }
        if ($manifest.Count -ne 570) { throw "Checkpoint 26 archive manifest must contain exactly 570 files; found $($manifest.Count)." }
        $actualNames = @($entries | ForEach-Object { $_.FullName.Replace('\','/') } | Where-Object { $_ -ne 'CHECKPOINT_26_SHA256SUMS.txt' })
...
try {
    ...
    $checkpointScript = Join-Path $stagingRoot 'tools\checkpoints\checkpoint-26\apply_checkpoint_26.ps1'
    if (-not (Test-Path -LiteralPath $checkpointScript -PathType Leaf)) { throw "Checkpoint 26 application script was not found in $resolvedArchive." }

    Write-Host '[2/5] Running clean-extraction repository-contract preflight...'
    & $checkpointScript -RepositoryContractOnly

    Write-Host '[3/5] Running clean warning-as-error build, tests, legacy scenarios, Phase A and Phase B corpora, and runner self-tests...'
    & $checkpointScript
    ...
    $archivedRunbook = Join-Path $validationDirectory 'archive\Checkpoint_25_TL1_Deterministic_Layered_Damage_Power_And_Phase_A_Scenario_Foundation.md'
    $staleActiveRunbook = Join-Path $validationDirectory 'Checkpoint_25_TL1_Deterministic_Layered_Damage_Power_And_Phase_A_Scenario_Foundation.md'
    if (-not (Test-Path -LiteralPath $archivedRunbook -PathType Leaf)) { throw 'The packaged Checkpoint 25 archived validation runbook was not found.' }
    Copy-Item -LiteralPath $archivedRunbook -Destination $staleActiveRunbook -Force

    $archivedConcept = Join-Path $stagingRoot 'docs\archive\Star_Cluster_Game_Concept_v0.3w.docx'
    $staleActiveConcept = Join-Path $stagingRoot 'docs\Star_Cluster_Game_Concept_v0.3w.docx'
    if (-not (Test-Path -LiteralPath $archivedConcept -PathType Leaf)) { throw 'The packaged Concept v0.3w archive copy was not found.' }
    ...
    $localArtifacts = @{
        ...
        'out\checkpoint-26-release-test\result.txt' = 'local output'
        'Checkpoint_25_Readme.txt' = 'stale checkpoint identity'
        'CHECKPOINT_25_SHA256SUMS.txt' = 'stale checkpoint identity'
        'StarCluster_Checkpoint_25_Local_Copy.zip' = 'local package copy'
    }
    ...
    if ($activeRunbooks.Count -ne 1 -or $activeRunbooks[0].Name -ne 'Checkpoint_26_TL1_Direct_Fire_Accuracy_Simultaneous_Volley_And_Kinetic_Mirror_Duel_Foundation.md') { throw 'Release validation left an invalid active-runbook set.' }
    ...
    Write-Host ''
    Write-Host 'Checkpoint 26 release validation completed successfully.'
