...
function Test-NormalizedRelativePathCompatibility {
    $testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-26-relative-path-test-{0}" -f ([guid]::NewGuid().ToString('N')))
...
function Ensure-ActiveConceptDocument {
    ...
    if ($activeConcepts.Count -ne 1 -or $activeConcepts[0].Name -ne $ExpectedFileName) {
        ...
        throw "Exactly the Checkpoint 26 concept document must remain active. Found: $activeNames"
    }
    Write-Host "       Active concept document: $ExpectedFileName"
}

function Test-ActiveConceptDocumentNormalization {
    $testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-26-concept-test-{0}" -f ([guid]::NewGuid().ToString('N')))
    $docsDirectory = Join-Path $testRoot 'docs'
    $archiveDirectory = Join-Path $docsDirectory 'archive'
    $expectedName = 'Star_Cluster_Game_Concept_v0.3x.docx'
...
function Assert-RepositoryManifest {
    ...
    foreach ($line in Get-Content -LiteralPath $ManifestPath) {
        ...
        if ($relativePath -eq 'CHECKPOINT_26_SHA256SUMS.txt') {
    ...
    if ($verified -ne 570) { throw "Repository manifest verified $verified files; the Checkpoint 26 archive requires exactly 570 files." }
...
function Ensure-ActiveValidationRunbook {
    ...
    if ($activeRunbooks.Count -ne 1 -or $activeRunbooks[0].Name -ne $ExpectedFileName) {
        ...
        throw "Exactly the Checkpoint 26 validation runbook must remain active. Found: $activeNames"
    }
    Write-Host "       Active validation runbook: $ExpectedFileName"
}

function Test-ActiveValidationRunbookNormalization {
    $testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-26-runbook-test-{0}" -f ([guid]::NewGuid().ToString('N')))
    $validationDirectory = Join-Path $testRoot 'validation'
    $archiveDirectory = Join-Path $validationDirectory 'archive'
    $expectedName = 'Checkpoint_26_TL1_Direct_Fire_Accuracy_Simultaneous_Volley_And_Kinetic_Mirror_Duel_Foundation.md'
...
function Assert-TechnologyCsvContracts {
    ...
    if ($recon.Count -ne 32) { throw "Expected 32 Checkpoint 26 reconciliation rows; found $($recon.Count)." }
    if (($recon[0].PSObject.Properties.Name -join ',') -ne 'topic,current_direction,reference_influence,checkpoint_25_decision,status,next_action') { throw 'Checkpoint 26 reconciliation header is invalid.' }
    ...
    if ($baselineHash -ne '4d1470d5bb858776e96c89a8c93e177d996d76c13dda20a55b6a82ebf2b7d08f') { throw "TL1 baseline SHA-256 is $baselineHash, not the accepted Checkpoint 26 hash." }
    $requiredValues = @{
...
function Assert-Tl1ScenarioCorpusContracts {
    ...
    $expectedHash = '4d1470d5bb858776e96c89a8c93e177d996d76c13dda20a55b6a82ebf2b7d08f'
    ...
    if ($implemented.Count -ne 12) { throw "Expected 12 implemented Checkpoint 26 matrix rows; found $($implemented.Count)." }
...
function Assert-WorkbookContract {
    $path = '.\docs\design\player_technology\StarCluster_Player_TL_Framework_Draft_v0_6.xlsx'
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw 'Technology workbook v0.6 was not found.' }
    Add-Type -AssemblyName System.IO.Compression.FileSystem
    $zip = [System.IO.Compression.ZipFile]::OpenRead((Resolve-Path -LiteralPath $path).Path)
    try {
        ...
        foreach ($sheetName in @('Overview','TL1 Baseline','TL1 Loadouts','TL1 Test Matrix','Phase A Runtime','TL1 Phase B','Component Schema','Checkpoint 25 Plan','TL Matrix','Components','Compatibility Profiles','Adaptation Rules','Reference Library','Reference Insights','Design Reconciliation','Level Themes','Design Decisions','Sources Used')) {
        ...
        foreach ($marker in @('D-244','TL1 Phase B','Phase B cases','36')) {
            if (-not $allXml.Contains($marker)) { throw "Workbook does not contain required Checkpoint 26 marker $marker." }
        }
    }
    finally { $zip.Dispose() }
    Write-Host '       Workbook OOXML: Checkpoint 26 Phase B sheet and Decision D-244 present; no structured table parts.'
}

function Assert-NewUnitTestContracts {
    $path = '.\tests\StarCluster.Tests\Combat\DirectFire\Tl1DirectFireAccuracyTests.cs'
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Checkpoint 26 direct-fire unit-test file was not found." }
    ...
    if ($facts -ne 10 -or $theories -ne 3) { throw "Expected 10 Fact methods and 3 Theory methods in the Checkpoint 26 direct-fire test file; found $facts and $theories." }
    Assert-FileContains $path @('CalculatorMatchesAcceptedTl1Examples','SimultaneousWindowAllowsMutualDestruction','KineticMirrorAllHitsMutuallyDestroysOnTurnTen') 'Checkpoint 26 direct-fire tests'
    Write-Host '       Checkpoint 26 unit-test source: 10 facts and 3 theories present.'
}
...
try {
    Write-Host '[1/9] Verifying complete Checkpoint 26 repository and parser contracts...'
    foreach ($requiredFile in @(
        '.\StarCluster.sln',
        '.\global.json',
        '.\CHECKPOINT_26_SHA256SUMS.txt',
        ...
        '.\docs\Star_Cluster_Game_Concept_v0.3x.docx',
        '.\docs\design\player_technology\TL1_Core_Combat_Test_Plan_v0_3.md',
        '.\docs\design\player_technology\StarCluster_Player_TL_Framework_Draft_v0_6.xlsx',
        '.\docs\checkpoints\Checkpoint_26_TL1_Direct_Fire_Accuracy_Simultaneous_Volley_And_Kinetic_Mirror_Duel_Foundation.md',
        '.\docs\validation\Checkpoint_26_TL1_Direct_Fire_Accuracy_Simultaneous_Volley_And_Kinetic_Mirror_Duel_Foundation.md',
        ...
        if (-not (Test-Path -LiteralPath $requiredFile -PathType Leaf)) { throw "Required Checkpoint 26 file $requiredFile was not found." }
    }
    Ensure-ActiveValidationRunbook -ValidationDirectory '.\docs\validation' -ExpectedFileName 'Checkpoint_26_TL1_Direct_Fire_Accuracy_Simultaneous_Volley_And_Kinetic_Mirror_Duel_Foundation.md' -ArchiveDirectory '.\docs\validation\archive'
    Ensure-ActiveConceptDocument -DocsDirectory '.\docs' -ExpectedFileName 'Star_Cluster_Game_Concept_v0.3x.docx' -ArchiveDirectory '.\docs\archive'
    Test-NormalizedRelativePathCompatibility
    Test-RepositoryManifestLocalArtifactPolicy
    Assert-RepositoryManifest '.\CHECKPOINT_26_SHA256SUMS.txt'
    Assert-PowerShellScriptsParse '.\tools'
    Test-ActiveValidationRunbookNormalization
    Test-ActiveConceptDocumentNormalization

    Write-Host '[2/9] Verifying Concept, reference library, exact data, schema, corpus, tests, and workbook contracts...'
    Assert-FileContains '.\README.md' @('Checkpoint 26','v0.3x.docx','7 documents and 36 cases','not a weapon-balance result') 'Repository README'
    Assert-FileContains '.\docs\checkpoints\Checkpoint_26_TL1_Direct_Fire_Accuracy_Simultaneous_Volley_And_Kinetic_Mirror_Duel_Foundation.md' @('first executable Phase B pass','mutual destruction','seven Phase B documents and 36 deterministic cases','No weapon-balance conclusion') 'Checkpoint 26 documentation contract'
    ...
    Assert-FileContains '.\docs\Prototype_TODO.md' @('Checkpoint 26','7 Phase B scenario documents / 36 cases','simultaneous committed return fire','paired seeded stochastic kinetic mirror trials') 'Prototype TODO'
    ...
    Assert-DocxContains '.\docs\Star_Cluster_Game_Concept_v0.3x.docx' @('Direct-fire accuracy contract','Simultaneous direct-fire window','D-244','seven documents and 36 deterministic cases','END OF DRAFT v0.3x') 'Concept v0.3x'
    ...
    if ($RepositoryContractOnly) {
        Write-Host ''
        Write-Host 'Checkpoint 26 repository-contract preflight completed successfully.'
    ...
    Write-Host '[3/9] Confirming Godot is closed and .NET SDK 8.0.423 is selected...'
    ...
    if ($godotProcesses) {
        ...
        throw "Close Godot before applying Checkpoint 26. Running process(es): $processNames"
    }
    $sdkVersion = dotnet --version
    Write-Host "       SDK: $sdkVersion"
    if ($sdkVersion -ne '8.0.423') { throw "Expected .NET SDK 8.0.423 from global.json, but dotnet selected $sdkVersion." }

    Write-Host '[4/9] Performing a clean compiler preflight with warnings as errors...'
    ...
    Write-Host '[5/9] Running 589 engine-independent tests...'
    $testOutput = & dotnet test '.\tests\StarCluster.Tests\StarCluster.Tests.csproj' --no-build --nologo 2>&1
    $testOutput | Write-Host
    if ($LASTEXITCODE -ne 0) { throw "dotnet test failed with exit code $LASTEXITCODE." }
    if (($testOutput | Out-String) -notmatch 'Passed:\s+589') { throw 'The complete suite did not report the expected 589 passed tests.' }

    Write-Host '[6/9] Running seven accepted deterministic moving-missile scenarios...'
    $legacyOutput = '.\out\checkpoint-26-deterministic'
    Remove-Item $legacyOutput -Recurse -Force -ErrorAction SilentlyContinue
    $legacyText = Invoke-Runner -Arguments @('run-all','--scenario-dir','.\src\StarCluster.ScenarioRunner\Scenarios','--output-dir',$legacyOutput) -Description 'Checkpoint 26 legacy deterministic corpus'
    ...
    Write-Host '[7/9] Running 12 TL1 Phase A documents and 54 mechanics cases...'
    $phaseAOutput = '.\out\checkpoint-26-tl1-phase-a'
    Remove-Item $phaseAOutput -Recurse -Force -ErrorAction SilentlyContinue
    $phaseAText = Invoke-Runner -Arguments @('tl1-phase-a','--scenario-dir','.\src\StarCluster.ScenarioRunner\Scenarios\TL1PhaseA','--baseline-file','.\docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_1.csv','--output-dir',$phaseAOutput) -Description 'Checkpoint 26 TL1 Phase A corpus'
    ...
    Write-Host '[8/9] Running seven TL1 Phase B documents and 36 direct-fire cases...'
    $phaseBOutput = '.\out\checkpoint-26-tl1-phase-b'
    ...
    Write-Host '[9/9] Running forty-six ScenarioRunner self-tests...'
    $selfTestText = Invoke-Runner -Arguments @('self-test','--scenario-file','.\src\StarCluster.ScenarioRunner\Scenarios\terminal-two-window-hit.json') -Description 'Checkpoint 26 runner self-tests'
    ...
    Write-Host ''
    Write-Host 'Checkpoint 26 completed successfully.'
    Write-Host 'Engine-independent tests passed: 589.'
