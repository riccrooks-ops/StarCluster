...
OUT=ROOT/'checkpoint-57-static-preflight.txt'
checks=[]
...
required=[
 ROOT/'Checkpoint_57_Readme.txt',ROOT/'README.md',ROOT/'docs/README.md',ROOT/'docs/Prototype_TODO.md',
 ...
 ROOT/'docs/validation/Checkpoint_57_TL4_Dual_Main_Mid_Tech_Foundation_And_Natural_Power_Screening.md',
 ...
 ROOT/'tools/calibration/checkpoints/checkpoint-57.json',ROOT/'tools/checkpoints/checkpoint-57/apply_checkpoint_57.ps1',ROOT/'tools/checkpoints/checkpoint-57/test_technology_architecture.ps1',ROOT/'tools/checkpoints/checkpoint-57/static_preflight_checkpoint_57.py',ROOT/'tools/checkpoints/checkpoint-57/checkpoint_56_scenario_hashes.txt',ROOT/'tools/checkpoints/checkpoint-57/checkpoint_56_critical_hashes.txt',
...
validate_hash_list(ROOT/'tools/checkpoints/checkpoint-57/checkpoint_56_scenario_hashes.txt',88,'frozen 56 scenario')
validate_hash_list(ROOT/'tools/checkpoints/checkpoint-57/checkpoint_56_critical_hashes.txt',495,'frozen 56 critical')
...
cp=loadj(ROOT/'tools/calibration/checkpoints/checkpoint-57.json'); sids=[s['id'] for s in cp['stages']]; ok('checkpoint id',cp.get('checkpointId')=='57'); ok('50 stages',len(sids)==50 and cp['checkpointMetrics']['stageCount']==50,len(sids)); ok('stage IDs unique',len(sids)==len(set(sids))); ok('self-test final',sids[-1]=='runner-self-tests',sids[-1])
...
for p in [ROOT/'README.md',ROOT/'docs/README.md',ROOT/'docs/Prototype_TODO.md',PT/'README.md',ROOT/'docs/validation/Checkpoint_57_TL4_Dual_Main_Mid_Tech_Foundation_And_Natural_Power_Screening.md',ROOT/'Checkpoint_57_Readme.txt']:
 if p.is_file(): ok(f'front door CP57 {p.relative_to(ROOT)}','Checkpoint 57' in read(p) or 'checkpoint-57' in read(p).lower())

passed=sum(1 for _,v,_ in checks if v); failed=[x for x in checks if not x[1]]
lines=[f'Checkpoint 57 static preflight: {passed}/{len(checks)} checks passed; {len(failed)} failed.']
