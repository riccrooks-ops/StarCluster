...
$checkpoint = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-57.json')
...
Assert-True ([string]$checkpoint.checkpointId -eq '57') 'Checkpoint definition ID mismatch.'
Assert-True (@($checkpoint.stages).Count -eq 50 -and [int]$checkpoint.checkpointMetrics.stageCount -eq 50) 'Checkpoint 57 must contain 50 stages.'
Assert-True ([int]$checkpoint.checkpointMetrics.monteCarloVariantCount -eq 13846 -and [int]$checkpoint.checkpointMetrics.trialsAtDefault -eq 138460000) 'Checkpoint 57 workload metrics mismatch.'
Assert-True ([string]$checkpoint.primaryStudy.id -eq 'tl4-itc01-foundation-transition' -and [int]$checkpoint.primaryStudy.variantCount -eq 180) 'Checkpoint 57 primary study metadata mismatch.'
...
Write-Host '       Checkpoint 57: 50 stages; 13,846 Monte Carlo variants; 138.46 million default trials.'
