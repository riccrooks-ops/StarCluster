...
$checkpoint = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-58d.json')
...
Assert-True ([string]$checkpoint.checkpointId -eq '58d') 'Checkpoint 58d definition ID mismatch.'
Assert-True (@($checkpoint.stages).Count -eq 56 -and [int]$checkpoint.checkpointMetrics.stageCount -eq 56) 'Checkpoint 58d must contain 56 stages.'
Assert-True ([int]$checkpoint.checkpointMetrics.monteCarloVariantCount -eq 14746 -and [int]$checkpoint.checkpointMetrics.trialsAtDefault -eq 147460000) 'Checkpoint 58d workload metrics mismatch.'
Assert-True ([string]$checkpoint.primaryStudy.id -eq 'tl4-itc04-single-main-axis-screening' -and [int]$checkpoint.primaryStudy.variantCount -eq 144) 'Checkpoint 58d primary study metadata mismatch.'
...
# Checkpoint 58d release-manifest contract: retained repository ZIPs are repository-owned files,
# not packaging artifacts, and therefore must be present exactly once in the top-level manifest.
$manifestPath = Join-Path $repositoryRoot 'CHECKPOINT_58D_SHA256SUMS.txt'
Assert-True (Test-Path -LiteralPath $manifestPath -PathType Leaf) 'Checkpoint 58d repository manifest is missing.'
...
foreach ($relative in $retainedZipPaths) {
    ...
    Assert-True ($matches.Count -eq 1) "Retained repository ZIP must appear exactly once in Checkpoint 58d manifest: $relative"
...
Assert-True ($runnerSource.Contains('study.Variants.All(v => v.SideASecondaryFamily is null && v.SideBSecondaryFamily is null)')) 'Checkpoint 58d single-main gate must read secondary-family state from study variants.'
Assert-True (-not $runnerSource.Contains('results.All(r => r.SideASecondaryFamily is null && r.SideBSecondaryFamily is null)')) 'Checkpoint 58d must not read non-existent secondary-family members from variant summaries.'
Assert-True ($runnerSource.Contains('v.ProfileLabel?.StartsWith("r58-defpair-none-", StringComparison.Ordinal) == true')) 'Checkpoint 58d powered-defense none-label check must be nullable-safe.'
Assert-True ($runnerSource.Contains('v.ProfileLabel?.StartsWith("r58-defpair-base-", StringComparison.Ordinal) == true')) 'Checkpoint 58d powered-defense base-label check must be nullable-safe.'

$applySource = Get-Content -LiteralPath (Join-Path $PSScriptRoot 'apply_checkpoint_58d.ps1') -Raw
Assert-True (-not ($applySource -match '(?im)^\s*&\s*(python|python3|py)(\.exe)?\b')) 'Checkpoint 58d launcher must not require Python.'
Assert-True ($applySource.Contains('checkpoint-58d.json')) 'Checkpoint 58d launcher must use the Checkpoint 58d definition.'
...
Write-Host '       Checkpoint 58d C# compile hotfix: single-main validation reads source variants and nullable profile labels are guarded.'
Write-Host '       Checkpoint 58d hotfix: 56 stages; 14,746 Monte Carlo variants; 147.46 million default trials.'
