...
OUT=ROOT/'out/checkpoint-58d-static-preflight.txt'; OUT.parent.mkdir(parents=True,exist_ok=True); checks=[]
...
required=[
 ROOT/'Checkpoint_58d_Readme.txt',ROOT/'README.md',ROOT/'docs/README.md',ROOT/'docs/Prototype_TODO.md',
 ROOT/'CHECKPOINT_58D_SHA256SUMS.txt',
 ...
 ROOT/'docs/validation/Checkpoint_58d_CSharp_Calibration_Runner_Compile_Hotfix.md',
 ...
 ROOT/'tools/calibration/checkpoints/checkpoint-58d.json',ROOT/'tools/checkpoints/checkpoint-58d/apply_checkpoint_58d.ps1',ROOT/'tools/checkpoints/checkpoint-58d/test_technology_architecture.ps1',ROOT/'tools/checkpoints/checkpoint-58d/static_preflight_checkpoint_58d.py',ROOT/'tools/checkpoints/checkpoint-58d/checkpoint_57a_scenario_hashes.txt',ROOT/'tools/checkpoints/checkpoint-58d/checkpoint_57a_critical_hashes.txt',
...
validate_hash_list(ROOT/'tools/checkpoints/checkpoint-58d/checkpoint_57a_scenario_hashes.txt',95,'frozen 57a scenario')
validate_hash_list(ROOT/'tools/checkpoints/checkpoint-58d/checkpoint_57a_critical_hashes.txt',498,'frozen 57a critical')
...
apply=read(ROOT/'tools/checkpoints/checkpoint-58d/apply_checkpoint_58d.ps1'); ok('launcher no Python',re.search(r'(?im)^\s*&\s*(python|python3|py)(\.exe)?\b',apply) is None); ok('launcher checkpoint58d definition','checkpoint-58d.json' in apply)

# Checkpoint 58d inherited release contracts and C# compile-regression contracts.
ps58d=read(ROOT/'tools/checkpoints/checkpoint-58d/test_technology_architecture.ps1')
ok('58d manifest check avoids regex format-string collision', '"^[0-9a-fA-F]{64}  {0}$" -f' not in ps58d)
ok('58d manifest check parses manifest entries', "[regex]::Match($_, '^([0-9a-fA-F]{64})  (.+)$')" in ps58d)
ok('58a validator rejects legacy .power access', all(x not in ps58d for x in ['$output.weapons.kinetic.power -eq 2','$output.weapons.energy.power -eq 3','$output.weapons.missile.power -eq 1']))
ok('58a validator uses kinetic powerCost', "Get-OptionalProperty $output.weapons.kinetic 'powerCost' -1" in ps58d)
ok('58a validator uses energy powerCost', "Get-OptionalProperty $output.weapons.energy 'powerCost' -1" in ps58d)
ok('58a validator uses missile powerCost', "Get-OptionalProperty $output.weapons.missile 'powerCost' -1" in ps58d)
...
runner58d=read(runner_path)
ok('58d single-main gate uses study variants', 'study.Variants.All(v => v.SideASecondaryFamily is null && v.SideBSecondaryFamily is null)' in runner58d)
ok('58d stale summary secondary-family access absent', 'results.All(r => r.SideASecondaryFamily is null && r.SideBSecondaryFamily is null)' not in runner58d)
ok('58d none-label nullable safe', 'v.ProfileLabel?.StartsWith("r58-defpair-none-", StringComparison.Ordinal) == true' in runner58d)
ok('58d base-label nullable safe', 'v.ProfileLabel?.StartsWith("r58-defpair-base-", StringComparison.Ordinal) == true' in runner58d)
ok('58d direct nullable ProfileLabel member dereference absent', re.search(r'\bProfileLabel\.[A-Za-z_]',runner58d) is None)


# Checkpoint 58d release manifest must lock every repository-owned file, including nested ZIPs.
manifest_path=ROOT/'CHECKPOINT_58D_SHA256SUMS.txt'
manifest_entries={}
...
lines=[f'Checkpoint 58d static preflight: {passed}/{len(checks)} checks passed; {len(failed)} failed.']
