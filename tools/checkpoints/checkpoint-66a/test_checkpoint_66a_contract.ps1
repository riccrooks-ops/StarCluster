...
$guardedPowerShellPaths = @(
    ...
    'tools/checkpoints/checkpoint-66a/apply_checkpoint_66a.ps1',
    'tools/checkpoints/checkpoint-66a/test_checkpoint_66a_contract.ps1',
    'tools/calibration/run_calibration_checkpoint.ps1'
)
$guardedDefinitionPaths = @(
    'tools/calibration/checkpoints/checkpoint-66a.json',
    'tools/calibration/checkpoints/checkpoint-66a-deep-calibration.json'
)
...
$active = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-66a.json')
$deep = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-66a-deep-calibration.json')
...
Assert-True ([string]$study.id -eq 'tl1-itc09-scripted-overload-tactics') 'Unexpected Checkpoint 66/66a study ID.'
Assert-True (@($study.variants).Count -eq 80) 'Checkpoint 66/66a study must define exactly 80 variants.'
...
Assert-True ([string]$active.checkpointId -eq '66a' -and [string]$deep.checkpointId -eq '66a') 'Checkpoint definitions must identify 66a.'
Assert-True ([string]$active.manifestFile -eq 'CHECKPOINT_66A_SHA256SUMS.txt' -and [string]$deep.manifestFile -eq 'CHECKPOINT_66A_SHA256SUMS.txt') 'Checkpoint definitions must bind the CP66a manifest.'
Assert-True ([string]$active.outputRoot -eq 'out/checkpoint-66a' -and [string]$deep.outputRoot -eq 'out/checkpoint-66a-deep-calibration') 'Checkpoint 66a output roots mismatch.'
...
foreach ($definition in @($active,$deep)) {
    ...
    Assert-True ([bool]$pre.required -and [string]$pre.script -eq 'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1') 'CP66a definitions must require the shared native dependency precheck.'
    Assert-True (@($pre.powerShellPaths) -contains 'tools/calibration/run_calibration_checkpoint.ps1' -and @($pre.powerShellPaths) -contains 'tools/checkpoints/checkpoint-66a/apply_checkpoint_66a.ps1' -and @($pre.powerShellPaths) -contains 'tools/checkpoints/checkpoint-66a/test_checkpoint_66a_contract.ps1') 'CP66a dependency precheck must inspect apply/contract/shared harness.'
    Assert-True (@($pre.checkpointDefinitionPaths) -contains 'tools/calibration/checkpoints/checkpoint-66a.json' -and @($pre.checkpointDefinitionPaths) -contains 'tools/calibration/checkpoints/checkpoint-66a-deep-calibration.json') 'CP66a dependency precheck must inspect normal and deep definitions.'
}

$applySource = Get-Content -LiteralPath (Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-66a\apply_checkpoint_66a.ps1') -Raw
...
Assert-True ($guardCallIndex -ge 0 -and $guardCallIndex -lt $contractCallIndex -and $contractCallIndex -lt $harnessCallIndex) 'CP66a apply script must run the native dependency precheck before contract and harness.'
Assert-True ($applySource -notmatch '\$LASTEXITCODE') 'CP66a apply script must not read LASTEXITCODE.'
...
$contractSource = Get-Content -LiteralPath (Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-66a\test_checkpoint_66a_contract.ps1') -Raw
Assert-True ($contractSource -notmatch '\$_\.key') 'CP66a contract must not read the nonexistent numerical-baseline .key property.'
Assert-True ($contractSource -match 'parameter_id' -and $contractSource -match 'numericColumns') 'CP66a contract must validate the numerical-baseline parameter_id/value schema before row lookup.'
...
Assert-True ($validationFiles.Count -eq 1 -and $validationFiles[0].Name -eq 'Checkpoint_66a_Numerical_Baseline_CSV_Contract_Hotfix.md') 'Exactly one CP66a active validation runbook must remain.'
$rootTxt = @(Get-ChildItem -LiteralPath $repositoryRoot -File -Filter '*.txt')
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_66A_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_66A_SHA256SUMS.txt as .txt.'
...
Write-Host '       Checkpoint 66a native dependency contract: shared harness + checkpoint guard reject Python runtime dependencies for CP66+.'
...
Write-Host '       Hotfix: numerical-baseline CSV schema is validated as parameter_id/value before StrictMode property access.'
