...
$contractCheck = Join-Path $PSScriptRoot 'test_checkpoint_66a_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-66a-deep-calibration.json' } else { 'checkpoint-66a.json' }
$definition = Join-Path $repositoryRoot (Join-Path 'tools\calibration\checkpoints' $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-66a.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-66a-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!66A(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!66A(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!66a(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_66a_Numerical_Baseline_CSV_Contract_Hotfix.md') {
            Remove-Item -LiteralPath $file.FullName -Force
        }
    }
}
