...
$normalRel = 'tools/calibration/checkpoints/checkpoint-82.json'
$deepRel = 'tools/calibration/checkpoints/checkpoint-82-deep-calibration.json'
$guardedPs = @(
    'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1',
    'tools/checkpoints/checkpoint-82/apply_checkpoint_82.ps1',
    'tools/checkpoints/checkpoint-82/test_checkpoint_82_contract.ps1',
...
Write-Host '       Validating Checkpoint 82 definitions and deterministic workload...'
$normal = Read-Json $normalRel
$deep = Read-Json $deepRel
Assert-True ([string]$normal.checkpointId -eq '82' -and [string]$deep.checkpointId -eq '82') 'Checkpoint 82 definition ID mismatch.'
Assert-True ([string]$normal.manifestFile -eq 'CHECKPOINT_82_SHA256SUMS.txt' -and [string]$deep.manifestFile -eq 'CHECKPOINT_82_SHA256SUMS.txt') 'Checkpoint 82 manifest binding mismatch.'
Assert-True ([string]$normal.outputRoot -eq 'out/checkpoint-82' -and [string]$deep.outputRoot -eq 'out/checkpoint-82-deep-calibration') 'Checkpoint 82 output-root binding mismatch.'
Assert-True (@($normal.stages).Count -eq 8 -and [int]$normal.checkpointMetrics.stageCount -eq 8) 'Checkpoint 82 normal stage-count mismatch.'
Assert-True ([int]$normal.checkpointMetrics.monteCarloVariantCount -eq 0 -and [int]$normal.checkpointMetrics.trialsAtDefault -eq 0) 'Checkpoint 82 normal suite must contain no Monte Carlo workload.'
Assert-True (@($deep.stages).Count -eq 33 -and [int]$deep.checkpointMetrics.monteCarloVariantCount -eq 1694 -and [int]$deep.checkpointMetrics.trialsAtDefault -eq 16940000) 'Checkpoint 82 Deep Calibration workload must preserve the accepted CP81a coverage.'
...
Assert-True (($normalStageIds -join '|') -eq ($expectedNormal -join '|')) 'Checkpoint 82 normal stage ordering mismatch.'
$self = @($normal.stages | Where-Object { [string]$_.id -eq 'runner-self-tests' })
Assert-True ($self.Count -eq 1 -and [int]$self[0].metrics.selfTestCount -eq 48) 'Checkpoint 82 must retain 48 ScenarioRunner self-tests.'
Assert-True ((@($normal.nativeDependencyPrecheck.powerShellPaths) -join '|') -eq ($guardedPs -join '|')) 'Checkpoint 82 normal native-dependency PowerShell binding mismatch.'
Assert-True ((@($deep.nativeDependencyPrecheck.powerShellPaths) -join '|') -eq ($guardedPs -join '|')) 'Checkpoint 82 deep native-dependency PowerShell binding mismatch.'
Assert-True ((@($normal.nativeDependencyPrecheck.checkpointDefinitionPaths) -join '|') -eq ($guardedDefs -join '|')) 'Checkpoint 82 normal native-dependency definition binding mismatch.'
Assert-True ((@($deep.nativeDependencyPrecheck.checkpointDefinitionPaths) -join '|') -eq ($guardedDefs -join '|')) 'Checkpoint 82 deep native-dependency definition binding mismatch.'
...
Assert-True ([int]$profile.referenceEnvironment.tacticalPowerReference -eq 5 -and [string]$profile.referenceEnvironment.reactorSixStatus -eq 'diagnostic_sensitivity_only_not_promoted') 'Checkpoint 82 must not promote reactor output 6.'
...
Assert-True ([int]$matrix.checkpoint -eq 82 -and [string]$matrix.authority.concept -eq 'docs/Star_Cluster_Game_Concept_v0.6t.docx') 'Matrix v1 Checkpoint 82 authority binding mismatch.'
...
Assert-True ($matrixMd.Contains('Historical runtime/study identifiers such as `tl2-production` are retained for deterministic compatibility')) 'Matrix v1 must separate historical tl2-production naming from current authority.'
...
foreach ($line in $sourceLines) {
    ...
    Assert-True ($actual -eq $expected) "Checkpoint 82 unexpectedly changed source/test file '$relative'."
}
...
Assert-True (@($productionApproximateEnable).Count -eq 0) 'Checkpoint 82 must not enable degraded fire on production Core/Game weapon construction.'
...
Assert-True ($validationFiles.Count -eq 1 -and $validationFiles[0].Name -eq 'Checkpoint_82_TL2_Information_Control_Working_Package_Consolidation.md') 'Exactly one Checkpoint 82 active validation runbook must remain.'
...
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_82_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_82_SHA256SUMS.txt as .txt.'
...
Write-Host 'Checkpoint 82 contract validation passed.'
