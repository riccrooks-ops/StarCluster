...
$contractCheck = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-82\test_checkpoint_82_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-82-deep-calibration.json' } else { 'checkpoint-82.json' }
$definition = Join-Path $repositoryRoot ('tools\calibration\checkpoints\' + $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-82.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-82-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!82(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!82(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!82(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_82_TL2_Information_Control_Working_Package_Consolidation.md') {
...
foreach ($concept in @(Get-ChildItem -LiteralPath $docsRoot -File -Filter 'Star_Cluster_Game_Concept_v*.docx')) {
    if ($concept.Name -ne 'Star_Cluster_Game_Concept_v0.6t.docx') {
...
Write-Host '[3/4] Verifying Checkpoint 82 TL2 working-package consolidation contracts...'
& $contractCheck
