...
$contractCheck = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-88\test_checkpoint_88_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-88-deep-calibration.json' } else { 'checkpoint-88.json' }
$definition = Join-Path $repositoryRoot ('tools\calibration\checkpoints\' + $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-88.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-88-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!88(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!88(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!88(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_88_Cross_TL_Power_Aware_Information_Control_And_Construction_Guardrails.md') {
...
foreach ($concept in @(Get-ChildItem -LiteralPath $docsRoot -File -Filter 'Star_Cluster_Game_Concept_v*.docx')) {
    if ($concept.Name -ne 'Star_Cluster_Game_Concept_v0.6y.docx') {
...
Write-Host '[3/4] Verifying Checkpoint 88 cross-TL power-aware information-control and construction contracts...'
& $contractCheck
