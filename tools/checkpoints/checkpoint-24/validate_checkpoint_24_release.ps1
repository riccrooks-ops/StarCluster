...
$stagingRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-24-release-{0}" -f ([guid]::NewGuid().ToString('N')))
...
function Assert-ArchiveContract {
    ...
    try {
        ...
        $manifestEntry = $zip.GetEntry('CHECKPOINT_24_SHA256SUMS.txt')
        ...
        foreach ($line in ($manifestText -split '\r?\n')) {
            ...
            if ($name -eq 'CHECKPOINT_24_SHA256SUMS.txt') { throw 'Archive manifest must not contain itself.' }
            if ($manifest.ContainsKey($name)) { throw "Duplicate archive manifest path: $name" }
            $manifest[$name] = $m.Groups[1].Value.ToLowerInvariant()
        }
        if ($manifest.Count -ne 485) { throw "Checkpoint 24 archive manifest must contain exactly 485 files; found $($manifest.Count)." }
        $actualNames = @($entries | ForEach-Object { $_.FullName.Replace('\','/') } | Where-Object { $_ -ne 'CHECKPOINT_24_SHA256SUMS.txt' })
...
try {
    ...
    $checkpointScript = Join-Path $stagingRoot 'tools\checkpoints\checkpoint-24\apply_checkpoint_24.ps1'
    if (-not (Test-Path -LiteralPath $checkpointScript -PathType Leaf)) { throw "Checkpoint 24 application script was not found in $resolvedArchive." }

    Write-Host '[2/5] Running clean-extraction repository-contract preflight...'
    & $checkpointScript -RepositoryContractOnly

    Write-Host '[3/5] Running clean warning-as-error build, tests, and headless acceptance...'
    & $checkpointScript
    ...
    $archivedRunbook = Join-Path $validationDirectory 'archive\Checkpoint_23a_Hull_Layered_Defense_And_Unified_Tactical_Power_Concept_Backup.md'
    $staleActiveRunbook = Join-Path $validationDirectory 'Checkpoint_23a_Hull_Layered_Defense_And_Unified_Tactical_Power_Concept_Backup.md'
    if (-not (Test-Path -LiteralPath $archivedRunbook -PathType Leaf)) { throw 'The packaged Checkpoint 23a archived validation runbook was not found.' }
    Copy-Item -LiteralPath $archivedRunbook -Destination $staleActiveRunbook -Force

    $archivedConcept = Join-Path $stagingRoot 'docs\archive\Star_Cluster_Game_Concept_v0.3u.docx'
    $staleActiveConcept = Join-Path $stagingRoot 'docs\Star_Cluster_Game_Concept_v0.3u.docx'
    if (-not (Test-Path -LiteralPath $archivedConcept -PathType Leaf)) { throw 'The packaged Concept v0.3u archive copy was not found.' }
    ...
    $localArtifacts = @{
        ...
        'out\checkpoint-24-release-test\result.txt' = 'local output'
        'Checkpoint_22d_Readme.txt' = 'stale checkpoint identity'
        'CHECKPOINT_22D_SHA256SUMS.txt' = 'stale checkpoint identity'
        'StarCluster_Checkpoint_22d_Local_Copy.zip' = 'local package copy'
    }
    ...
    if ($activeRunbooks.Count -ne 1 -or $activeRunbooks[0].Name -ne 'Checkpoint_24_Unified_Tactical_Power_Component_State_And_TL1_Core_Combat_Test_Foundation.md') { throw 'Release validation left an invalid active-runbook set.' }
    ...
    Write-Host ''
    Write-Host 'Checkpoint 24 release validation completed successfully.'
