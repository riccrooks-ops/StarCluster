...
$contractCheck = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-95\test_checkpoint_95_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-95-deep-calibration.json' } else { 'checkpoint-95.json' }
$definition = Join-Path $repositoryRoot ('tools\calibration\checkpoints\' + $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-95.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-95-deep-calibration.json'
...
foreach ($file in @(Get-ChildItem -LiteralPath $repositoryRoot -File -Filter '*.txt')) {
    if ($file.Name -match '^CHECKPOINT_(?!95(?:_|$)).*_SHA256SUMS\.txt$' -or
        $file.Name -match '^Checkpoint_(?!95(?:_|$)).*_Readme\.txt$' -or
        $file.Name -match '^checkpoint-(?!95(?:-|$)).*-static-preflight\.txt$') {
...
foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
    if ($file.Name -ne 'Checkpoint_95_Post_Movement_Readiness_Instrumentation_Hardening.md') {
...
$currentTestingFiles = @(
    'README.md',
    'Checkpoint_95_Validation_Tiers.md',
    'checkpoint_95_validation_suite_policy_v0_1.json',
    'Technology_Integration_Permutation_Suite_Architecture_v0_12.md',
    'technology_integration_permutation_suite_v0_12.json'
)
...
Write-Host '[3/4] Verifying Checkpoint 95 post-Movement readiness instrumentation contracts...'
