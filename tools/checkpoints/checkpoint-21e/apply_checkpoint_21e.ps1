...
function Assert-FileContains {
    ...
    if (-not (Test-Path $Path)) {
        throw "$Description file $Path was not found. Re-extract the complete Checkpoint 21e package."
    }
...
try {
    Write-Host '[1/13] Verifying repository and partially applied Checkpoint 21d baseline...'
    ...
    foreach ($baselineFile in @(
        ...
        if (-not (Test-Path $baselineFile)) {
            throw "Required Checkpoint 21e baseline file $baselineFile was not found."
        }
    }
    ...
    Get-ChildItem '.\docs\validation' -File -ErrorAction SilentlyContinue |
        Where-Object { $_.Name -ne 'Checkpoint_21e_Global_Trial_Block_Scheduler_And_Scaling_Gate.md' } |
        Remove-Item -Force
    ...
    if ($godotProcesses) {
        ...
        throw "Close Godot before applying Checkpoint 21e. Running process(es): $processNames"
    }
    ...
    Assert-FileContains '.\src\StarCluster.Core\Combat\Missiles\MissileInterceptionPhaseContext.cs' @(
        ...
        'MissileInterceptionOpportunity.PreTerminalAttack;') 'Checkpoint 21e interception policy'
    Assert-FileContains '.\src\StarCluster.Core\Simulation\ScenarioInitializationService.cs' @(
        'restoredGuidancePhaseCount = Math.Max(',
        'definition.RetainedDatalink.ReceivedGuidancePhase') 'Checkpoint 21e shared initialization'
    Assert-FileContains '.\src\StarCluster.Core\Combat\Missiles\MissileAutonomousGuidanceService.cs' @(
        ...
        'MissileRoutePlanner.FindRoute(') 'Checkpoint 21e authoritative guidance path'
    Assert-FileContains '.\src\StarCluster.Core\Combat\Missiles\MissileTerminalResolutionService.cs' @(
        'MissileTerminalOutcome.Dud',
        'MissileTerminalOutcome.CriticalHit') 'Checkpoint 21e terminal contract'

    Write-Host '[5/13] Verifying global trial blocks, server GC, telemetry, and the early scaling gate seams...'
    foreach ($runnerFile in @(
        ...
        if (-not (Test-Path $runnerFile)) {
            throw "Required Checkpoint 21e runner file $runnerFile was not found."
        }
    }
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\StarCluster.ScenarioRunner.csproj' @(
        '<ServerGarbageCollection>true</ServerGarbageCollection>',
        '<ConcurrentGarbageCollection>true</ConcurrentGarbageCollection>') 'Checkpoint 21e GC configuration'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\FullFlightCalibrationRunner.cs' @(
        ...
        'ProcessAffinityProcessorCount') 'Checkpoint 21e global trial scheduler'
    Assert-FileNotContains '.\src\StarCluster.ScenarioRunner\FullFlightCalibrationRunner.cs' @(
        ...
        'MonteCarloBatchRunner.Run(') 'Checkpoint 21e rejected coarse scheduler'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\FullFlightCalibrationDocuments.cs' @(
        ...
        'ServerGarbageCollection') 'Checkpoint 21e execution telemetry'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\ScenarioRunnerSelfTests.cs' @(
        ...
        'scenario runner uses server garbage collection') 'Checkpoint 21e scheduler tests'
    ...
    if ($pursuit.id -ne 'checkpoint-21c-full-flight-pursuit-calibration' -or
        ...
        throw 'Checkpoint 21e requires the unchanged Checkpoint 21c pursuit study metadata and cardinality.'
    }
    ...
    if ($policyNames.Count -ne 4 -or
        ...
        throw 'Checkpoint 21e requires the four accepted relative-motion trajectories.'
    }
    ...
    Write-Host '[7/13] Verifying synchronized Checkpoint 21e documentation...'
    foreach ($documentationFile in @(
        '.\docs\README.md',
        '.\docs\Prototype_TODO.md',
        '.\docs\checkpoints\Checkpoint_21e_Global_Trial_Block_Scheduler_And_Scaling_Gate.md',
        '.\docs\validation\Checkpoint_21e_Global_Trial_Block_Scheduler_And_Scaling_Gate.md',
        '.\docs\validation\archive\Checkpoint_21d_Nullable_Dequeue_Build_Hotfix.md',
        ...
        if (-not (Test-Path $documentationFile)) {
            throw "Required Checkpoint 21e documentation file $documentationFile was not found."
        }
    }
    Assert-FileContains '.\docs\validation\Checkpoint_21e_Global_Trial_Block_Scheduler_And_Scaling_Gate.md' @(
        '506/506',
        'thirty-seven runner self-tests',
        '--jobs 1',
        '--jobs 24',
        '32 trials per variant',
        'at least 2.0 times',
        'within 30 minutes',
        'No mechanical Godot validation is required') 'Checkpoint 21e active validation runbook'
    Assert-FileContains '.\docs\checkpoints\Checkpoint_21e_Global_Trial_Block_Scheduler_And_Scaling_Gate.md' @(
        'Global trial-block scheduler',
        'server and concurrent garbage collection',
        'Progress is printed every five percent',
        'The 288,000-trial calibration does not start if the scaling gate fails') 'Checkpoint 21e implementation record'
    $activeValidationFiles = @(Get-ChildItem '.\docs\validation' -File)
    if ($activeValidationFiles.Count -ne 1 -or
        $activeValidationFiles[0].Name -ne 'Checkpoint_21e_Global_Trial_Block_Scheduler_And_Scaling_Gate.md') {
        throw 'Exactly the Checkpoint 21e validation runbook must remain active.'
    }
    ...
    Write-Host '[11/13] Running deterministic corpus and thirty-seven runner self-tests...'
    $deterministicOutputDirectory = '.\out\checkpoint-21e-deterministic'
    ...
    if ($selfTestText -notmatch 'Runner self-tests:\s+37 passed, 0 failed, 37 total\.') {
        throw 'The runner did not report the expected 37/37 self-test result.'
    }

    Write-Host '[12/13] Proving worker independence and requiring material 24-worker speedup...'
    $reproStudy = Join-Path $studyDirectory 'checkpoint-19-reproducibility.sweep.json'
    $reproHashes = @{}
    foreach ($jobs in @(1, 24)) {
        $outputDirectory = ".\out\checkpoint-21e-repro-j$jobs"
    ...
    $schedulerExecutions = @{}
    foreach ($jobs in @(1, 24)) {
        $outputDirectory = ".\out\checkpoint-21e-scheduler-proof-j$jobs"
        Remove-Item -Recurse -Force $outputDirectory -ErrorAction SilentlyContinue
        $schedulerText = Invoke-Runner -Arguments @(
            ...
            '--jobs', $jobs.ToString(),
            '--trials', '32',
            '--output-dir', $outputDirectory) -Description "Global trial-block scheduler proof at jobs=$jobs"
        ...
        if ($execution.schemaVersion -ne 3 -or
            ...
            $execution.peakActiveWorkers -ne $jobs -or
            $execution.innerTrialWorkersPerVariant -ne 0 -or
            ...
            [double]$execution.computeTrialsPerSecond -le 0) {
            throw "The jobs=$jobs scheduler telemetry did not preserve the global block contract."
        }
        $schedulerHashes[$jobs] = (Get-Content (Join-Path $outputDirectory 'full-flight-result.sha256') -Raw).Trim()
        $schedulerExecutions[$jobs] = $execution
    }
    ...
    $singleRate = [double]$schedulerExecutions[1].computeTrialsPerSecond
    $parallelRate = [double]$schedulerExecutions[24].computeTrialsPerSecond
    $speedup = $parallelRate / $singleRate
    $projectedSeconds = 288000.0 / $parallelRate
    Write-Host ("       Scheduler proof: {0:N2} trials/s at jobs=1; {1:N2} trials/s at jobs=24; {2:N2}x speedup." -f $singleRate, $parallelRate, $speedup)
    Write-Host ("       Projected 288,000-trial compute time: {0:N1} minutes." -f ($projectedSeconds / 60.0))
    $parallelProcessorCount = [int]$schedulerExecutions[24].environmentProcessorCount
    $parallelAffinityCount = [int]$schedulerExecutions[24].processAffinityProcessorCount
    $parallelEffectiveCores = [double]$schedulerExecutions[24].effectiveProcessorCores
    $parallelNormalizedCpu = [double]$schedulerExecutions[24].normalizedCpuUtilizationPercent
    Write-Host ("       Parallel runtime visibility: Environment.ProcessorCount={0}; affinity={1}; effective cores={2:N2}; normalized CPU={3:N2}%." -f $parallelProcessorCount, $parallelAffinityCount, $parallelEffectiveCores, $parallelNormalizedCpu)
    if ($speedup -lt 2.0) {
        throw ("The 24-worker scheduler achieved only {0:N2}x compute speedup. The full calibration was not started; preserve both scheduler-proof directories." -f $speedup)
    }
    if ($projectedSeconds -gt 1800.0) {
        throw ("The 24-worker proof projects {0:N1} minutes for the full study, above the 30-minute safety limit. The full calibration was not started." -f ($projectedSeconds / 60.0))
    }

    Write-Host '[13/13] Running the 288-variant full-flight calibration at 24 workers and finalizing...'
    $fullOutputDirectory = '.\out\checkpoint-21e-full-flight-pursuit-calibration'
    Remove-Item -Recurse -Force $fullOutputDirectory -ErrorAction SilentlyContinue
    $fullText = Invoke-Runner -Arguments @(
        'pursuit-calibrate',
        $pursuitPath,
        '--jobs', '24',
        '--output-dir', $fullOutputDirectory) -Description 'Checkpoint 21e full-flight calibration'
    ...
    if ($fullSummary.schemaVersion -ne 3 -or
        ...
        throw 'The full-flight summary failed its Checkpoint 21e acceptance contract.'
    }
    ...
    if ($fullExecution.schemaVersion -ne 3 -or
        ...
        $fullExecution.innerTrialWorkersPerVariant -ne 0 -or
        ...
        throw 'The full-flight execution telemetry did not preserve the 24-worker global block contract.'
    }
    ...
    Write-Host ''
    Write-Host 'Checkpoint 21e completed successfully.'
    Write-Host 'Engine-independent tests passed: 506.'
    Write-Host 'Deterministic headless scenarios passed: 7.'
    Write-Host 'Runner self-tests passed: 37.'
    Write-Host "Ordinary worker-independent reproducibility hash: $($reproHashes[24])."
    Write-Host "Global scheduler-proof hash: $($schedulerHashes[24])."
    Write-Host ("Measured 24-worker compute speedup: {0:N2}x." -f $speedup)
    Write-Host 'Global scheduler proof variants passed: 24 at jobs 1 and jobs 24.'
    Write-Host 'Peak active global trial-block workers at jobs 24: 24.'
    ...
    Write-Host ("Full-flight compute throughput: {0:N2} trials/second over {1} ms." -f [double]$fullExecution.computeTrialsPerSecond, [long]$fullExecution.computeElapsedMilliseconds)
    Write-Host 'Core mechanics, study inputs, and random streams are unchanged.'
    Write-Host 'No mechanical Godot validation is required.'
    Write-Host 'Preserve out\checkpoint-21e-full-flight-pursuit-calibration and both scheduler-proof directories for review.'
}
finally {
    Pop-Location
}
