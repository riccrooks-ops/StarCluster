...
$contractCheck = Join-Path $PSScriptRoot 'test_checkpoint_74a_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-74a-deep-calibration.json' } else { 'checkpoint-74a.json' }
$definition = Join-Path $repositoryRoot (Join-Path 'tools\calibration\checkpoints' $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-74a.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-74a-deep-calibration.json'
...
# Preserve clean full-repository checkpoint packaging when extracting over CP74
# or another older worktree.
$staleRootPatterns = @(
    '^CHECKPOINT_(?!74A(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!74a(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!74a(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_74a_Degraded_Fire_Contract_Hotfix.md') {
