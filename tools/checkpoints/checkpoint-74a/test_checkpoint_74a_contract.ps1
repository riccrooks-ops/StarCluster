...
$normalRel = 'tools/calibration/checkpoints/checkpoint-74a.json'
$deepRel = 'tools/calibration/checkpoints/checkpoint-74a-deep-calibration.json'
$guardedPs = @(
    'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1',
    'tools/checkpoints/checkpoint-74a/apply_checkpoint_74a.ps1',
    'tools/checkpoints/checkpoint-74a/test_checkpoint_74a_contract.ps1',
...
Assert-True ([string]$normal.checkpointId -eq '74a' -and [string]$deep.checkpointId -eq '74a') 'Checkpoint 74a definition ID mismatch.'
Assert-True ([string]$normal.manifestFile -eq 'CHECKPOINT_74A_SHA256SUMS.txt' -and [string]$deep.manifestFile -eq 'CHECKPOINT_74A_SHA256SUMS.txt') 'Checkpoint 74a manifest binding mismatch.'
Assert-True ([string]$normal.outputRoot -eq 'out/checkpoint-74a' -and [string]$deep.outputRoot -eq 'out/checkpoint-74a-deep-calibration') 'Checkpoint 74a output-root binding mismatch.'
Assert-True ((@($normal.nativeDependencyPrecheck.powerShellPaths) -join '|') -eq ($guardedPs -join '|')) 'Checkpoint 74a normal native-dependency PowerShell path binding mismatch.'
Assert-True ((@($deep.nativeDependencyPrecheck.powerShellPaths) -join '|') -eq ($guardedPs -join '|')) 'Checkpoint 74a deep native-dependency PowerShell path binding mismatch.'
Assert-True ((@($normal.nativeDependencyPrecheck.checkpointDefinitionPaths) -join '|') -eq ($guardedDefs -join '|')) 'Checkpoint 74a normal native-dependency definition binding mismatch.'
Assert-True ((@($deep.nativeDependencyPrecheck.checkpointDefinitionPaths) -join '|') -eq ($guardedDefs -join '|')) 'Checkpoint 74a deep native-dependency definition binding mismatch.'
Assert-True (@($normal.stages).Count -eq 11 -and @($deep.stages).Count -eq 30) 'Checkpoint 74a stage-count mismatch.'
Assert-True ([int]$normal.checkpointMetrics.monteCarloVariantCount -eq 20 -and [int]$normal.checkpointMetrics.trialsAtDefault -eq 200000 -and [int]$normal.checkpointMetrics.smokeTrialsAtDefault -eq 20 -and [int]$normal.checkpointMetrics.totalTrialExecutionsAtDefault -eq 200020) 'Checkpoint 74a normal workload mismatch.'
Assert-True ([int]$deep.checkpointMetrics.monteCarloVariantCount -eq 1564 -and [int]$deep.checkpointMetrics.trialsAtDefault -eq 15640000 -and [int]$deep.checkpointMetrics.smokeTrialsAtDefault -eq 20 -and [int]$deep.checkpointMetrics.totalTrialExecutionsAtDefault -eq 15640020) 'Checkpoint 74a Deep Calibration workload mismatch.'

$policy = Read-Json 'docs/design/testing/checkpoint_74_validation_suite_policy_v0_1.json'
Assert-True ([string]$policy.aiDoctrineControls.registryVersion -eq '0.2' -and [string]$policy.aiDoctrineControls.defaultEwDoctrine -eq 'tl1-ew-preserve-combat-package-v1' -and -not [bool]$policy.aiDoctrineControls.cp73RerunRequired) 'Checkpoint 74a AI doctrine-promotion policy mismatch.'
Assert-True ([bool]$policy.productionControls.degradedFireFoundationImplementedByCheckpoint74 -and -not [bool]$policy.productionControls.productionWeaponDegradedFireEnabledByCheckpoint74 -and -not [bool]$policy.productionControls.movementPhaseFireImplementedByCheckpoint74) 'Checkpoint 74a production/deferred-feature policy mismatch.'

$registry = Read-Json 'docs/archive/ai/pre-cp165-active/ai_doctrine_registry_v0_2.json'
Assert-True ([string]$registry.registryVersion -eq '0.2' -and [string]$registry.defaults.'electronic-warfare' -eq 'tl1-ew-preserve-combat-package-v1') 'Checkpoint 74a registry default mismatch.'
...
Assert-True ([string]$schema.'$id' -eq 'star-cluster-tl1-integrated-tactical-combat-schema-v0-18' -and $null -ne $schema.'$defs'.variant.properties.sideAAllowsApproximateDirectFire -and $null -ne $schema.'$defs'.variant.properties.sideAApproximateDirectFireAccuracyPenalty) 'Checkpoint 74a integrated schema degraded-fire controls missing.'

$study = Read-Json 'src/StarCluster.ScenarioRunner/Scenarios/TL1Calibration/tl1-itc16-approximate-track-degraded-fire.json'
Assert-True ([string]$study.id -eq 'tl1-itc16-approximate-track-degraded-fire' -and [int]$study.trialsPerVariant -eq 10000 -and @($study.variants).Count -eq 20 -and [int64]$study.masterSeed -eq 740100) 'Checkpoint 74a study identity/workload mismatch.'
$variants = @($study.variants)
foreach ($group in @($variants | Group-Object comparisonGroup)) {
    Assert-True ($group.Count -eq 5) "CP74a group '$($group.Name)' must contain five variants."
}
Assert-True (@($variants | Where-Object { [string]$_.profileLabel -eq 'firm-reference' }).Count -eq 4 -and @($variants | Where-Object { [string]$_.profileLabel -eq 'approx-firm-only' }).Count -eq 4 -and @($variants | Where-Object { [string]$_.profileLabel -match '^approx-p(10|20|30)$' }).Count -eq 12) 'Checkpoint 74a control/penalty coverage mismatch.'
Assert-True (@($variants | Where-Object { [string]$_.sideAFamily -eq 'Missile' -or [string]$_.sideBFamily -eq 'Missile' }).Count -eq 0) 'Checkpoint 74a degraded-fire study must exclude missiles.'

# CP74a fixes the CP74 contract false negative. The implementation places the
...
Assert-True ($runner.Contains('Tl1ApproximateTrackDegradedFireStudyId') -and $runner.Contains('WriteTl1ApproximateTrackDegradedFireReview') -and $runner.Contains('approximateTrackAccuracyPenalty')) 'Integrated CP74a study wiring missing.'

# Contract-only hotfix: freeze the CP74 mechanics, study, doctrine registry, and
# Concept so this hotfix cannot silently alter substantive game behavior.
$cp74FrozenHashes = @{
    ...
    'src/StarCluster.ScenarioRunner/TL1Calibration/Tl1IntegratedTacticalCombatRunner.cs' = '14b05bdb7ad50be0bcd927aa3b51e23d8d9e3513e2d67228fb1dda3eb24e8b11'
...
foreach ($entry in $cp74FrozenHashes.GetEnumerator()) {
    ...
    Assert-True ($actual -eq [string]$entry.Value) "CP74a contract-only hotfix changed frozen CP74 authority/input: $($entry.Key)."
}

$concepts = @(Get-ChildItem -LiteralPath (Join-Path $repositoryRoot 'docs') -File -Filter 'Star_Cluster_Game_Concept_v*.docx')
Assert-True ($concepts.Count -eq 1 -and $concepts[0].Name -eq 'Star_Cluster_Game_Concept_v0.6m.docx') 'Checkpoint 74a must expose exactly one active Concept v0.6m.'
Assert-True (Test-Path -LiteralPath (Join-Path $repositoryRoot 'docs/archive/Star_Cluster_Game_Concept_v0.6l.docx') -PathType Leaf) 'Checkpoint 74a must retain archived Concept v0.6l.'

$validationFiles = @(Get-ChildItem -LiteralPath (Join-Path $repositoryRoot 'docs\validation') -File -Filter 'Checkpoint_*.md')
Assert-True ($validationFiles.Count -eq 1 -and $validationFiles[0].Name -eq 'Checkpoint_74a_Degraded_Fire_Contract_Hotfix.md') 'Exactly one Checkpoint 74a active validation runbook must remain.'
$rootTxt = @(Get-ChildItem -LiteralPath $repositoryRoot -File -Filter '*.txt')
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_74A_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_74A_SHA256SUMS.txt as .txt.'

Write-Host '       Checkpoint 74a hotfix scope: contract-only; CP74 mechanics, study, doctrine registry, and Concept hashes are frozen.'
Write-Host '       Degraded-fire eligibility contract now validates the actual cross-file AccuracyModifier wiring rather than requiring the result property name in DirectFireTargetEligibility.cs.'
...
Write-Host 'Checkpoint 74a degraded-fire contract hotfix validation passed.'
