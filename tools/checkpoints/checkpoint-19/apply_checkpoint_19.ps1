...
function Assert-FileContains {
    ...
    if (-not (Test-Path $Path)) {
        throw "$Description file $Path was not found. Re-extract the complete Checkpoint 19 package."
    }
...
try {
    Write-Host '[1/13] Verifying repository and accepted Checkpoint 18d baseline...'
    ...
    foreach ($baselineFile in @(
        ...
        if (-not (Test-Path $baselineFile)) {
            throw "Required Checkpoint 18d baseline file $baselineFile was not found."
        }
    }
    ...
    if ($godotProcesses) {
        ...
        throw "Close Godot before applying Checkpoint 19. Running process(es): $processNames"
    }
    ...
    Write-Host '[7/13] Verifying synchronized Checkpoint 19 documentation...'
    foreach ($documentationFile in @(
        ...
        '.\docs\validation\Checkpoint_19_Reproducible_Monte_Carlo_And_Parameter_Sweep_Foundation.md',
        ...
        if (-not (Test-Path $documentationFile)) {
            throw "Required Checkpoint 19 documentation file $documentationFile was not found."
        }
    }
    Assert-FileContains '.\docs\validation\Checkpoint_19_Reproducible_Monte_Carlo_And_Parameter_Sweep_Foundation.md' @(
        '506/506',
        'seven deterministic scenarios',
        'eight runner self-tests',
        '--jobs 1',
        '--jobs 24',
        'No Godot run') 'Checkpoint 19 active validation runbook'
    ...
    if ($activeValidationFiles.Count -ne 1 -or
        $activeValidationFiles[0].Name -ne 'Checkpoint_19_Reproducible_Monte_Carlo_And_Parameter_Sweep_Foundation.md') {
        throw 'Checkpoint 19 must leave exactly one active validation runbook.'
    }

    Write-Host ''
    Write-Host 'Checkpoint 19 completed successfully.'
