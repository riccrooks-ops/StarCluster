...
$contractCheck = Join-Path $PSScriptRoot 'test_checkpoint_66b_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-66b-deep-calibration.json' } else { 'checkpoint-66b.json' }
$definition = Join-Path $repositoryRoot (Join-Path 'tools\calibration\checkpoints' $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-66b.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-66b-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!66B(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!66B(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!66b(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_66b_Integrated_Gate_Scope_Compile_Hotfix.md') {
            Remove-Item -LiteralPath $file.FullName -Force
        }
    }
}
