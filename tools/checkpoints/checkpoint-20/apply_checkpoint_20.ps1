...
try {
    Write-Host '[1/13] Verifying repository and accepted Checkpoint 19a baseline...'
    ...
    foreach ($baselineFile in @(
        ...
        if (-not (Test-Path $baselineFile)) {
            throw "Required Checkpoint 19a baseline file $baselineFile was not found."
        }
    }
    ...
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\TechnologyCalibrationRunner.cs' @(
        ...
        'statistically contradictory marginals') 'Checkpoint 20 calibration reporting'
    ...
    Write-Host '[7/13] Verifying synchronized Checkpoint 20 documentation...'
    foreach ($documentationFile in @(
        ...
        '.\docs\validation\Checkpoint_20_Representative_Missile_Profiles_And_TL_Calibration.md',
    ...
    Assert-FileContains '.\docs\validation\Checkpoint_20_Representative_Missile_Profiles_And_TL_Calibration.md' @(
        '506/506',
        'seven deterministic headless scenarios',
        'twelve runner self-tests',
        '108 variants',
        '--jobs 24',
        'No Godot run') 'Checkpoint 20 active validation runbook'
    ...
    if ($activeValidationFiles.Count -ne 1 -or
        $activeValidationFiles[0].Name -ne 'Checkpoint_20_Representative_Missile_Profiles_And_TL_Calibration.md') {
        throw 'Checkpoint 20 must leave exactly one active validation runbook.'
    }

    Write-Host ''
    Write-Host 'Checkpoint 20 completed successfully.'
