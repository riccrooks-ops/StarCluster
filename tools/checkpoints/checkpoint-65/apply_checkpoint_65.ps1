...
$contractCheck = Join-Path $PSScriptRoot 'test_checkpoint_65_contract.ps1'
$staticPreflight = Join-Path $PSScriptRoot 'static_preflight_checkpoint_65.py'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-65-deep-calibration.json' } else { 'checkpoint-65.json' }
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!65(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!65(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!65(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_65_TL1_Bilateral_Tactical_Geometry_Fuel_And_Movement_Order.md') {
            Remove-Item -LiteralPath $file.FullName -Force
        }
    }
}
...
& $contractCheck
if ($LASTEXITCODE -ne 0) { throw "Checkpoint 65 design contract failed with exit code $LASTEXITCODE." }
python $staticPreflight --repository-root $repositoryRoot
if ($LASTEXITCODE -ne 0) { throw "Checkpoint 65 static preflight failed with exit code $LASTEXITCODE." }
