...
$active = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-65.json')
$deep = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-65-deep-calibration.json')

Assert-True ([string]$active.checkpointId -eq '65' -and [string]$deep.checkpointId -eq '65') 'Checkpoint 65 definitions must identify checkpoint 65.'
Assert-True ([string]$active.manifestFile -eq 'CHECKPOINT_65_SHA256SUMS.txt' -and [string]$deep.manifestFile -eq 'CHECKPOINT_65_SHA256SUMS.txt') 'Checkpoint 65 definitions must bind CHECKPOINT_65_SHA256SUMS.txt.'
Assert-True ([string]$active.outputRoot -eq 'out/checkpoint-65' -and [string]$deep.outputRoot -eq 'out/checkpoint-65-deep-calibration') 'Checkpoint 65 output roots must be checkpoint-specific.'
Assert-True (@($active.documentation) -contains 'docs/validation/Checkpoint_65_TL1_Bilateral_Tactical_Geometry_Fuel_And_Movement_Order.md') 'Checkpoint 65 definition must reference the active runbook.'
...
Assert-True ($validationFiles.Count -eq 1 -and $validationFiles[0].Name -eq 'Checkpoint_65_TL1_Bilateral_Tactical_Geometry_Fuel_And_Movement_Order.md') 'Exactly one Checkpoint 65 active validation runbook must remain.'
$rootTxt = @(Get-ChildItem -LiteralPath $repositoryRoot -File -Filter '*.txt')
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_65_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_65_SHA256SUMS.txt as a .txt file.'
...
Write-Host '       Checkpoint 65 design contract: balanced TL1 cruisers on the radius-5 map, 3x3 weapon families, 3 bilateral regimes, 2 movement orders = 54 variants.'
