...
def main() -> int:
    ...
    paths = {
        ...
        "active": root / "tools/calibration/checkpoints/checkpoint-65a.json",
        "deep": root / "tools/calibration/checkpoints/checkpoint-65a-deep-calibration.json",
        "runbook": root / "docs/validation/Checkpoint_65a_TL1_Bilateral_Tactical_Geometry_Fuel_And_Movement_Order_Hotfix.md",
        ...
        "manifest": root / "CHECKPOINT_65A_SHA256SUMS.txt",
        "apply": root / "tools/checkpoints/checkpoint-65a/apply_checkpoint_65a.ps1",
    }
    ...
    check(active.get("checkpointId") == "65a" and deep.get("checkpointId") == "65a", "checkpoint ID binding mismatch")
    check(active.get("manifestFile") == "CHECKPOINT_65A_SHA256SUMS.txt" and deep.get("manifestFile") == "CHECKPOINT_65A_SHA256SUMS.txt", "manifest binding mismatch")
    check(active.get("outputRoot") == "out/checkpoint-65a", "normal output root mismatch")
    check(deep.get("outputRoot") == "out/checkpoint-65a-deep-calibration", "deep output root mismatch")
    ...
    apply_src = paths["apply"].read_text(encoding="utf-8")
    check("& $contractCheck" in apply_src and "python $staticPreflight" in apply_src,
          "65a apply script must invoke the PowerShell contract before native static preflight")
    try:
        contract_pos = apply_src.index("& $contractCheck")
        native_pos = apply_src.index("python $staticPreflight", contract_pos)
        early_window = apply_src[contract_pos:native_pos]
        check("$LASTEXITCODE" not in early_window,
              "65a apply script must not read $LASTEXITCODE before a native process initializes it")
        check("$LASTEXITCODE" in apply_src[native_pos:],
              "65a apply script must still validate the native static-preflight exit code")
    except ValueError:
        check(False, "65a apply-script invocation order could not be inspected")
    ...
    check(len(root_txt) == 1 and root_txt[0].name == "CHECKPOINT_65A_SHA256SUMS.txt",
          "repository root must contain only current CP65a manifest as .txt")
    ...
    if paths["manifest"].is_file():
        ...
        for path in root.rglob("*"):
            ...
            if rel == "CHECKPOINT_65A_SHA256SUMS.txt" or rel.startswith("out/") or "/bin/" in f"/{rel}" or "/obj/" in f"/{rel}":
                continue
            owned.add(rel)
        check(owned == listed, f"manifest ownership mismatch: missing={len(owned-listed)} unexpected={len(listed-owned)}")

    if FAILURES:
        print(f"Checkpoint 65a static preflight FAILED: {CHECKS - len(FAILURES)}/{CHECKS} checks passed.")
        for failure in FAILURES:
            print(f"FAILED: {failure}")
        return 1
    print(f"Checkpoint 65a static preflight: {CHECKS}/{CHECKS} checks passed.")
    return 0
