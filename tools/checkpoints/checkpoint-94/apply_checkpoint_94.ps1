...
$contractCheck = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-94\test_checkpoint_94_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-94-deep-calibration.json' } else { 'checkpoint-94.json' }
$definition = Join-Path $repositoryRoot ('tools\calibration\checkpoints\' + $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-94.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-94-deep-calibration.json'
...
foreach ($file in @(Get-ChildItem -LiteralPath $repositoryRoot -File -Filter '*.txt')) {
    if ($file.Name -match '^CHECKPOINT_(?!94(?:_|$)).*_SHA256SUMS\.txt$' -or
        $file.Name -match '^Checkpoint_(?!94(?:_|$)).*_Readme\.txt$' -or
        $file.Name -match '^checkpoint-(?!94(?:-|$)).*-static-preflight\.txt$') {
...
foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
    if ($file.Name -ne 'Checkpoint_94_Adaptive_Sampling_And_Mover_Neutral_Cross_TL_Screening.md') {
...
$currentTestingFiles = @(
    'README.md',
    'Checkpoint_94_Validation_Tiers.md',
    'checkpoint_94_validation_suite_policy_v0_1.json',
    'Technology_Integration_Permutation_Suite_Architecture_v0_11.md',
    'technology_integration_permutation_suite_v0_11.json'
)
...
Write-Host '[3/4] Verifying Checkpoint 94 adaptive-sampling/mover-neutral contracts...'
