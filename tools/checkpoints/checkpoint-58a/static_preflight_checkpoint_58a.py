...
OUT=ROOT/'checkpoint-58a-static-preflight.txt'; checks=[]
...
required=[
 ROOT/'Checkpoint_58a_Readme.txt',ROOT/'README.md',ROOT/'docs/README.md',ROOT/'docs/Prototype_TODO.md',
 ...
 ROOT/'docs/validation/Checkpoint_58a_Repository_Validator_Power_Cost_Property_Hotfix.md',
 ...
 ROOT/'tools/calibration/checkpoints/checkpoint-58a.json',ROOT/'tools/checkpoints/checkpoint-58a/apply_checkpoint_58a.ps1',ROOT/'tools/checkpoints/checkpoint-58a/test_technology_architecture.ps1',ROOT/'tools/checkpoints/checkpoint-58a/static_preflight_checkpoint_58a.py',ROOT/'tools/checkpoints/checkpoint-58a/checkpoint_57a_scenario_hashes.txt',ROOT/'tools/checkpoints/checkpoint-58a/checkpoint_57a_critical_hashes.txt',
...
validate_hash_list(ROOT/'tools/checkpoints/checkpoint-58a/checkpoint_57a_scenario_hashes.txt',95,'frozen 57a scenario')
validate_hash_list(ROOT/'tools/checkpoints/checkpoint-58a/checkpoint_57a_critical_hashes.txt',498,'frozen 57a critical')
...
apply=read(ROOT/'tools/checkpoints/checkpoint-58a/apply_checkpoint_58a.ps1'); ok('launcher no Python',re.search(r'(?im)^\s*&\s*(python|python3|py)(\.exe)?\b',apply) is None); ok('launcher checkpoint58a definition','checkpoint-58a.json' in apply)

# Checkpoint 58a hotfix contract: canonical weapon power property is powerCost.
ps58a=read(ROOT/'tools/checkpoints/checkpoint-58a/test_technology_architecture.ps1')
ok('58a validator rejects legacy .power access', all(x not in ps58a for x in ['$output.weapons.kinetic.power -eq 2','$output.weapons.energy.power -eq 3','$output.weapons.missile.power -eq 1']))
ok('58a validator uses kinetic powerCost', "Get-OptionalProperty $output.weapons.kinetic 'powerCost' -1" in ps58a)
ok('58a validator uses energy powerCost', "Get-OptionalProperty $output.weapons.energy 'powerCost' -1" in ps58a)
ok('58a validator uses missile powerCost', "Get-OptionalProperty $output.weapons.missile 'powerCost' -1" in ps58a)
...
lines=[f'Checkpoint 58a static preflight: {passed}/{len(checks)} checks passed; {len(failed)} failed.']
