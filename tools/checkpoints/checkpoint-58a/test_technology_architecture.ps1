...
$checkpoint = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-58a.json')
...
Assert-True ([string]$checkpoint.checkpointId -eq '58a') 'Checkpoint 58a definition ID mismatch.'
Assert-True (@($checkpoint.stages).Count -eq 56 -and [int]$checkpoint.checkpointMetrics.stageCount -eq 56) 'Checkpoint 58a must contain 56 stages.'
Assert-True ([int]$checkpoint.checkpointMetrics.monteCarloVariantCount -eq 14746 -and [int]$checkpoint.checkpointMetrics.trialsAtDefault -eq 147460000) 'Checkpoint 58a workload metrics mismatch.'
Assert-True ([string]$checkpoint.primaryStudy.id -eq 'tl4-itc04-single-main-axis-screening' -and [int]$checkpoint.primaryStudy.variantCount -eq 144) 'Checkpoint 58a primary study metadata mismatch.'
...
$applySource = Get-Content -LiteralPath (Join-Path $PSScriptRoot 'apply_checkpoint_58a.ps1') -Raw
Assert-True (-not ($applySource -match '(?im)^\s*&\s*(python|python3|py)(\.exe)?\b')) 'Checkpoint 58a launcher must not require Python.'
Assert-True ($applySource.Contains('checkpoint-58a.json')) 'Checkpoint 58a launcher must use the Checkpoint 58a definition.'
...
Write-Host '       Checkpoint 58a hotfix: 56 stages; 14,746 Monte Carlo variants; 147.46 million default trials.'
