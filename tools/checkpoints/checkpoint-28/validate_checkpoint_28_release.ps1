...
$stagingRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-28-release-{0}" -f ([guid]::NewGuid().ToString('N')))
...
function Assert-ArchiveContract {
    ...
    try {
        ...
        $manifestEntry = $zip.GetEntry('CHECKPOINT_28_SHA256SUMS.txt')
        ...
        foreach ($line in ($manifestText -split '\r?\n')) {
            ...
            if ($name -eq 'CHECKPOINT_28_SHA256SUMS.txt') { throw 'Archive manifest must not contain itself.' }
            if ($manifest.ContainsKey($name)) { throw "Duplicate archive manifest path: $name" }
            $manifest[$name] = $m.Groups[1].Value.ToLowerInvariant()
        }
        if ($manifest.Count -ne 597) { throw "Checkpoint 28 archive manifest must contain exactly 597 files; found $($manifest.Count)." }
        $normalizationInputs = @(
        ...
        $actualNames = @($entries | ForEach-Object { $_.FullName.Replace('\','/') } | Where-Object {
            $_ -ne 'CHECKPOINT_28_SHA256SUMS.txt' -and $normalizationInputs -notcontains $_
        })
...
try {
    ...
    $checkpointScript = Join-Path $stagingRoot 'tools\checkpoints\checkpoint-28\apply_checkpoint_28.ps1'
    if (-not (Test-Path -LiteralPath $checkpointScript -PathType Leaf)) { throw "Checkpoint 28 application script was not found in $resolvedArchive." }
    ...
    $localArtifacts = @{
        ...
        'out\checkpoint-28-release-test\result.txt' = 'local output'
    ...
    if ($activeRunbooks.Count -ne 1 -or $activeRunbooks[0].Name -ne 'Checkpoint_28_TL1_Energy_Weapon_And_Tactical_Power_Calibration.md') { throw 'Release validation left an invalid active-runbook set.' }
    ...
    Write-Host ''
    Write-Host 'Checkpoint 28 release validation completed successfully.'
