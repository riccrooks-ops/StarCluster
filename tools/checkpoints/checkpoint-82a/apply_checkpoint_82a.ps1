...
$contractCheck = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-82a\test_checkpoint_82a_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-82a-deep-calibration.json' } else { 'checkpoint-82a.json' }
$definition = Join-Path $repositoryRoot ('tools\calibration\checkpoints\' + $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-82a.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-82a-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!82a(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!82a(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!82a(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_82a_Matrix_Historical_TL2_Production_Authority_Clarification_Hotfix.md') {
...
Write-Host '[3/4] Verifying Checkpoint 82a Matrix historical-authority clarification contracts...'
& $contractCheck
