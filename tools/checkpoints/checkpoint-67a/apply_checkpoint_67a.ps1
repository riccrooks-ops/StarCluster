...
$contractCheck = Join-Path $PSScriptRoot 'test_checkpoint_67a_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-67a-deep-calibration.json' } else { 'checkpoint-67a.json' }
$definition = Join-Path $repositoryRoot (Join-Path 'tools\calibration\checkpoints' $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-67a.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-67a-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!67A(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!67A(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!67a(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_67a_EW_Strain_Ref_Compile_Hotfix.md') {
            Remove-Item -LiteralPath $file.FullName -Force
        }
    }
}
