...
$contractCheck = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-87a\test_checkpoint_87a_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-87a-deep-calibration.json' } else { 'checkpoint-87a.json' }
$definition = Join-Path $repositoryRoot ('tools\calibration\checkpoints\' + $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-87a.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-87a-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!87a(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!87a(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!87a(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_87a_CP87_Study_ID_Integration_Contract_Hotfix.md') {
...
Write-Host '[3/4] Verifying Checkpoint 87a CP87 study-ID integration hotfix and cross-TL contracts...'
& $contractCheck
