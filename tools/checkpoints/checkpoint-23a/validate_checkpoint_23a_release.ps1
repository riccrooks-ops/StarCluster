...
$stagingRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-23a-release-{0}" -f ([guid]::NewGuid().ToString('N')))
...
function Assert-ArchiveContract {
    ...
    try {
        ...
        $manifestEntry = $zip.GetEntry('CHECKPOINT_23A_SHA256SUMS.txt')
        ...
        foreach ($line in ($manifestText -split '\r?\n')) {
            ...
            if ($name -eq 'CHECKPOINT_23A_SHA256SUMS.txt') { throw 'Archive manifest must not contain itself.' }
            if ($manifest.ContainsKey($name)) { throw "Duplicate archive manifest path: $name" }
            $manifest[$name] = $m.Groups[1].Value.ToLowerInvariant()
        }
        if ($manifest.Count -ne 473) { throw "Checkpoint 23a archive manifest must contain exactly 473 files; found $($manifest.Count)." }
        $actualNames = @($entries | ForEach-Object { $_.FullName.Replace('\','/') } | Where-Object { $_ -ne 'CHECKPOINT_23A_SHA256SUMS.txt' })
...
try {
    ...
    $checkpointScript = Join-Path $stagingRoot 'tools\checkpoints\checkpoint-23a\apply_checkpoint_23a.ps1'
    if (-not (Test-Path -LiteralPath $checkpointScript -PathType Leaf)) { throw "Checkpoint 23a application script was not found in $resolvedArchive." }
    ...
    $archivedRunbook = Join-Path $validationDirectory 'archive\Checkpoint_23_Player_Technology_Reference_Mining_And_Framework_Foundation.md'
    $staleActiveRunbook = Join-Path $validationDirectory 'Checkpoint_23_Player_Technology_Reference_Mining_And_Framework_Foundation.md'
    if (-not (Test-Path -LiteralPath $archivedRunbook -PathType Leaf)) { throw 'The packaged Checkpoint 23 archived validation runbook was not found.' }
    Copy-Item -LiteralPath $archivedRunbook -Destination $staleActiveRunbook -Force

    $archivedConcept = Join-Path $stagingRoot 'docs\archive\Star_Cluster_Game_Concept_v0.3t.docx'
    $staleActiveConcept = Join-Path $stagingRoot 'docs\Star_Cluster_Game_Concept_v0.3t.docx'
    if (-not (Test-Path -LiteralPath $archivedConcept -PathType Leaf)) { throw 'The packaged Concept v0.3t archive copy was not found.' }
    ...
    $localArtifacts = @{
        ...
        'out\checkpoint-23a-release-test\result.txt' = 'local output'
    ...
    if ($activeRunbooks.Count -ne 1 -or $activeRunbooks[0].Name -ne 'Checkpoint_23a_Hull_Layered_Defense_And_Unified_Tactical_Power_Concept_Backup.md') { throw 'Release validation left an invalid active-runbook set.' }
    ...
    Write-Host ''
    Write-Host 'Checkpoint 23a release validation completed successfully.'
