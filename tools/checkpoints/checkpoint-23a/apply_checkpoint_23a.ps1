...
function Test-NormalizedRelativePathCompatibility {
    $testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-23a-relative-path-test-{0}" -f ([guid]::NewGuid().ToString('N')))
...
function Test-RepositoryManifestLocalArtifactPolicy {
    $allowed = @(
        ...
        'out/checkpoint-23a/example.txt',
...
function Ensure-ActiveConceptDocument {
    ...
    if ($activeConcepts.Count -ne 1 -or $activeConcepts[0].Name -ne $ExpectedFileName) {
        ...
        throw "Exactly the Checkpoint 23a concept document must remain active. Found: $activeNames"
    }
    Write-Host "       Active concept document: $ExpectedFileName"
}

function Test-ActiveConceptDocumentNormalization {
    $testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-23a-concept-test-{0}" -f ([guid]::NewGuid().ToString('N')))
    $docsDirectory = Join-Path $testRoot 'docs'
    $archiveDirectory = Join-Path $docsDirectory 'archive'
    $expectedName = 'Star_Cluster_Game_Concept_v0.3u.docx'
    $duplicateName = 'Star_Cluster_Game_Concept_v0.3t.docx'
    $importName = 'Star_Cluster_Game_Concept_v0.3s.docx'
    try {
        ...
        $imported = @(Get-ChildItem -LiteralPath $archiveDirectory -Filter 'Star_Cluster_Game_Concept_v0.3s_Imported_*.docx' -File)
...
function Assert-RepositoryManifest {
    ...
    foreach ($line in Get-Content -LiteralPath $ManifestPath) {
        ...
        if ($relativePath -eq 'CHECKPOINT_23A_SHA256SUMS.txt') {
    ...
    if ($verified -ne 473) { throw "Repository manifest verified $verified files; the Checkpoint 23a archive requires exactly 473 files." }
...
function Ensure-ActiveValidationRunbook {
    ...
    if ($activeRunbooks.Count -ne 1 -or $activeRunbooks[0].Name -ne $ExpectedFileName) {
        ...
        throw "Exactly the Checkpoint 23a validation runbook must remain active. Found: $activeNames"
    }
    Write-Host "       Active validation runbook: $ExpectedFileName"
}

function Test-ActiveValidationRunbookNormalization {
    $testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-23a-runbook-test-{0}" -f ([guid]::NewGuid().ToString('N')))
    $validationDirectory = Join-Path $testRoot 'validation'
    $archiveDirectory = Join-Path $validationDirectory 'archive'
    $expectedName = 'Checkpoint_23a_Hull_Layered_Defense_And_Unified_Tactical_Power_Concept_Backup.md'
    $staleName = 'Checkpoint_23_Player_Technology_Reference_Mining_And_Framework_Foundation.md'
    try {
        ...
        $imported = @(Get-ChildItem -LiteralPath $archiveDirectory -Filter 'Checkpoint_23_Player_Technology_Reference_Mining_And_Framework_Foundation_Imported_*.md' -File)
...
try {
    Write-Host '[1/7] Verifying complete Checkpoint 23a repository and parser contracts...'
    foreach ($requiredFile in @(
        '.\StarCluster.sln',
        '.\global.json',
        '.\CHECKPOINT_23A_SHA256SUMS.txt',
        ...
        '.\docs\Star_Cluster_Game_Concept_v0.3u.docx',
        '.\docs\archive\Star_Cluster_Game_Concept_v0.3t.docx',
        ...
        '.\docs\checkpoints\Checkpoint_23a_Hull_Layered_Defense_And_Unified_Tactical_Power_Concept_Backup.md',
        '.\docs\validation\Checkpoint_23a_Hull_Layered_Defense_And_Unified_Tactical_Power_Concept_Backup.md',
        ...
        if (-not (Test-Path -LiteralPath $requiredFile -PathType Leaf)) { throw "Required Checkpoint 23a file $requiredFile was not found." }
    }
    Ensure-ActiveValidationRunbook -ValidationDirectory '.\docs\validation' -ExpectedFileName 'Checkpoint_23a_Hull_Layered_Defense_And_Unified_Tactical_Power_Concept_Backup.md' -ArchiveDirectory '.\docs\validation\archive'
    Ensure-ActiveConceptDocument -DocsDirectory '.\docs' -ExpectedFileName 'Star_Cluster_Game_Concept_v0.3u.docx' -ArchiveDirectory '.\docs\archive'
    Test-NormalizedRelativePathCompatibility
    Test-RepositoryManifestLocalArtifactPolicy
    Assert-RepositoryManifest '.\CHECKPOINT_23A_SHA256SUMS.txt'
    ...
    Assert-FileContains '.\README.md' @('Checkpoint 23a','Concept v0.3u','References may inspire original Star Cluster design') 'Repository README'
    ...
    Assert-FileContains '.\docs\checkpoints\Checkpoint_23a_Hull_Layered_Defense_And_Unified_Tactical_Power_Concept_Backup.md' @('SC23A_HULL_MODEL=STRUCTURAL_PLATFORM_WITH_BOUNDED_ENGINEERED_SELF_REPAIR','SC23A_ARMOR_MODEL=LAYERED_AP_AI_SINGLE_CALCULATION_PER_LAYER','SC23A_SHIELD_MODEL=RENEWABLE_CAPACITY_WITH_OPTIONAL_SHIELD_PROTECTION','SC23A_TACTICAL_POWER_MODEL=ONE_POOL_AVAILABLE_COMMITTED_CONSUMED','SC23A_RECHARGE_TIMING=START_OF_TURN_BASELINE_END_OF_TURN_COMPARISON','SC23A_ORGANIC_HULL_POLICY=ALIEN_PRECURSOR_OR_RARE_BIOTECH') 'Checkpoint 23a documentation contract'
    Assert-FileContains '.\docs\design\player_technology\Hull_Armor_Shields_And_Tactical_Power_Foundation.md' @('Effective AP = max(0, Current AP - APEN)','Available','Committed','Consumed','Ablative Armor') 'Checkpoint 23a foundation design record'
    Assert-FileContains '.\docs\design\player_technology\Online_Reference_Review_Queue.md' @('FTL: Faster Than Light','Galactic Civilizations','must not copy proprietary prose') 'Checkpoint 23a online reference review queue'
    ...
    if ($RepositoryContractOnly) {
        Write-Host ''
        Write-Host 'Checkpoint 23a repository-contract preflight completed successfully.'
    ...
    if ($godotProcesses) {
        ...
        throw "Close Godot before applying Checkpoint 23a. Running process(es): $processNames"
    }
    ...
    $deterministicOutput = '.\out\checkpoint-23a-deterministic'
    Remove-Item $deterministicOutput -Recurse -Force -ErrorAction SilentlyContinue
    $scenarioText = Invoke-Runner -Arguments @('run-all','--scenario-dir','.\src\StarCluster.ScenarioRunner\Scenarios','--output-dir',$deterministicOutput) -Description 'Checkpoint 23a deterministic corpus'
    ...
    $selfTestText = Invoke-Runner -Arguments @('self-test','--scenario-file','.\src\StarCluster.ScenarioRunner\Scenarios\terminal-two-window-hit.json') -Description 'Checkpoint 23a runner self-tests'
    ...
    Write-Host ''
    Write-Host 'Checkpoint 23a completed successfully.'
    Write-Host 'No runtime mechanics or numerical TL balance were changed.'
