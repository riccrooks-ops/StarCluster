...
$contractCheck = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-86a\test_checkpoint_86a_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-86a-deep-calibration.json' } else { 'checkpoint-86a.json' }
$definition = Join-Path $repositoryRoot ('tools\calibration\checkpoints\' + $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-86a.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-86a-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!86a(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!86a(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!86a(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_86a_CP86_Standing_Suite_Promotion_Contract_Hotfix.md') {
...
Write-Host '[3/4] Verifying Checkpoint 86a CP86 standing-suite promotion-contract hotfix and weapon-penetration contracts...'
& $contractCheck
