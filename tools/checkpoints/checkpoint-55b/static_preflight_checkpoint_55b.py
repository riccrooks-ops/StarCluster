...
OUT=ROOT/'checkpoint-55b-static-preflight.txt'
checks=[]
...
required=[
 ROOT/'README.md',ROOT/'Checkpoint_55b_Readme.txt',ROOT/'docs/README.md',ROOT/'docs/Prototype_TODO.md',
 ...
 ROOT/'tools/calibration/checkpoints/checkpoint-55b.json',ROOT/'tools/checkpoints/checkpoint-55b/apply_checkpoint_55b.ps1',ROOT/'tools/checkpoints/checkpoint-55b/test_technology_architecture.ps1',ROOT/'tools/checkpoints/checkpoint-55b/static_preflight_checkpoint_55b.py',ROOT/'tools/checkpoints/checkpoint-55b/checkpoint_54a_scenario_hashes.txt',
...
for f in ['CHECKPOINT_55A_SHA256SUMS.txt','Checkpoint_55a_Readme.txt','checkpoint-55a-static-preflight.txt']:
    ok(f'Checkpoint 55a release archived {f}',(ROOT/'docs/archive/checkpoint-55a-release'/f).is_file())
    ok(f'no stale active {f}',not (ROOT/f).exists())
...
# Checkpoint 55b hotfix: every new integrated-combat study must carry the complete canonical envelope.
cs_runner=read(ROOT/'src/StarCluster.ScenarioRunner/TL1Calibration/Tl1IntegratedTacticalCombatRunner.cs')
m=re.search(r'private const string SchemaVersion\s*=\s*\n?\s*"([^"]+)";',cs_runner)
canonical_integrated_schema=m.group(1) if m else ''
baseline_path=PT/'tl1_core_combat_numerical_baseline_v0_1.csv'
canonical_baseline_sha=sha(baseline_path)
expected_envelopes={
    'tl3-itc03-low-tech-capstone-profile-screening.json':('tl3-itc03-low-tech-capstone-profile-screening',550100,102),
    'tl3-aux02-low-tech-capstone-two-capacity-screening.json':('tl3-aux02-low-tech-capstone-two-capacity-screening',550200,585),
    'tl3-aux03-component-isolation.json':('tl3-aux03-component-isolation',550300,78),
    'tl3-pwr02-single-main-power-envelope.json':('tl3-pwr02-single-main-power-envelope',550400,54),
}
...
for rel,(sid,seed,count) in expected_envelopes.items():
    study=loadj(AT/rel)
    ok(f'CP55b exact top-level envelope {rel}',set(study)==expected_keys,sorted(study))
    ok(f'CP55b canonical integrated schema {rel}',study.get('schemaVersion')==canonical_integrated_schema,study.get('schemaVersion'))
    ok(f'CP55b study id {rel}',study.get('id')==sid,study.get('id'))
    ok(f'CP55b study checkpoint {rel}',study.get('checkpoint')==55,study.get('checkpoint'))
    ok(f'CP55b baseline hash {rel}',study.get('baselineSha256','').lower()==canonical_baseline_sha,(study.get('baselineSha256'),canonical_baseline_sha))
    ok(f'CP55b explicit master seed {rel}',study.get('masterSeed')==seed,study.get('masterSeed'))
    ok(f'CP55b default trials {rel}',study.get('trialsPerVariant')==10000,study.get('trialsPerVariant'))
    ok(f'CP55b technology catalog {rel}',study.get('technologyProfileCatalog')=='src/StarCluster.ScenarioRunner/Scenarios/ArchitectureTechnology/tl1-tl3-standard-runtime-profiles-v0_2.json',study.get('technologyProfileCatalog'))
    ok(f'CP55b auxiliary catalog {rel}',study.get('auxiliaryProfileCatalog')=='src/StarCluster.ScenarioRunner/Scenarios/ArchitectureTechnology/tl3-auxiliary-capstone-profiles-v0_2.json',study.get('auxiliaryProfileCatalog'))
    ok(f'CP55b variant count {rel}',len(study.get('variants',[]))==count,len(study.get('variants',[])))
ok('CP55b master seeds unique',len({seed for _,seed,_ in expected_envelopes.values()})==len(expected_envelopes))

# Frozen accepted 54a scenario JSON snapshot.
hashp=ROOT/'tools/checkpoints/checkpoint-55b/checkpoint_54a_scenario_hashes.txt'; lines=[x for x in read(hashp).splitlines() if x.strip()]
...
cp=loadj(ROOT/'tools/calibration/checkpoints/checkpoint-55b.json'); sids=[s['id'] for s in cp['stages']]
ok('checkpoint ID',cp['checkpointId']=='55b'); ok('checkpoint manifest',cp['manifestFile']=='CHECKPOINT_55B_SHA256SUMS.txt'); ok('checkpoint output',cp['outputRoot']=='out/checkpoint-55b')
...
lines=[f'Checkpoint 55b static preflight: {passed}/{len(checks)} checks passed.','']
