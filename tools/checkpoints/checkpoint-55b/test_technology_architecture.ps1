...
$canonicalIntegratedSchema = 'star-cluster-tl1-integrated-tactical-combat-v2'
foreach ($study in @($profileStudy, $auxStudy, $isolationStudy, $powerStudy)) {
    Assert-True ([string]$study.schemaVersion -eq $canonicalIntegratedSchema) "Checkpoint 55b integrated study schema mismatch: $($study.id)"
}
...
$expectedStudyEnvelope = @(
    @{ Study = $profileStudy; Id = 'tl3-itc03-low-tech-capstone-profile-screening'; Seed = 550100; Count = 102 },
    @{ Study = $auxStudy; Id = 'tl3-aux02-low-tech-capstone-two-capacity-screening'; Seed = 550200; Count = 585 },
    @{ Study = $isolationStudy; Id = 'tl3-aux03-component-isolation'; Seed = 550300; Count = 78 },
    @{ Study = $powerStudy; Id = 'tl3-pwr02-single-main-power-envelope'; Seed = 550400; Count = 54 }
)
foreach ($entry in $expectedStudyEnvelope) {
    $study = $entry.Study
    Assert-True ([string]$study.id -eq [string]$entry.Id) "Checkpoint 55b integrated study ID mismatch: $($study.id)"
    Assert-True ([string]$study.baselineSha256 -eq $canonicalBaselineHash) "Checkpoint 55b integrated study baseline hash mismatch: $($study.id)"
    Assert-True ([uint64]$study.masterSeed -eq [uint64]$entry.Seed) "Checkpoint 55b integrated study master seed mismatch: $($study.id)"
    Assert-True ([int]$study.trialsPerVariant -eq 10000) "Checkpoint 55b integrated study default trial count mismatch: $($study.id)"
    Assert-True (@($study.variants).Count -eq [int]$entry.Count) "Checkpoint 55b integrated study variant count mismatch: $($study.id)"
    Assert-True ([string]$study.technologyProfileCatalog -eq 'src/StarCluster.ScenarioRunner/Scenarios/ArchitectureTechnology/tl1-tl3-standard-runtime-profiles-v0_2.json') "Checkpoint 55b integrated study technology catalog mismatch: $($study.id)"
    Assert-True ([string]$study.auxiliaryProfileCatalog -eq 'src/StarCluster.ScenarioRunner/Scenarios/ArchitectureTechnology/tl3-auxiliary-capstone-profiles-v0_2.json') "Checkpoint 55b integrated study auxiliary catalog mismatch: $($study.id)"
}
...
$checkpoint = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-55b.json')
...
Assert-True (@($aux.profiles).Count -eq 27) 'Checkpoint 55 TL3 AUX catalog must contain 27 profiles.'
...
Assert-True ([string]$checkpoint.checkpointId -eq '55b') 'Checkpoint 55b definition ID mismatch.'
...
Write-Host '       Checkpoint 55b hotfix definition: unchanged 39 stages, 10,696 Monte Carlo variants, 106.96 million default trials.'
