...
SKIP='docs/validation/evidence/checkpoint-134/CP134_REPOSITORY_SHA256SUMS.txt'
...
def validate_native(native,final=False):
    name='CP134_NATIVE_ACCEPTANCE_SUMMARY.json' if final else 'CP134_REPOSITORY_ONLY_ACCEPTANCE.json'; s=js(native/name)
    req(s['checkpoint']==134 and s['failedGates']==[],'native identity')
    req(s['python'].startswith('Python 3.13') and s['dotnetSdk']=='8.0.423' and s['buildPassed'] is True,'runtimes/build')
    req(s['pythonTestsPassed']==204 and s['xunitPassed']==913 and s['xunitFailed']==0 and s['xunitSkipped']==0,'unit tests')
    req(s['scenarioRunnerSelfTestsPassed']==70 and s['researchParityPassed']==25 and s['cp134KernelTestsPassed']==8,'standing deterministic gates')
    req(s['deterministicScenarioCorpusPassed'] is True and s['tl1PhaseACorpusPassed'] is True,'scenario corpora')
    req(s['canonicalKernelVersion']=='0.2' and s['canonicalDamageModel']=='penetration-hardening-v1','kernel identity')
    req(s['logicalContexts']==196 and s['generatedVariants']==392 and s['tl6Variants']==136,'plan shape')
    req(s['symmetryComparisons']==50 and s['symmetryMismatches']==0 and s['smokeTrials']==392 and s['smokeTrialErrors']==0,'symmetry/smoke')
...
def main():
    ...
    try:
        d=js(repo/'tools/checkpoints/checkpoint-134/checkpoint_134_definition.json');req(d['checkpoint']==134 and d['declaredSubstantiveTrials']==1960000 and d['expectedVariants']==392,'definition')
        json_count=0
        ...
        if a.native_results:
            n=Path(a.native_results).resolve();final=(n/'CP134_NATIVE_ACCEPTANCE_SUMMARY.json').is_file();validate_native(n,final)
        print(f'       CP134 contract verified: {count} repository-owned files; {json_count} JSON files; kernel 0.2; 196 contexts / 392 variants / 136 TL6 variants; candidate balance remains review-only.')
        return 0
    except Exception as e:
        print(f'CP134 contract failure: {e}',file=sys.stderr);return 1
