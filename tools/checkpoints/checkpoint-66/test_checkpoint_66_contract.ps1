...
$guardedPowerShellPaths = @(
    ...
    'tools/checkpoints/checkpoint-66/apply_checkpoint_66.ps1',
    'tools/checkpoints/checkpoint-66/test_checkpoint_66_contract.ps1',
    'tools/calibration/run_calibration_checkpoint.ps1'
)
$guardedDefinitionPaths = @(
    'tools/calibration/checkpoints/checkpoint-66.json',
    'tools/calibration/checkpoints/checkpoint-66-deep-calibration.json'
)
...
$active = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-66.json')
$deep = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-66-deep-calibration.json')
...
function Numeric-Value([string]$Key) {
    $row = @($numericRows | Where-Object { [string]$_.key -eq $Key })
    Assert-True ($row.Count -eq 1) "Expected exactly one numerical-baseline row for $Key."
...
Assert-True ([string]$study.id -eq 'tl1-itc09-scripted-overload-tactics') 'Unexpected Checkpoint 66 study ID.'
Assert-True (@($study.variants).Count -eq 80) 'Checkpoint 66 study must define exactly 80 variants.'
...
Assert-True ([int]$active.checkpointId -eq 66 -and [int]$deep.checkpointId -eq 66) 'Checkpoint definitions must identify 66.'
Assert-True ([string]$active.manifestFile -eq 'CHECKPOINT_66_SHA256SUMS.txt' -and [string]$deep.manifestFile -eq 'CHECKPOINT_66_SHA256SUMS.txt') 'Checkpoint definitions must bind the CP66 manifest.'
Assert-True ([string]$active.outputRoot -eq 'out/checkpoint-66' -and [string]$deep.outputRoot -eq 'out/checkpoint-66-deep-calibration') 'Checkpoint 66 output roots mismatch.'
...
foreach ($definition in @($active,$deep)) {
    ...
    Assert-True ([bool]$pre.required -and [string]$pre.script -eq 'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1') 'CP66 definitions must require the shared native dependency precheck.'
    Assert-True (@($pre.powerShellPaths) -contains 'tools/calibration/run_calibration_checkpoint.ps1' -and @($pre.powerShellPaths) -contains 'tools/checkpoints/checkpoint-66/apply_checkpoint_66.ps1' -and @($pre.powerShellPaths) -contains 'tools/checkpoints/checkpoint-66/test_checkpoint_66_contract.ps1') 'CP66 dependency precheck must inspect apply/contract/shared harness.'
    Assert-True (@($pre.checkpointDefinitionPaths) -contains 'tools/calibration/checkpoints/checkpoint-66.json' -and @($pre.checkpointDefinitionPaths) -contains 'tools/calibration/checkpoints/checkpoint-66-deep-calibration.json') 'CP66 dependency precheck must inspect normal and deep definitions.'
}

$applySource = Get-Content -LiteralPath (Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-66\apply_checkpoint_66.ps1') -Raw
...
Assert-True ($guardCallIndex -ge 0 -and $guardCallIndex -lt $contractCallIndex -and $contractCallIndex -lt $harnessCallIndex) 'CP66 apply script must run the native dependency precheck before contract and harness.'
Assert-True ($applySource -notmatch '\$LASTEXITCODE') 'CP66 apply script must not read LASTEXITCODE.'
...
Assert-True ($validationFiles.Count -eq 1 -and $validationFiles[0].Name -eq 'Checkpoint_66_TL1_Scripted_Overload_Tactics.md') 'Exactly one CP66 active validation runbook must remain.'
$rootTxt = @(Get-ChildItem -LiteralPath $repositoryRoot -File -Filter '*.txt')
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_66_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_66_SHA256SUMS.txt as .txt.'
...
Write-Host '       Checkpoint 66 native dependency contract: shared harness + checkpoint guard reject Python runtime dependencies for CP66+.'
