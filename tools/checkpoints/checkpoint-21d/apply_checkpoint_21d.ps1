...
function Assert-FileContains {
    ...
    if (-not (Test-Path $Path)) {
        throw "$Description file $Path was not found. Re-extract the complete Checkpoint 21d package."
    }
...
try {
    Write-Host '[1/13] Verifying repository and partially applied Checkpoint 21c baseline...'
    ...
    foreach ($baselineFile in @(
        ...
        if (-not (Test-Path $baselineFile)) {
            throw "Required Checkpoint 21d baseline file $baselineFile was not found."
        }
    }
    ...
    Get-ChildItem '.\docs\validation' -File -ErrorAction SilentlyContinue |
        Where-Object { $_.Name -ne 'Checkpoint_21d_Nullable_Dequeue_Build_Hotfix.md' } |
        Remove-Item -Force
    ...
    if ($godotProcesses) {
        ...
        throw "Close Godot before applying Checkpoint 21d. Running process(es): $processNames"
    }
    ...
    Assert-FileContains '.\src\StarCluster.Core\Combat\Missiles\MissileInterceptionPhaseContext.cs' @(
        ...
        'MissileInterceptionOpportunity.PreTerminalAttack;') 'Checkpoint 21d interception policy'
    Assert-FileContains '.\src\StarCluster.Core\Simulation\ScenarioInitializationService.cs' @(
        'restoredGuidancePhaseCount = Math.Max(',
        'definition.RetainedDatalink.ReceivedGuidancePhase') 'Checkpoint 21d shared initialization'
    Assert-FileContains '.\src\StarCluster.Core\Combat\Missiles\MissileAutonomousGuidanceService.cs' @(
        ...
        'MissileRoutePlanner.FindRoute(') 'Checkpoint 21d authoritative guidance path'
    Assert-FileContains '.\src\StarCluster.Core\Combat\Missiles\MissileTerminalResolutionService.cs' @(
        'MissileTerminalOutcome.Dud',
        'MissileTerminalOutcome.CriticalHit') 'Checkpoint 21d terminal contract'
    ...
    foreach ($runnerFile in @(
        ...
        if (-not (Test-Path $runnerFile)) {
            throw "Required Checkpoint 21d runner file $runnerFile was not found."
        }
    }
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\ScenarioExecutor.cs' @(
        ...
        'DiagnosticEventType.MissileTerminalAcquisitionResolved') 'Checkpoint 21d Search/Wait diagnostic separation'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\MonteCarloTrialResult.cs' @(
        ...
        'result.TerminalOpportunities.Count') 'Checkpoint 21d trial semantics'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\MonteCarloBatchRunner.cs' @(
        'WriteErrorJournal(',
        '"errors.jsonl"',
        'if (options.Jobs == 1)',
        'if (options.KeepTrialJournal || options.Resume)') 'Checkpoint 21d compact error evidence'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\FullFlightCalibrationModel.cs' @(
        ...
        'The scheduler proof corpus did not produce its expected unique variants.') 'Checkpoint 21d scheduler-proof corpus'
    Assert-FileNotContains '.\src\StarCluster.ScenarioRunner\FullFlightCalibrationModel.cs' @(
        'CrossingWeavePolicy => (turn - 1) % 4 switch') 'Checkpoint 21d crossing-weave parser guard'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\FullFlightCalibrationRunner.cs' @(
        ...
        'Full-flight failure categories:') 'Checkpoint 21d dedicated scheduler and reporting'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\FullFlightCalibrationRunner.cs' @(
        'out PreparedFullFlightCalibrationVariant? item',
        'if (item is null)',
        'The full-flight variant queue returned a null work item.') 'Checkpoint 21d nullable dequeue repair'
    Assert-FileNotContains '.\src\StarCluster.ScenarioRunner\FullFlightCalibrationRunner.cs' @(
        'out PreparedFullFlightCalibrationVariant item))') 'Checkpoint 21d nullable dequeue regression guard'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\PairedMarginalStatistics.cs' @(
        'expectedDirection != "descriptive"',
        'if (expectedDirection == "descriptive")',
        '"descriptive" => 1.0') 'Checkpoint 21d descriptive marginal support'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\RunnerHashUtility.cs' @(
        ...
        'CoreAssemblyHash.Value') 'Checkpoint 21d thread-safe assembly-hash caching'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\Program.cs' @(
        'HasFlag(args, "--scheduler-proof")',
        '[--scheduler-proof]') 'Checkpoint 21d scheduler-proof CLI'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\ScenarioRunnerSelfTests.cs' @(
        ...
        'semantic occlusion permits resolution before a guidance update') 'Checkpoint 21d self-test coverage'
    ...
    Assert-FileContains (Join-Path $scenarioDirectory 'blocked-retained-report-search.json') @(
        ...
        '"MissileGuidanceResolved"') 'Checkpoint 21d co-located blocked retained-report scenario'
    ...
    if ($pursuit.id -ne 'checkpoint-21c-full-flight-pursuit-calibration' -or
        ...
        throw 'Checkpoint 21d requires the unchanged Checkpoint 21c pursuit study metadata and cardinality.'
    }
    ...
    if ($policyNames.Count -ne 4 -or
        ...
        throw 'Checkpoint 21d requires the four accepted relative-motion trajectories.'
    }
    ...
    Write-Host '[7/13] Verifying synchronized Checkpoint 21d documentation...'
    foreach ($documentationFile in @(
        '.\docs\README.md',
        '.\docs\Prototype_TODO.md',
        '.\docs\checkpoints\Checkpoint_21d_Nullable_Dequeue_Build_Hotfix.md',
        '.\docs\validation\Checkpoint_21d_Nullable_Dequeue_Build_Hotfix.md',
        '.\docs\validation\archive\Checkpoint_21c_Full_Flight_Diagnostics_Semantic_Contracts_And_Dedicated_24_Worker_Repair.md',
        ...
        if (-not (Test-Path $documentationFile)) {
            throw "Required Checkpoint 21d documentation file $documentationFile was not found."
        }
    }
    Assert-FileContains '.\docs\validation\Checkpoint_21d_Nullable_Dequeue_Build_Hotfix.md' @(
        '506/506',
        ...
        'No mechanical Godot validation is required') 'Checkpoint 21d active validation runbook'
    Assert-FileContains '.\docs\checkpoints\Checkpoint_21d_Nullable_Dequeue_Build_Hotfix.md' @(
        'CandidateCoordinateReached',
        ...
        'No mechanical Godot validation is required') 'Checkpoint 21d implementation record'
    $activeValidationFiles = @(Get-ChildItem '.\docs\validation' -File)
    if ($activeValidationFiles.Count -ne 1 -or
        $activeValidationFiles[0].Name -ne 'Checkpoint_21d_Nullable_Dequeue_Build_Hotfix.md') {
        throw 'Exactly the Checkpoint 21d validation runbook must remain active.'
    }
    ...
    $deterministicOutputDirectory = '.\out\checkpoint-21d-deterministic'
    ...
    foreach ($jobs in @(1, 24)) {
        $outputDirectory = ".\out\checkpoint-21d-repro-j$jobs"
    ...
    $schedulerHashes = @{}
    foreach ($jobs in @(1, 24)) {
        $outputDirectory = ".\out\checkpoint-21d-scheduler-proof-j$jobs"
        Remove-Item -Recurse -Force $outputDirectory -ErrorAction SilentlyContinue
    ...
    $fullOutputDirectory = '.\out\checkpoint-21d-full-flight-pursuit-calibration'
    Remove-Item -Recurse -Force $fullOutputDirectory -ErrorAction SilentlyContinue
    $fullText = Invoke-Runner -Arguments @(
        'pursuit-calibrate',
        $pursuitPath,
        '--jobs', '24',
        '--output-dir', $fullOutputDirectory) -Description 'Checkpoint 21d full-flight calibration'
    ...
    if ($fullSummary.schemaVersion -ne 3 -or
        ...
        throw 'The full-flight summary failed its Checkpoint 21d acceptance contract.'
    }
    ...
    Write-Host ''
    Write-Host 'Checkpoint 21d completed successfully.'
    ...
    Write-Host 'Checkpoint 21c mechanics and random streams are unchanged; only nullable dequeue typing was repaired.'
    Write-Host 'No mechanical Godot validation is required.'
    Write-Host 'Preserve out\checkpoint-21d-full-flight-pursuit-calibration for review.'
}
finally {
    Pop-Location
}
