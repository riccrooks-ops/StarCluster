...
function Assert-FileContains {
    ...
    if (-not (Test-Path $Path)) {
        throw "$Description file $Path was not found. Re-extract the complete Checkpoint 20 package."
    }
...
try {
    Write-Host '[1/13] Verifying repository and Checkpoint 20 overlay baseline...'
    ...
    foreach ($baselineFile in @(
        ...
        if (-not (Test-Path $baselineFile)) {
            throw "Required Checkpoint 20 overlay baseline file $baselineFile was not found."
        }
    }
    ...
    if ($godotProcesses) {
        ...
        throw "Close Godot before applying Checkpoint 20. Running process(es): $processNames"
    }
    ...
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\Program.cs' @(
        ...
        '--keep-trials',
        '--discard-trials') 'Checkpoint 20 runner modes'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\TechnologyCalibrationModel.cs' @(
        ...
        'ExpectedEffectiveHit',
        'RequireUniqueAxis') 'Checkpoint 20 analytical model'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\TechnologyCalibrationRunner.cs' @(
        ...
        'contradictory marginals') 'Checkpoint 20a calibration reporting guard'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\MonteCarloBatchRunner.cs' @(
        'execution-history.jsonl',
        'KeepTrialJournal',
        'File.Delete(trialsPath)') 'Checkpoint 20 compact provenance'
    ...
    if (@($calibration.missileProfiles).Count -ne 4 -or
        ...
        [Math]::Abs([double]$calibration.maximumAbsoluteError - 0.04) -gt 0.0000001) {
        throw 'Checkpoint 20 calibration must contain four profiles, 2,000 trials per variant, and 0.04 maximum absolute error.'
    }
    $variantCount = (
    ...
    Write-Host '[7/13] Verifying synchronized Checkpoint 20a documentation...'
    foreach ($documentationFile in @(
        '.\docs\README.md',
        '.\docs\Prototype_TODO.md',
        '.\docs\checkpoints\Checkpoint_20_Representative_Missile_Profiles_And_TL_Calibration.md',
        '.\docs\checkpoints\Checkpoint_20a_Calibration_Reporting_Guard_Hotfix.md',
        '.\docs\validation\Checkpoint_20a_Calibration_Reporting_Guard_Hotfix.md',
    ...
    Assert-FileContains '.\docs\validation\Checkpoint_20a_Calibration_Reporting_Guard_Hotfix.md' @(
        '506/506',
        'seven deterministic headless scenarios',
        'twelve runner self-tests',
        '108 variants',
        '--jobs 24',
        'No Godot run') 'Checkpoint 20a active validation runbook'
    ...
    Assert-FileContains '.\docs\checkpoints\Checkpoint_20a_Calibration_Reporting_Guard_Hotfix.md' @(
        'source-text guard',
        'contradictory marginals',
        'mechanics are unchanged') 'Checkpoint 20a hotfix record'
    ...
    Write-Host '[11/13] Running deterministic corpus and twelve runner self-tests...'
    $deterministicOutputDirectory = '.\out\checkpoint-20-deterministic'
    ...
    if ($selfTestText -notmatch 'Runner self-tests:\s+12 passed, 0 failed, 12 total\.') {
        throw 'The runner did not report the expected 12/12 self-test result.'
    }
    ...
    $reproDirectories = @{
        1 = '.\out\checkpoint-20-repro-j1'
        24 = '.\out\checkpoint-20-repro-j24'
    }
    $reproHashes = @{}
    ...
    $calibrationOutputDirectory = '.\out\checkpoint-20-terminal-tl-calibration'
    ...
    if ($calibrationText -notmatch 'Calibration preflight:\s+108 variants across 4 missile profiles passed\.' -or
        $calibrationText -notmatch 'Calibration:\s+108 variants passed, 0 failed; 0 statistically contradictory marginals\.') {
    ...
    if (-not $summary.passed -or
        ...
        @($summary.marginals | Where-Object { $_.statisticallyContradictory }).Count -ne 0) {
        throw 'Calibration summary does not contain 108 passing variants and 216 non-contradictory marginals.'
    }
    ...
    if ($activeValidationFiles.Count -ne 1 -or
        $activeValidationFiles[0].Name -ne 'Checkpoint_20a_Calibration_Reporting_Guard_Hotfix.md') {
        throw 'Checkpoint 20a must leave exactly one active validation runbook.'
    }

    Write-Host ''
    Write-Host 'Checkpoint 20a completed successfully.'
    Write-Host 'Engine-independent tests passed: 506.'
    Write-Host 'Deterministic headless scenarios passed: 7.'
    Write-Host 'Runner self-tests passed: 12.'
    ...
    Write-Host 'Statistically contradictory adjacent-TL marginals: 0.'
    Write-Host "Calibration result hash: $actualHash."
    Write-Host 'No mechanical Godot validation is required.'
    Write-Host 'Checkpoint 20 calibration mechanics are unchanged; the source-text reporting guard is repaired.'
    Write-Host 'Preserve out\checkpoint-20-terminal-tl-calibration for review.'
}
finally {
    Pop-Location
}
