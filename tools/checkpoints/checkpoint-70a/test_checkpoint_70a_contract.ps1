...
$normalRel = 'tools/calibration/checkpoints/checkpoint-70a.json'
$deepRel = 'tools/calibration/checkpoints/checkpoint-70a-deep-calibration.json'
$guardedPs = @(
    'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1',
    'tools/checkpoints/checkpoint-70a/apply_checkpoint_70a.ps1',
    'tools/checkpoints/checkpoint-70a/test_checkpoint_70a_contract.ps1',
...
Assert-True ([string]$normal.checkpointId -eq '70a' -and [string]$deep.checkpointId -eq '70a') 'Checkpoint 70a definition ID mismatch.'
Assert-True ([string]$normal.manifestFile -eq 'CHECKPOINT_70A_SHA256SUMS.txt' -and [string]$deep.manifestFile -eq 'CHECKPOINT_70A_SHA256SUMS.txt') 'Checkpoint 70a manifest binding mismatch.'
Assert-True ([string]$normal.outputRoot -eq 'out/checkpoint-70a' -and [string]$deep.outputRoot -eq 'out/checkpoint-70a-deep-calibration') 'Checkpoint 70a output-root binding mismatch.'
...
foreach ($cost in 1..5) {
    $matchingCost = @($variants | Where-Object {
        $value = Get-OptionalPropertyValue -Object $_ -Name 'sideBEcmNormalPowerCostOverride'
        $null -ne $value -and [int]$value -eq $cost
    })
    Assert-True ($matchingCost.Count -eq 18) "Checkpoint 70a ECM cost $cost must have exactly 18 variants."
}
$missingEcmOverride = @($variants | Where-Object { $null -eq (Get-OptionalPropertyValue -Object $_ -Name 'sideBEcmNormalPowerCostOverride') })
Assert-True ($missingEcmOverride.Count -eq 9) 'Checkpoint 70a requires exactly nine clear controls without an ECM power-cost override.'
...
$counterWithWrongEccmCost = @($counter | Where-Object {
    $value = Get-OptionalPropertyValue -Object $_ -Name 'sideAEccmNormalPowerCostOverride'
    $null -eq $value -or [int]$value -ne 1
})
Assert-True ($counter.Count -eq 45 -and $counterWithWrongEccmCost.Count -eq 0) 'Checkpoint 70a matched ECCM variants must explicitly hold normal ECCM cost at 1 TP.'
$missingEccmOverride = @($variants | Where-Object { $null -eq (Get-OptionalPropertyValue -Object $_ -Name 'sideAEccmNormalPowerCostOverride') })
Assert-True ($missingEccmOverride.Count -eq 54) 'Checkpoint 70a expects ECCM power-cost override only on the 45 matched-counter variants.'
...
# CP70a is validation-only: freeze the complete CP70 experimental runtime/study surface.
$cp70FrozenHashes = @{
    'src/StarCluster.ScenarioRunner/TL1Calibration/Tl1IntegratedTacticalCombatDocuments.cs' = '2908631c50912cc4ed510c7973480091a466b81397db2805fc77998b7cc10a1b'
    'src/StarCluster.ScenarioRunner/TL1Calibration/Tl1IntegratedTacticalCombatRunner.cs' = '7e691c3093b2940f625ba8270e5b86f709cca5b44b2bf0590d4ff24c5e4c2691'
    'src/StarCluster.ScenarioRunner/Program.cs' = 'c4ac6246742c4ee94332a37a46683958f11ad5ebf93545485ca26f50ebae0594'
    'src/StarCluster.ScenarioRunner/Scenarios/TL1Calibration/tl1-itc12-ecm-power-cost-point-blank-counterplay.json' = '950905f28be3c7399582bb2e5d22db009609ff42512e108e7dc1243a2e408835'
    'docs/design/player_technology/tl1_integrated_tactical_combat_schema_v0_15.json' = 'bbb444bbfb8c044019d0cf1fce800bc5d036f5e0b56ca2d90c2441e5d20f7d8e'
    'docs/design/testing/checkpoint_70_validation_suite_policy_v0_1.json' = 'edcafbd96eb1ebd55398e2f5346fe83c3b45372aa8491d7cb3fc71e24ba639ac'
    'docs/Star_Cluster_Game_Concept_v0.6i.docx' = '9bff5cbc25dba470fc27646a4f2cbb435dc9e0aef193bfd3608ed7e245c05c50'
    'tools/calibration/run_calibration_checkpoint.ps1' = 'fb7295a6bc2a1b1fa57145a7b10c06d5736eef6eb9299f33e3125c740aa55713'
}
foreach ($entry in $cp70FrozenHashes.GetEnumerator()) {
    $actual = (Get-FileHash -LiteralPath (Join-Path $repositoryRoot $entry.Key) -Algorithm SHA256).Hash.ToLowerInvariant()
    Assert-True ($actual -eq [string]$entry.Value) "Checkpoint 70a changed frozen CP70 experimental runtime/input: $($entry.Key)."
}
...
# CP70a regression: optional JSON properties may be absent on legitimate control variants under StrictMode.
$contractSource = Get-Content -LiteralPath $MyInvocation.MyCommand.Path -Raw
$rawEcmAccess = '$' + '_.sideBEcmNormalPowerCostOverride'
$rawEccmAccess = '$' + '_.sideAEccmNormalPowerCostOverride'
Assert-True (-not $contractSource.Contains($rawEcmAccess)) 'StrictMode regression: optional Side-B ECM override may not be accessed directly in the contract.'
Assert-True (-not $contractSource.Contains($rawEccmAccess)) 'StrictMode regression: optional Side-A ECCM override may not be accessed directly in the contract.'
Assert-True ($contractSource.Contains('function Get-OptionalPropertyValue')) 'StrictMode regression: optional-property helper is missing.'

$validationFiles = @(Get-ChildItem -LiteralPath (Join-Path $repositoryRoot 'docs\validation') -File -Filter 'Checkpoint_*.md')
Assert-True ($validationFiles.Count -eq 1 -and $validationFiles[0].Name -eq 'Checkpoint_70a_ECM_Power_Cost_Contract_StrictMode_Hotfix.md') 'Exactly one Checkpoint 70a active validation runbook must remain.'
$rootTxt = @(Get-ChildItem -LiteralPath $repositoryRoot -File -Filter '*.txt')
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_70A_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_70A_SHA256SUMS.txt as .txt.'
...
Write-Host '       Checkpoint 70a native dependency contract: PowerShell + pinned .NET only; active Python runtime dependencies are rejected before native work.'
...
Write-Host 'Checkpoint 70a StrictMode contract hotfix validation passed.'
