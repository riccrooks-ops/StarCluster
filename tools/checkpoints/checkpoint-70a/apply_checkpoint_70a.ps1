...
$contractCheck = Join-Path $PSScriptRoot 'test_checkpoint_70a_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-70a-deep-calibration.json' } else { 'checkpoint-70a.json' }
$definition = Join-Path $repositoryRoot (Join-Path 'tools\calibration\checkpoints' $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-70a.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-70a-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!70A(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!70a(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!70a(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_70a_ECM_Power_Cost_Contract_StrictMode_Hotfix.md') {
