...
$normalRel='tools/calibration/checkpoints/checkpoint-90a.json'; $deepRel='tools/calibration/checkpoints/checkpoint-90a-deep-calibration.json'
$guardedPs=@('tools/checkpoints/Test-NativeAcceptanceDependencies.ps1','tools/checkpoints/checkpoint-90a/apply_checkpoint_90a.ps1','tools/checkpoints/checkpoint-90a/test_checkpoint_90a_contract.ps1','tools/calibration/run_calibration_checkpoint.ps1')
& $guard -RepositoryRoot $repositoryRoot -PowerShellPaths $guardedPs -CheckpointDefinitionPaths @($normalRel,$deepRel)

Write-Host '       Validating Checkpoint 90a definitions and unchanged CP90 workload accounting...'
$normal=Read-Json $normalRel; $deep=Read-Json $deepRel
Assert-True ([string]$normal.checkpointId -eq '90a' -and [string]$deep.checkpointId -eq '90a') 'Checkpoint 90a definition ID mismatch.'
Assert-True ([string]$normal.manifestFile -eq 'CHECKPOINT_90a_SHA256SUMS.txt' -and [string]$deep.manifestFile -eq 'CHECKPOINT_90a_SHA256SUMS.txt') 'Checkpoint 90a manifest binding mismatch.'
...
Assert-True (-not $integrated.Contains('out Tl1IntegratedShipBuildDocument buildA')) 'CP90a must not reintroduce the nullable-unsafe generated Side-A build lookup.'
Assert-True (-not $integrated.Contains('out Tl1IntegratedShipBuildDocument buildB')) 'CP90a must not reintroduce the nullable-unsafe generated Side-B build lookup.'
foreach($needle in @('out Tl1IntegratedShipBuildDocument? buildA','buildA is null','out Tl1IntegratedShipBuildDocument? buildB','buildB is null','references an unknown generated build ID')){ Require-Contains $integrated $needle "CP90a nullable generated-build lookup hotfix is missing '$needle'." }
...
Write-Host '       Validating CP90a hotfix isolation from the generated CP90 study...'
$cp90Critical=@{
 'src/StarCluster.ScenarioRunner/CrossTlIntegration/CrossTlBuildPermutationRunner.cs'='a083e033b0844eef71459d031ea693e8cb2c35d2d40ac94c5e9b7e6a62e54b01';
 'src/StarCluster.ScenarioRunner/ScenarioRunnerSelfTests.cs'='733b9a0a3653f52453d7e9c2133544682879e52de718d966d4d64dd3bad14fe7';
 'src/StarCluster.ScenarioRunner/Scenarios/ArchitectureTechnology/cross-tl-build-permutation-foundation-v0_3.json'='d802a594424b141cd5ddf8431175d2ffc4ee62e2021de57f2834d508c8daa9c3';
 'src/StarCluster.ScenarioRunner/TL1Calibration/Tl1IntegratedTacticalCombatDocuments.cs'='8a9898792cda1ae29434a88fe786cb2f5df0552dee6ffcdc91dec5f98bb2c6db';
 'docs/Star_Cluster_Game_Concept_v0.6z.docx'='d2c514299ee4afaa2929904333b8883889732790782accec7af96b19750ff58c';
 'docs/design/player_technology/StarCluster_Technology_Architecture_Matrix_v1.xlsx'='b56cac0b3b00ffb33439e80bd4d738e0164350a0d407aa1f652b52e7aec3101a';
 'docs/archive/player_technology/pre-cp165-active/component_installation_space_catalog_v1.json'='9234d28c859edabd36f68e004d580abb5728ea6ad38b688288569d74394a7d92';
 'docs/design/testing/Technology_Integration_Permutation_Suite_Architecture_v0_9.md'='3beafcc0ffc735cc5d4658ae55489ec4f80539f39b720379bdb90e1dc3365b05';
 'docs/design/testing/technology_integration_permutation_suite_v0_9.json'='0f50e16c9d28af1e87f0a7b9ced892a1259fa0498004dde4d5796377776c0814'
}
foreach($rel in @($cp90Critical.Keys)){ Assert-True ((Hash-Rel $rel) -eq [string]$cp90Critical[$rel]) "CP90a unexpectedly changed CP90 substantive file '$rel'." }
Assert-True ((Hash-Rel 'src/StarCluster.ScenarioRunner/TL1Calibration/Tl1IntegratedTacticalCombatRunner.cs') -eq '6af9b4b48aee6e29c9bf7f4148e5070a13f94344bdd387c05c84a9e928828224') 'CP90a patched integrated-combat runner bytes drifted from the reviewed nullable hotfix.'
...
$activeValidation=@(Get-ChildItem -LiteralPath (RelPath 'docs/validation') -File -Filter 'Checkpoint_*.md'); Assert-True ($activeValidation.Count -eq 1 -and $activeValidation[0].Name -eq 'Checkpoint_90a_CP90_Native_Nullable_Generated_Build_Lookup_Hotfix.md') 'Exactly one active CP90a validation runbook must remain.'
...
Write-Host 'Checkpoint 90a repository contracts passed (CP90 study unchanged; nullable generated-build lookup corrected for warning-as-error native build).'
