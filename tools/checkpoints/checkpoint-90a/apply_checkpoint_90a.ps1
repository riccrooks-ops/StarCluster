...
$contractCheck = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-90a\test_checkpoint_90a_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-90a-deep-calibration.json' } else { 'checkpoint-90a.json' }
$definition = Join-Path $repositoryRoot ('tools\calibration\checkpoints\' + $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-90a.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-90a-deep-calibration.json'
$guardedPowerShellPaths = @(
    'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1',
    'tools/checkpoints/checkpoint-90a/apply_checkpoint_90a.ps1',
    'tools/checkpoints/checkpoint-90a/test_checkpoint_90a_contract.ps1',
...
foreach ($file in @(Get-ChildItem -LiteralPath $repositoryRoot -File -Filter '*.txt')) {
    if ($file.Name -match '^CHECKPOINT_(?!90a(?:_|$)).*_SHA256SUMS\.txt$' -or
        $file.Name -match '^Checkpoint_(?!90a(?:_|$)).*_Readme\.txt$' -or
        $file.Name -match '^checkpoint-(?!90a(?:-|$)).*-static-preflight\.txt$') {
...
foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
    if ($file.Name -ne 'Checkpoint_90a_CP90_Native_Nullable_Generated_Build_Lookup_Hotfix.md') {
...
Write-Host '[3/4] Verifying Checkpoint 90a CP90 native nullable generated-build lookup hotfix contracts...'
& $contractCheck
