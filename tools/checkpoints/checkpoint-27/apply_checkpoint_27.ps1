...
function Ensure-ActiveConceptDocument {
    ...
    if ($activeConcepts.Count -ne 1 -or $activeConcepts[0].Name -ne $ExpectedFileName) {
        ...
        throw "Exactly the Checkpoint 27 concept document must remain active. Found: $activeNames"
    }
...
function Test-ActiveConceptDocumentNormalization {
    ...
    $expectedName = 'Star_Cluster_Game_Concept_v0.3y.docx'
...
function Assert-RepositoryManifest {
    ...
    foreach ($line in Get-Content -LiteralPath $ManifestPath) {
        ...
        if ($relativePath -eq 'CHECKPOINT_27_SHA256SUMS.txt') {
    ...
    if ($verified -ne 583) { throw "Repository manifest verified $verified files; the Checkpoint 27 archive requires exactly 583 files." }
...
function Ensure-ActiveValidationRunbook {
    ...
    if ($activeRunbooks.Count -ne 1 -or $activeRunbooks[0].Name -ne $ExpectedFileName) {
        ...
        throw "Exactly the Checkpoint 27 validation runbook must remain active. Found: $activeNames"
    }
...
function Test-ActiveValidationRunbookNormalization {
    ...
    $expectedName = 'Checkpoint_27_Revised_TL1_Defensive_Envelope_And_Kinetic_Interaction_Calibration.md'
...
function Assert-TechnologyCsvContracts {
    ...
    if ($recon.Count -ne 32) { throw "Expected 32 Checkpoint 27 reconciliation rows; found $($recon.Count)." }
    if (($recon[0].PSObject.Properties.Name -join ',') -ne 'topic,current_direction,reference_influence,checkpoint_25_decision,status,next_action') { throw 'Checkpoint 27 reconciliation header is invalid.' }
    ...
    if ($baselineHash -ne 'd49fdffb60f3c7ee41c5e587daaabcdabf95ffd230e3aa947be5de5a291c6a1e') { throw "TL1 baseline SHA-256 is $baselineHash, not the accepted Checkpoint 27 hash." }
    $requiredValues = @{
...
function Assert-Tl1ScenarioCorpusContracts {
    ...
    if ($implemented.Count -ne 12) { throw "Expected 12 implemented Checkpoint 27 matrix rows; found $($implemented.Count)." }
...
function Assert-WorkbookContract {
    $path = '.\docs\design\player_technology\StarCluster_Player_TL_Framework_Draft_v0_7.xlsx'
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw 'Technology workbook v0.7 was not found.' }
    Add-Type -AssemblyName System.IO.Compression.FileSystem
    $zip = [System.IO.Compression.ZipFile]::OpenRead((Resolve-Path -LiteralPath $path).Path)
    try {
        ...
        foreach ($sheetName in @('Overview','TL1 Baseline','TL1 Loadouts','TL1 Test Matrix','Phase A Runtime','TL1 Phase B','TL1 Calibration','Component Schema','Checkpoint 25 Plan','TL Matrix','Components','Compatibility Profiles','Adaptation Rules','Reference Library','Reference Insights','Design Reconciliation','Level Themes','Design Decisions','Sources Used')) {
        ...
        foreach ($marker in @('D-249','TL1 Calibration','29','100 attack packages')) {
            if (-not $allXml.Contains($marker)) { throw "Workbook does not contain required Checkpoint 27 marker $marker." }
        }
    }
    finally { $zip.Dispose() }
    Write-Host '       Workbook OOXML: Checkpoint 27 TL1 Calibration sheet and Decision D-249 present; no structured table parts.'
}

function Assert-NewUnitTestContracts {
    $path = '.\tests\StarCluster.Tests\Combat\DirectFire\Tl1DirectFireAccuracyTests.cs'
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Checkpoint 27 direct-fire unit-test file was not found." }
    ...
    if ($facts -ne 10 -or $theories -ne 3) { throw "Expected 10 Fact methods and 3 Theory methods in the Checkpoint 27 direct-fire test file; found $facts and $theories." }
    Assert-FileContains $path @('CalculatorMatchesAcceptedTl1Examples','SimultaneousWindowAllowsMutualDestruction','KineticMirrorAllHitsMutuallyDestroysOnTurnNine') 'Checkpoint 27 direct-fire tests'
    $calibrationPath = '.\tests\StarCluster.Tests\Combat\DirectFire\Tl1KineticDuelCalibrationTests.cs'
    if (-not (Test-Path -LiteralPath $calibrationPath -PathType Leaf)) { throw 'Checkpoint 27 calibration test file was not found.' }
    ...
    Assert-FileContains $calibrationPath @('RevisedBaselineAllHitsMutuallyDestroysOnTurnNine','ArmorProtectionTwoWithoutApenCanStallDamageThree','ApenTwoRestoresProgressAgainstArmorProtectionTwo') 'Checkpoint 27 calibration tests'
    Write-Host '       Checkpoint 27 unit-test source: 10 direct-fire facts, 3 theories, and 8 calibration facts present.'
}
...
try {
    Write-Host '[1/10] Verifying complete Checkpoint 27 repository and parser contracts...'
    foreach ($requiredFile in @(
        '.\StarCluster.sln',
        '.\global.json',
        '.\CHECKPOINT_27_SHA256SUMS.txt',
        ...
        '.\docs\Star_Cluster_Game_Concept_v0.3y.docx',
        '.\docs\design\player_technology\TL1_Core_Combat_Test_Plan_v0_3.md',
        '.\docs\design\player_technology\StarCluster_Player_TL_Framework_Draft_v0_7.xlsx',
        '.\docs\checkpoints\Checkpoint_27_Revised_TL1_Defensive_Envelope_And_Kinetic_Interaction_Calibration.md',
        '.\docs\validation\Checkpoint_27_Revised_TL1_Defensive_Envelope_And_Kinetic_Interaction_Calibration.md',
        ...
        if (-not (Test-Path -LiteralPath $requiredFile -PathType Leaf)) { throw "Required Checkpoint 27 file $requiredFile was not found." }
    }
    Ensure-ActiveValidationRunbook -ValidationDirectory '.\docs\validation' -ExpectedFileName 'Checkpoint_27_Revised_TL1_Defensive_Envelope_And_Kinetic_Interaction_Calibration.md' -ArchiveDirectory '.\docs\validation\archive'
    Ensure-ActiveConceptDocument -DocsDirectory '.\docs' -ExpectedFileName 'Star_Cluster_Game_Concept_v0.3y.docx' -ArchiveDirectory '.\docs\archive'
    Test-NormalizedRelativePathCompatibility
    Test-RepositoryManifestLocalArtifactPolicy
    Assert-RepositoryManifest '.\CHECKPOINT_27_SHA256SUMS.txt'
    Assert-PowerShellScriptsParse '.\tools'
    Test-ActiveValidationRunbookNormalization
    Test-ActiveConceptDocumentNormalization

    Write-Host '[2/10] Verifying Concept, reference library, exact data, schema, corpus, tests, and workbook contracts...'
    Assert-FileContains '.\README.md' @('Checkpoint 27','v0.3y','29-variant','597 engine-independent tests') 'Repository README'
    Assert-FileContains '.\docs\checkpoints\Checkpoint_27_Revised_TL1_Defensive_Envelope_And_Kinetic_Interaction_Calibration.md' @('revised exact provisional baseline','29 kinetic calibration variants','common-random-number','does not lock final TL1 balance') 'Checkpoint 27 documentation contract'
    ...
    Assert-FileContains '.\docs\Prototype_TODO.md' @('Checkpoint 27 status','100 packages','29-variant kinetic calibration study','10,000-trial Windows acceptance study') 'Prototype TODO'
    ...
    Assert-DocxContains '.\docs\Star_Cluster_Game_Concept_v0.3y.docx' @('Revised provisional TL1 baseline','Phase B kinetic calibration study','D-249','100 attack packages','END OF DRAFT v0.3y') 'Concept v0.3y'
    ...
    if ($RepositoryContractOnly) {
        Write-Host ''
        Write-Host 'Checkpoint 27 repository-contract preflight completed successfully.'
        Write-Host 'Manifest, parser, normalization, reference, documentation, CSV, Phase A/B/kinetic calibration corpus, schema, test-source, and workbook contracts passed.'
        return
    }

    Write-Host '[3/10] Confirming Godot is closed and .NET SDK 8.0.423 is selected...'
    ...
    if ($godotProcesses) {
        ...
        throw "Close Godot before applying Checkpoint 27. Running process(es): $processNames"
    }
    $sdkVersion = dotnet --version
    Write-Host "       SDK: $sdkVersion"
    if ($sdkVersion -ne '8.0.423') { throw "Expected .NET SDK 8.0.423 from global.json, but dotnet selected $sdkVersion." }

    Write-Host '[4/10] Performing a clean compiler preflight with warnings as errors...'
    ...
    Write-Host '[5/10] Running 597 engine-independent tests...'
    $testOutput = & dotnet test '.\tests\StarCluster.Tests\StarCluster.Tests.csproj' --no-build --nologo 2>&1
    $testOutput | Write-Host
    if ($LASTEXITCODE -ne 0) { throw "dotnet test failed with exit code $LASTEXITCODE." }
    if (($testOutput | Out-String) -notmatch 'Passed:\s+597') { throw 'The complete suite did not report the expected 597 passed tests.' }

    Write-Host '[6/10] Running seven accepted deterministic moving-missile scenarios...'
    $legacyOutput = '.\out\checkpoint-27-deterministic'
    Remove-Item $legacyOutput -Recurse -Force -ErrorAction SilentlyContinue
    $legacyText = Invoke-Runner -Arguments @('run-all','--scenario-dir','.\src\StarCluster.ScenarioRunner\Scenarios','--output-dir',$legacyOutput) -Description 'Checkpoint 27 legacy deterministic corpus'
    ...
    Write-Host '[7/10] Running 12 TL1 Phase A documents and 54 mechanics cases...'
    $phaseAOutput = '.\out\checkpoint-27-tl1-phase-a'
    Remove-Item $phaseAOutput -Recurse -Force -ErrorAction SilentlyContinue
    $phaseAText = Invoke-Runner -Arguments @('tl1-phase-a','--scenario-dir','.\src\StarCluster.ScenarioRunner\Scenarios\TL1PhaseA','--baseline-file','.\docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_1.csv','--output-dir',$phaseAOutput) -Description 'Checkpoint 27 TL1 Phase A corpus'
    ...
    Write-Host '[8/10] Running seven TL1 Phase B documents and 36 direct-fire cases...'
    $phaseBOutput = '.\out\checkpoint-27-tl1-phase-b'
    ...
    Write-Host '[9/10] Running 29 TL1 kinetic-interaction calibration variants at 10,000 trials each...'
    $phaseCOutput = '.\out\checkpoint-27-tl1-kinetic-calibration'
    Remove-Item $phaseCOutput -Recurse -Force -ErrorAction SilentlyContinue
    $phaseCText = Invoke-Runner -Arguments @('tl1-kinetic-calibration','--study-file','.\src\StarCluster.ScenarioRunner\Scenarios\TL1Calibration\tl1-kc01-kinetic-interaction-study.json','--baseline-file','.\docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_1.csv','--trials','10000','--jobs','24','--output-dir',$phaseCOutput) -Description 'Checkpoint 27 TL1 kinetic calibration study'
    ...
    Write-Host '[10/10] Running forty-six ScenarioRunner self-tests...'
    $selfTestText = Invoke-Runner -Arguments @('self-test','--scenario-file','.\src\StarCluster.ScenarioRunner\Scenarios\terminal-two-window-hit.json') -Description 'Checkpoint 27 runner self-tests'
    ...
    Write-Host ''
    Write-Host 'Checkpoint 27 completed successfully.'
    Write-Host 'Engine-independent tests passed: 597.'
    ...
    Write-Host 'Kinetic Monte Carlo calibration completed; no mechanical Godot validation is required.'
}
