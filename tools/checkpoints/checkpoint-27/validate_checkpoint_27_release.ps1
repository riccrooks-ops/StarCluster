...
$stagingRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-27-release-{0}" -f ([guid]::NewGuid().ToString('N')))
...
function Assert-ArchiveContract {
    ...
    try {
        ...
        $manifestEntry = $zip.GetEntry('CHECKPOINT_27_SHA256SUMS.txt')
        ...
        foreach ($line in ($manifestText -split '\r?\n')) {
            ...
            if ($name -eq 'CHECKPOINT_27_SHA256SUMS.txt') { throw 'Archive manifest must not contain itself.' }
            if ($manifest.ContainsKey($name)) { throw "Duplicate archive manifest path: $name" }
            $manifest[$name] = $m.Groups[1].Value.ToLowerInvariant()
        }
        if ($manifest.Count -ne 583) { throw "Checkpoint 27 archive manifest must contain exactly 583 files; found $($manifest.Count)." }
        $normalizationInputs = @(
        ...
        $actualNames = @($entries | ForEach-Object { $_.FullName.Replace('\','/') } | Where-Object {
            $_ -ne 'CHECKPOINT_27_SHA256SUMS.txt' -and $normalizationInputs -notcontains $_
        })
        foreach ($normalizationInput in $normalizationInputs) {
            if (-not $names.Contains($normalizationInput)) { throw "Archive normalization input is missing: $normalizationInput" }
        }
...
try {
    ...
    $checkpointScript = Join-Path $stagingRoot 'tools\checkpoints\checkpoint-27\apply_checkpoint_27.ps1'
    if (-not (Test-Path -LiteralPath $checkpointScript -PathType Leaf)) { throw "Checkpoint 27 application script was not found in $resolvedArchive." }

    Write-Host '[2/5] Running clean-extraction repository-contract preflight...'
    & $checkpointScript -RepositoryContractOnly

    Write-Host '[3/5] Running clean warning-as-error build, tests, legacy scenarios, Phase A/B corpora, kinetic calibration, and runner self-tests...'
    & $checkpointScript
    ...
    $archivedRunbook = Join-Path $validationDirectory 'archive\Checkpoint_26_TL1_Direct_Fire_Accuracy_Simultaneous_Volley_And_Kinetic_Mirror_Duel_Foundation.md'
    $staleActiveRunbook = Join-Path $validationDirectory 'Checkpoint_26_TL1_Direct_Fire_Accuracy_Simultaneous_Volley_And_Kinetic_Mirror_Duel_Foundation.md'
    if (-not (Test-Path -LiteralPath $archivedRunbook -PathType Leaf)) { throw 'The packaged Checkpoint 26 archived validation runbook was not found.' }
    Copy-Item -LiteralPath $archivedRunbook -Destination $staleActiveRunbook -Force

    $archivedConcept = Join-Path $stagingRoot 'docs\archive\Star_Cluster_Game_Concept_v0.3x.docx'
    $staleActiveConcept = Join-Path $stagingRoot 'docs\Star_Cluster_Game_Concept_v0.3x.docx'
    if (-not (Test-Path -LiteralPath $archivedConcept -PathType Leaf)) { throw 'The packaged Concept v0.3x archive copy was not found.' }
    ...
    $localArtifacts = @{
        ...
        'out\checkpoint-27-release-test\result.txt' = 'local output'
        'Checkpoint_26_Readme.txt' = 'stale checkpoint identity'
        'CHECKPOINT_26_SHA256SUMS.txt' = 'stale checkpoint identity'
        'StarCluster_Checkpoint_26_Local_Copy.zip' = 'local package copy'
    }
    ...
    if ($activeRunbooks.Count -ne 1 -or $activeRunbooks[0].Name -ne 'Checkpoint_27_Revised_TL1_Defensive_Envelope_And_Kinetic_Interaction_Calibration.md') { throw 'Release validation left an invalid active-runbook set.' }
    ...
    Write-Host ''
    Write-Host 'Checkpoint 27 release validation completed successfully.'
    Write-Host 'Archive integrity, clean extraction, full acceptance, dirty-tree normalization, local-artifact tolerance, and unknown-source rejection passed.'
}
