...
$active = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-63.json')
$deep = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-63-deep-calibration.json')
...
Assert-True ([int]$production.powerAndControl.reactorOutput -eq 5) 'Checkpoint 63a may not silently change the production TL1 reactor from 5 TP.'

Assert-True ([string]$study.id -eq 'tl1-itc06-35-space-operational-sensor-acquisition-ew') 'Unexpected Checkpoint 63a study ID.'
Assert-True (@($study.builds).Count -eq 6) 'Checkpoint 63a study must define all six accepted legal composed builds.'
Assert-True (@($study.variants).Count -eq 72) 'Checkpoint 63a study must define exactly 72 variants.'
...
Assert-True (($buildIds | Select-Object -Unique).Count -eq 6) 'Checkpoint 63a build IDs must be unique.'
...
Assert-True ($must.Count -eq 8 -and $must -contains 'tl1-operational-sensor-acquisition-ew' -and -not ($must -contains 'tl1-power-doctrine-reactor-sensitivity')) 'Checkpoint 63a normal suite must contain the current sensor study and retire Checkpoint 62 Monte Carlo from the normal lineup.'
Assert-True ($deepOnly.Count -eq 14 -and $deepOnly -contains 'tl1-power-doctrine-reactor-sensitivity' -and $deepOnly -contains 'tl1-composed-ship-odd-build-combat') 'Checkpoint 63a Deep Calibration must retain Checkpoints 62 and 61 plus twelve historical stochastic stages.'
Assert-True ([int]$active.checkpointMetrics.stageCount -eq 8 -and [int]$active.checkpointMetrics.monteCarloVariantCount -eq 72 -and [int]$active.checkpointMetrics.trialsAtDefault -eq 720000) 'Checkpoint 63a active workload metrics mismatch.'
Assert-True ([int]$deep.checkpointMetrics.stageCount -eq 22 -and [int]$deep.checkpointMetrics.monteCarloVariantCount -eq 1260 -and [int]$deep.checkpointMetrics.trialsAtDefault -eq 12600000) 'Checkpoint 63a Deep workload metrics mismatch.'
$activeIds = @($active.stages | ForEach-Object { [string]$_.id })
$deepIds = @($deep.stages | ForEach-Object { [string]$_.id })
Assert-True ($activeIds[3] -eq 'tl1-installation-space-envelope' -and $activeIds[4] -eq 'tl1-operational-sensor-acquisition-ew') 'Checkpoint 63a construction/sensor stages are out of order.'
Assert-True ($deepIds[4] -eq 'tl1-operational-sensor-acquisition-ew' -and $deepIds[5] -eq 'tl1-power-doctrine-reactor-sensitivity' -and $deepIds[6] -eq 'tl1-composed-ship-odd-build-combat') 'Checkpoint 63a Deep Calibration must run current sensor study before Checkpoints 62 and 61 historical regressions.'
...
Assert-True ($docs -match 'Tl1OperationalTrackPolicy' -and $docs -match 'sideANetEwRangePenalty') 'Integrated combat document model is missing Checkpoint 63a track/EW controls.'
Assert-True ($runner -match 'tl1-itc06-35-space-operational-sensor-acquisition-ew') 'Integrated runner is missing the Checkpoint 63a study ID.'
...
Assert-True ($runner -match 'operational-sensor-acquisition-matrix.csv' -and $runner -match 'tl1-c63-outcomes-review-only') 'Integrated runner is missing Checkpoint 63a review outputs/gates.'
Assert-True ($runner -match 'A lane may legitimately remain NoTrack' -and $runner -match 'sensorEnvelope.PassiveFirmRange' -and $runner -match 'sensorlessBuildIds.All') 'Checkpoint 63a passive-core hotfix gate semantics are missing.'

$validationFiles = @(Get-ChildItem -LiteralPath (Join-Path $repositoryRoot 'docs\validation') -File -Filter '*.md')
Assert-True ($validationFiles.Count -eq 1 -and $validationFiles[0].Name -eq 'Checkpoint_63a_Passive_Core_Gate_Hotfix.md') 'Exactly one Checkpoint 63a active validation runbook must remain.'
...
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_63A_SHA256SUMS.txt') 'Repository root must contain only the current Checkpoint 63a manifest as a .txt file.'
...
Write-Host '       Checkpoint 63a design contract: six accepted TL1 builds x three weapon families x four paired acquisition regimes = 72 variants.'
