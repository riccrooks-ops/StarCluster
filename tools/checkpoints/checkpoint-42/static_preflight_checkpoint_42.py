#!/usr/bin/env python3
"""Repository-only release checks for Checkpoint 42.
...
def main():
    ...
    definition=json.loads((root/'tools/calibration/checkpoints/checkpoint-42.json').read_text());assert len(definition['stages'])==18 and definition['manifestFile']=='CHECKPOINT_42_SHA256SUMS.txt'
    ...
    ok('Checkpoint 42 definition resolves 18 stages and 1,350 Monte Carlo variants')
    ...
    concept=root/'docs/Star_Cluster_Game_Concept_v0.4o.docx';workbook=root/'docs/design/player_technology/StarCluster_Player_TL_Framework_Draft_v0_23.xlsx'
    for p in (concept,workbook):
        with zipfile.ZipFile(p) as z:assert z.testzip() is None
    from docx import Document
    doc=Document(concept);assert any('Checkpoint 42 within-band progression' in p.text for p in doc.paragraphs);assert doc.paragraphs[-1].text.strip()=='END OF DRAFT v0.4o'
    from openpyxl import load_workbook
    wb=load_workbook(workbook,data_only=False);assert 'Checkpoint 42 Progression' in wb.sheetnames
    bad=[]
    ...
    assert not bad
    ok(f'active Concept/workbook package integrity; {len(wb.sheetnames)} workbook sheets')
    output='\n'.join(lines)+'\n'
