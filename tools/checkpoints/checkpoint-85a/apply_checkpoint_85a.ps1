...
$contractCheck = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-85a\test_checkpoint_85a_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-85a-deep-calibration.json' } else { 'checkpoint-85a.json' }
$definition = Join-Path $repositoryRoot ('tools\calibration\checkpoints\' + $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-85a.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-85a-deep-calibration.json'
$guardedPowerShellPaths = @(
    'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1',
    'tools/checkpoints/checkpoint-85a/apply_checkpoint_85a.ps1',
    'tools/checkpoints/checkpoint-85a/test_checkpoint_85a_contract.ps1',
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!85a(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!85a(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!85a(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_85a_CP85_Native_Contract_ProfileLabel_Hotfix.md') {
...
Write-Host '[3/4] Verifying Checkpoint 85a CP85 native-contract hotfix and Armor study contracts...'
& $contractCheck
