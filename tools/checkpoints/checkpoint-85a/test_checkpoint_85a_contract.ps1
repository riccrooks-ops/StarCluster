...
$normalRel='tools/calibration/checkpoints/checkpoint-85a.json'
$deepRel='tools/calibration/checkpoints/checkpoint-85a-deep-calibration.json'
$guardedPs=@('tools/checkpoints/Test-NativeAcceptanceDependencies.ps1','tools/checkpoints/checkpoint-85a/apply_checkpoint_85a.ps1','tools/checkpoints/checkpoint-85a/test_checkpoint_85a_contract.ps1','tools/calibration/run_calibration_checkpoint.ps1')
...
Write-Host '       Validating Checkpoint 85a definitions and unchanged CP85 workload accounting...'
$normal=Read-Json $normalRel; $deep=Read-Json $deepRel
Assert-True ([string]$normal.checkpointId -eq '85a' -and [string]$deep.checkpointId -eq '85a') 'Checkpoint 85a definition ID mismatch.'
Assert-True ([string]$normal.manifestFile -eq 'CHECKPOINT_85a_SHA256SUMS.txt' -and [string]$deep.manifestFile -eq 'CHECKPOINT_85a_SHA256SUMS.txt') 'Checkpoint 85a manifest binding mismatch.'
Assert-True ([string]$normal.outputRoot -eq 'out/checkpoint-85a' -and [string]$deep.outputRoot -eq 'out/checkpoint-85a-deep-calibration') 'Checkpoint 85a output-root binding mismatch.'
...
Assert-True ($activeValidation.Count -eq 1 -and $activeValidation[0].Name -eq 'Checkpoint_85a_CP85_Native_Contract_ProfileLabel_Hotfix.md') 'Exactly one CP85a active validation runbook must remain.'
...
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_85a_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_85a_SHA256SUMS.txt as .txt.'
...
Write-Host 'Checkpoint 85a contract validation passed (CP85 study unchanged; profileLabel contract corrected).'
