...
$contractCheck = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-79\test_checkpoint_79_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-79-deep-calibration.json' } else { 'checkpoint-79.json' }
$definition = Join-Path $repositoryRoot ('tools\calibration\checkpoints\' + $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-79.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-79-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!79(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!79(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!79(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_79_TL2_Sensor_EW_Discrimination_And_Rating_Isolation.md') {
...
Write-Host '[3/4] Verifying Checkpoint 79 TL2 Sensor/EW isolation contracts...'
& $contractCheck
