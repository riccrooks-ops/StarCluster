...
$contractCheck = Join-Path $PSScriptRoot 'test_checkpoint_74c_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-74c-deep-calibration.json' } else { 'checkpoint-74c.json' }
$definition = Join-Path $repositoryRoot (Join-Path 'tools\calibration\checkpoints' $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-74c.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-74c-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!74C(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!74c(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!74c(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_74c_Degraded_Fire_Family_Coverage_Gate_Hotfix.md') {
