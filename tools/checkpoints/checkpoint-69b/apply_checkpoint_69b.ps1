...
$contractCheck = Join-Path $PSScriptRoot 'test_checkpoint_69b_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-69b-deep-calibration.json' } else { 'checkpoint-69b.json' }
$definition = Join-Path $repositoryRoot (Join-Path 'tools\calibration\checkpoints' $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-69b.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-69b-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!69B(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!69b(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!69b(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_69b_Sensor_EW_Candidate_Nullable_Control_Compile_Hotfix.md') {
