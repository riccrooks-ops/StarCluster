...
$normalRel = 'tools/calibration/checkpoints/checkpoint-69b.json'
$deepRel = 'tools/calibration/checkpoints/checkpoint-69b-deep-calibration.json'
$guardedPs = @(
    'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1',
    'tools/checkpoints/checkpoint-69b/apply_checkpoint_69b.ps1',
    'tools/checkpoints/checkpoint-69b/test_checkpoint_69b_contract.ps1',
...
Assert-True ([string]$normal.checkpointId -eq '69b') 'Checkpoint 69b normal definition ID mismatch.'
Assert-True ([string]$deep.checkpointId -eq '69b') 'Checkpoint 69b deep definition ID mismatch.'
Assert-True ([string]$normal.manifestFile -eq 'CHECKPOINT_69B_SHA256SUMS.txt') 'Checkpoint 69b normal manifest binding mismatch.'
Assert-True ([string]$deep.manifestFile -eq 'CHECKPOINT_69B_SHA256SUMS.txt') 'Checkpoint 69b deep manifest binding mismatch.'
Assert-True ([string]$normal.outputRoot -eq 'out/checkpoint-69b') 'Checkpoint 69b normal output root mismatch.'
Assert-True ([string]$deep.outputRoot -eq 'out/checkpoint-69b-deep-calibration') 'Checkpoint 69b deep output root mismatch.'
$expectedGuardedPs = @(
    'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1',
    'tools/checkpoints/checkpoint-69b/apply_checkpoint_69b.ps1',
    'tools/checkpoints/checkpoint-69b/test_checkpoint_69b_contract.ps1',
...
foreach ($definitionDocument in @($normal, $deep)) {
    Assert-True ([bool]$definitionDocument.nativeDependencyPrecheck.required) 'Checkpoint 69b definitions must require the native dependency precheck.'
    Assert-True ((@($definitionDocument.nativeDependencyPrecheck.powerShellPaths) -join '|') -eq ($expectedGuardedPs -join '|')) 'Checkpoint 69b native dependency PowerShell path binding mismatch.'
    Assert-True ((@($definitionDocument.nativeDependencyPrecheck.checkpointDefinitionPaths) -join '|') -eq ($expectedGuardedDefs -join '|')) 'Checkpoint 69b native dependency definition binding mismatch.'
}
...
# CP69b compile regressions learned from the failed native CP69/CP69a builds.
Assert-True (-not $runnerSource.Contains('v.ProtectedCompartmentation || !v.BaseShieldRechargeEnabled ||')) 'CP69b compile regression: nullable BaseShieldRechargeEnabled may not be negated directly inside a non-nullable || chain.'
Assert-True ($runnerSource.Contains('v.ProtectedCompartmentation || !(v.BaseShieldRechargeEnabled ?? true) ||')) 'CP69b compile regression: fixed-control shield check must coalesce nullable BaseShieldRechargeEnabled to its existing default true value before negation.'
Assert-True (-not $runnerSource.Contains('v.EvasiveManeuversEnabled || !v.PdsEnabled || v.EscapeDisengagementEnabled ||')) 'CP69b compile regression: nullable Evasive/PDS/Escape controls may not be used directly inside a non-nullable || chain.'
Assert-True ($runnerSource.Contains('v.EvasiveManeuversEnabled != false || v.PdsEnabled != true ||')) 'CP69b compile regression: fixed-control validation must require explicit Evasive=false and PDS=true values.'
Assert-True ($runnerSource.Contains('v.EscapeDisengagementEnabled != false ||')) 'CP69b compile regression: fixed-control validation must require explicit EscapeDisengagement=false.'
...
Assert-True ($reviewStart -ge 0 -and $reviewEnd -gt $reviewStart) 'CP69b compile regression: unable to isolate Sensor/EW candidate review writer.'
$reviewText = $runnerSource.Substring($reviewStart, $reviewEnd - $reviewStart)
Assert-True ($reviewText.Contains('static double Conditional(Tl1IntegratedTacticalCombatVariantSummary result)')) 'CP69b compile regression: Sensor/EW candidate review writer must define its own method-local Conditional helper.'
Assert-True ($reviewText.Contains('F(Conditional(result))')) 'CP69b compile regression: candidate review output must continue using the local conditional-win helper.'

# CP69b is compile-only: pin all non-runner Sensor/EW authority and study inputs to the original CP69 package.
$cp69FrozenHashes = @{
...
foreach ($entry in $cp69FrozenHashes.GetEnumerator()) {
    ...
    Assert-True ($actual -eq [string]$entry.Value) "CP69b compile-only hotfix changed frozen CP69 authority/input: $($entry.Key)."
}

$validationFiles = @(Get-ChildItem -LiteralPath (Join-Path $repositoryRoot 'docs\validation') -File -Filter 'Checkpoint_*.md')
Assert-True ($validationFiles.Count -eq 1 -and $validationFiles[0].Name -eq 'Checkpoint_69b_Sensor_EW_Candidate_Nullable_Control_Compile_Hotfix.md') 'Exactly one Checkpoint 69b active validation runbook must remain.'
$rootTxt = @(Get-ChildItem -LiteralPath $repositoryRoot -File -Filter '*.txt')
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_69B_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_69B_SHA256SUMS.txt as .txt.'
...
Write-Host '       Checkpoint 69b native dependency contract: PowerShell + pinned .NET only; active Python runtime dependencies are rejected before native work.'
...
Write-Host 'Checkpoint 69b nullable-control compile-hotfix contract validation passed.'
