...
required = [
    ...
    "docs/validation/Checkpoint_63_TL1_Operational_Sensor_Acquisition_And_EW.md",
    ...
    "tools/checkpoints/checkpoint-63/apply_checkpoint_63.ps1",
    "tools/checkpoints/checkpoint-63/test_checkpoint_63_contract.ps1",
    "tools/checkpoints/checkpoint-63/static_preflight_checkpoint_63.py",
    "CHECKPOINT_63_SHA256SUMS.txt",
]
for rel in required:
...
ok(root_txt == ["CHECKPOINT_63_SHA256SUMS.txt"], f"root txt hygiene {root_txt}")
...
ok(
    ...
    and active_runbooks[0].name == "Checkpoint_63_TL1_Operational_Sensor_Acquisition_And_EW.md",
...
apply_script = (ROOT / "tools/checkpoints/checkpoint-63/apply_checkpoint_63.ps1").read_text(encoding="utf-8")
ok("Checkpoint_*.md" in apply_script and "Checkpoint_63_TL1_Operational_Sensor_Acquisition_And_EW.md" in apply_script, "overlay-safe stale active runbook cleanup")

# Manifest format and independent content hash verification.
manifest = ROOT / "CHECKPOINT_63_SHA256SUMS.txt"
if manifest.is_file():
    entries = []
    ...
    ok("CHECKPOINT_63_SHA256SUMS.txt" not in paths, "manifest excludes itself")
...
print(f"Checkpoint 63 static preflight: {checks - len(errors)}/{checks} checks passed.")
if errors:
