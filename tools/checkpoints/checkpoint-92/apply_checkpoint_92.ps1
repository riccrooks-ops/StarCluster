...
$contractCheck = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-92\test_checkpoint_92_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-92-deep-calibration.json' } else { 'checkpoint-92.json' }
$definition = Join-Path $repositoryRoot ('tools\calibration\checkpoints\' + $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-92.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-92-deep-calibration.json'
$guardedPowerShellPaths = @(
    'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1',
    'tools/checkpoints/checkpoint-92/apply_checkpoint_92.ps1',
    'tools/checkpoints/checkpoint-92/test_checkpoint_92_contract.ps1',
...
foreach ($file in @(Get-ChildItem -LiteralPath $repositoryRoot -File -Filter '*.txt')) {
    if ($file.Name -match '^CHECKPOINT_(?!92(?:_|$)).*_SHA256SUMS\.txt$' -or
        $file.Name -match '^Checkpoint_(?!92(?:_|$)).*_Readme\.txt$' -or
        $file.Name -match '^checkpoint-(?!92(?:-|$)).*-static-preflight\.txt$') {
...
foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
    if ($file.Name -ne 'Checkpoint_92_Expanded_Spacedock_Reference_Corpus_And_Completed_Queue.md') {
...
foreach ($file in @(Get-ChildItem -LiteralPath $testingRoot -File -Filter 'Checkpoint_*_Validation_Tiers.md')) {
    if ($file.Name -ne 'Checkpoint_92_Validation_Tiers.md') { Move-Item -LiteralPath $file.FullName -Destination (Join-Path $testingArchive $file.Name) -Force }
}
foreach ($file in @(Get-ChildItem -LiteralPath $testingRoot -File -Filter 'checkpoint_*_validation_suite_policy*.json')) {
    if ($file.Name -ne 'checkpoint_92_validation_suite_policy_v0_1.json') { Move-Item -LiteralPath $file.FullName -Destination (Join-Path $testingArchive $file.Name) -Force }
}

Write-Host '[3/4] Verifying Checkpoint 92 expanded Spacedock reference-mining contracts...'
