...
$guardedPowerShellPaths = @(
    ...
    'tools/checkpoints/checkpoint-66c/apply_checkpoint_66c.ps1',
    'tools/checkpoints/checkpoint-66c/test_checkpoint_66c_contract.ps1',
    'tools/calibration/run_calibration_checkpoint.ps1'
)
$guardedDefinitionPaths = @(
    'tools/calibration/checkpoints/checkpoint-66c.json',
    'tools/calibration/checkpoints/checkpoint-66c-deep-calibration.json'
)
...
$active = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-66c.json')
$deep = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-66c-deep-calibration.json')
...
Assert-True ([string]$study.id -eq 'tl1-itc09-scripted-overload-tactics') 'Unexpected Checkpoint 66/66c study ID.'
Assert-True (@($study.variants).Count -eq 80) 'Checkpoint 66/66c study must define exactly 80 variants.'
...
Assert-True ([string]$active.checkpointId -eq '66c' -and [string]$deep.checkpointId -eq '66c') 'Checkpoint definitions must identify 66c.'
Assert-True ([string]$active.manifestFile -eq 'CHECKPOINT_66C_SHA256SUMS.txt' -and [string]$deep.manifestFile -eq 'CHECKPOINT_66C_SHA256SUMS.txt') 'Checkpoint definitions must bind the CP66c manifest.'
Assert-True ([string]$active.outputRoot -eq 'out/checkpoint-66c' -and [string]$deep.outputRoot -eq 'out/checkpoint-66c-deep-calibration') 'Checkpoint 66c output roots mismatch.'
...
foreach ($definition in @($active,$deep)) {
    ...
    Assert-True ([bool]$pre.required -and [string]$pre.script -eq 'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1') 'CP66c definitions must require the shared native dependency precheck.'
    Assert-True (@($pre.powerShellPaths) -contains 'tools/calibration/run_calibration_checkpoint.ps1' -and @($pre.powerShellPaths) -contains 'tools/checkpoints/checkpoint-66c/apply_checkpoint_66c.ps1' -and @($pre.powerShellPaths) -contains 'tools/checkpoints/checkpoint-66c/test_checkpoint_66c_contract.ps1') 'CP66c dependency precheck must inspect apply/contract/shared harness.'
    Assert-True (@($pre.checkpointDefinitionPaths) -contains 'tools/calibration/checkpoints/checkpoint-66c.json' -and @($pre.checkpointDefinitionPaths) -contains 'tools/calibration/checkpoints/checkpoint-66c-deep-calibration.json') 'CP66c dependency precheck must inspect normal and deep definitions.'
}

$applySource = Get-Content -LiteralPath (Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-66c\apply_checkpoint_66c.ps1') -Raw
...
Assert-True ($guardCallIndex -ge 0 -and $guardCallIndex -lt $contractCallIndex -and $contractCallIndex -lt $harnessCallIndex) 'CP66c apply script must run the native dependency precheck before contract and harness.'
Assert-True ($applySource -notmatch '\$LASTEXITCODE') 'CP66c apply script must not read LASTEXITCODE.'
...
$contractSource = Get-Content -LiteralPath (Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-66c\test_checkpoint_66c_contract.ps1') -Raw
Assert-True ($contractSource -notmatch '\$_\.key') 'CP66c contract must not read the nonexistent numerical-baseline .key property.'
Assert-True ($contractSource -match 'parameter_id' -and $contractSource -match 'numericColumns') 'CP66c contract must validate the numerical-baseline parameter_id/value schema before row lookup.'
...
Assert-True ($validationFiles.Count -eq 1 -and $validationFiles[0].Name -eq 'Checkpoint_66c_Technology_Profile_Baseline_Binding_Hotfix.md') 'Exactly one CP66c active validation runbook must remain.'
$rootTxt = @(Get-ChildItem -LiteralPath $repositoryRoot -File -Filter '*.txt')
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_66C_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_66C_SHA256SUMS.txt as .txt.'
...
Write-Host '       Checkpoint 66c native dependency contract: shared harness + checkpoint guard reject Python runtime dependencies for CP66+.'
...
Write-Host '       Hotfix: CP66 uses the active v0.2 numerical baseline directly for tl1-production; stale frozen technology-catalog binding is removed and preflighted.'
