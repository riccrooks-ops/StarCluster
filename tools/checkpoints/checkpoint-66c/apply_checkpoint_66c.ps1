...
$contractCheck = Join-Path $PSScriptRoot 'test_checkpoint_66c_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-66c-deep-calibration.json' } else { 'checkpoint-66c.json' }
$definition = Join-Path $repositoryRoot (Join-Path 'tools\calibration\checkpoints' $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-66c.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-66c-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!66C(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!66C(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!66c(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_66c_Technology_Profile_Baseline_Binding_Hotfix.md') {
            Remove-Item -LiteralPath $file.FullName -Force
        }
    }
}
