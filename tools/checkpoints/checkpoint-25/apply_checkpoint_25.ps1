...
function Test-NormalizedRelativePathCompatibility {
    $testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-25-relative-path-test-{0}" -f ([guid]::NewGuid().ToString('N')))
...
function Ensure-ActiveConceptDocument {
    ...
    if ($activeConcepts.Count -ne 1 -or $activeConcepts[0].Name -ne $ExpectedFileName) {
        ...
        throw "Exactly the Checkpoint 25 concept document must remain active. Found: $activeNames"
    }
    Write-Host "       Active concept document: $ExpectedFileName"
}

function Test-ActiveConceptDocumentNormalization {
    $testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-25-concept-test-{0}" -f ([guid]::NewGuid().ToString('N')))
    $docsDirectory = Join-Path $testRoot 'docs'
    $archiveDirectory = Join-Path $docsDirectory 'archive'
    $expectedName = 'Star_Cluster_Game_Concept_v0.3w.docx'
...
function Assert-RepositoryManifest {
    ...
    foreach ($line in Get-Content -LiteralPath $ManifestPath) {
        ...
        if ($relativePath -eq 'CHECKPOINT_25_SHA256SUMS.txt') {
    ...
    if ($verified -ne 544) { throw "Repository manifest verified $verified files; the Checkpoint 25 archive requires exactly 544 files." }
...
function Ensure-ActiveValidationRunbook {
    ...
    if ($activeRunbooks.Count -ne 1 -or $activeRunbooks[0].Name -ne $ExpectedFileName) {
        ...
        throw "Exactly the Checkpoint 25 validation runbook must remain active. Found: $activeNames"
    }
    Write-Host "       Active validation runbook: $ExpectedFileName"
}

function Test-ActiveValidationRunbookNormalization {
    $testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-25-runbook-test-{0}" -f ([guid]::NewGuid().ToString('N')))
    $validationDirectory = Join-Path $testRoot 'validation'
    $archiveDirectory = Join-Path $validationDirectory 'archive'
    $expectedName = 'Checkpoint_25_TL1_Deterministic_Layered_Damage_Power_And_Phase_A_Scenario_Foundation.md'
...
function Assert-TechnologyCsvContracts {
    ...
    if ($recon.Count -ne 32) { throw "Expected 32 Checkpoint 25 reconciliation rows; found $($recon.Count)." }
    if (($recon[0].PSObject.Properties.Name -join ',') -ne 'topic,current_direction,reference_influence,checkpoint_25_decision,status,next_action') { throw 'Checkpoint 25 reconciliation header is invalid.' }
    ...
    if ($baselineHash -ne '50316e0528f5e80a16957017ecf407ce4655c40d57dc9e077d09d0d86e19bd7a') { throw "TL1 baseline SHA-256 is $baselineHash, not the accepted Checkpoint 25 hash." }
    $requiredValues = @{
    ...
    Write-Host "       Technology/test data: 99 components, 11 profiles, 13 references, 40 insights, 32 reconciliations, 117 baseline values, 13 loadouts, 62 scenarios."
}
...
function Assert-Tl1ScenarioCorpusContracts {
    ...
    $expectedHash = '50316e0528f5e80a16957017ecf407ce4655c40d57dc9e077d09d0d86e19bd7a'
    ...
    if ($implemented.Count -ne 12) { throw "Expected 12 implemented Checkpoint 25 matrix rows; found $($implemented.Count)." }
...
function Assert-WorkbookContract {
    $path = '.\docs\design\player_technology\StarCluster_Player_TL_Framework_Draft_v0_5.xlsx'
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw 'Technology workbook v0.5 was not found.' }
    Add-Type -AssemblyName System.IO.Compression.FileSystem
    $zip = [System.IO.Compression.ZipFile]::OpenRead((Resolve-Path -LiteralPath $path).Path)
    try {
        ...
        foreach ($sheetName in @('Overview','TL1 Baseline','TL1 Loadouts','TL1 Test Matrix','Phase A Runtime','Component Schema','Checkpoint 25 Plan','TL Matrix','Components','Compatibility Profiles','Adaptation Rules','Reference Library','Reference Insights','Design Reconciliation','Level Themes','Design Decisions','Sources Used')) {
        ...
        foreach ($marker in @('D-240','Phase A Runtime','12 executable Phase A documents with 54 cases','50316e0528f5e80a16957017ecf407ce4655c40d57dc9e077d09d0d86e19bd7a')) {
            if (-not $allXml.Contains($marker)) { throw "Workbook does not contain required Checkpoint 25 marker $marker." }
        }
    }
    finally { $zip.Dispose() }
    Write-Host '       Workbook OOXML: Checkpoint 25 sheets, Phase A runtime, and Decision D-240 present; no structured table parts.'
}

function Assert-NewUnitTestContracts {
    $testFiles = @(
        '.\tests\StarCluster.Tests\Combat\Damage\LayeredDamageResolverTests.cs',
        '.\tests\StarCluster.Tests\Combat\Damage\ShieldRechargeServiceTests.cs',
        '.\tests\StarCluster.Tests\Combat\Power\TacticalPowerLedgerTests.cs',
        '.\tests\StarCluster.Tests\Combat\Power\ReactorStateTests.cs',
        '.\tests\StarCluster.Tests\Combat\Weapons\WeaponStateTests.cs'
    )
    $facts = 0
    foreach ($path in $testFiles) {
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Checkpoint 25 unit-test file $path was not found." }
        $facts += @(Select-String -LiteralPath $path -SimpleMatch '[Fact]').Count
    }
    if ($facts -ne 56) { throw "Expected 56 new Checkpoint 25 [Fact] tests; found $facts." }
    Write-Host '       Checkpoint 25 unit-test source: 56 focused facts present.'
}
...
try {
    Write-Host '[1/8] Verifying complete Checkpoint 25 repository and parser contracts...'
    foreach ($requiredFile in @(
        '.\StarCluster.sln',
        '.\global.json',
        '.\CHECKPOINT_25_SHA256SUMS.txt',
        ...
        '.\tests\StarCluster.Tests\Combat\Power\TacticalPowerLedgerTests.cs',
        '.\tests\StarCluster.Tests\Combat\Weapons\WeaponStateTests.cs',
        '.\docs\Star_Cluster_Game_Concept_v0.3w.docx',
        ...
        '.\docs\checkpoints\Checkpoint_25_TL1_Deterministic_Layered_Damage_Power_And_Phase_A_Scenario_Foundation.md',
        '.\docs\validation\Checkpoint_25_TL1_Deterministic_Layered_Damage_Power_And_Phase_A_Scenario_Foundation.md',
        '.\docs\validation\archive\Checkpoint_24_Unified_Tactical_Power_Component_State_And_TL1_Core_Combat_Test_Foundation.md')) {
        if (-not (Test-Path -LiteralPath $requiredFile -PathType Leaf)) { throw "Required Checkpoint 25 file $requiredFile was not found." }
    }
    Ensure-ActiveValidationRunbook -ValidationDirectory '.\docs\validation' -ExpectedFileName 'Checkpoint_25_TL1_Deterministic_Layered_Damage_Power_And_Phase_A_Scenario_Foundation.md' -ArchiveDirectory '.\docs\validation\archive'
    Ensure-ActiveConceptDocument -DocsDirectory '.\docs' -ExpectedFileName 'Star_Cluster_Game_Concept_v0.3w.docx' -ArchiveDirectory '.\docs\archive'
    Test-NormalizedRelativePathCompatibility
    Test-RepositoryManifestLocalArtifactPolicy
    Assert-RepositoryManifest '.\CHECKPOINT_25_SHA256SUMS.txt'
    Assert-PowerShellScriptsParse '.\tools'
    Test-ActiveValidationRunbookNormalization
    Test-ActiveConceptDocumentNormalization

    Write-Host '[2/8] Verifying Concept, reference library, exact data, schema, corpus, tests, and workbook contracts...'
    Assert-FileContains '.\README.md' @('Checkpoint 25','v0.3w.docx','54 deterministic cases','do not promote weapon balance') 'Repository README'
    Assert-FileContains '.\docs\checkpoints\Checkpoint_25_TL1_Deterministic_Layered_Damage_Power_And_Phase_A_Scenario_Foundation.md' @('SC25_DAMAGE_ORDER=SPEN_SA_SHIELD_APEN_AP_AI_AP_HULL','SC25_POWER_LEDGER=AVAILABLE_POWERED_SPENT_WITH_EARMARKS','SC25_SCENARIO_CORPUS=12_DOCUMENTS_54_CASES','SC25_BALANCE_CLAIM=NONE_PHASE_A_FIDELITY_ONLY') 'Checkpoint 25 documentation contract'
    ...
    Assert-FileContains '.\docs\Prototype_TODO.md' @('Checkpoint 25','562','12 executable Phase A documents','54 deterministic mechanics cases','Immediate next pass - Phase B') 'Prototype TODO'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\README.md' @('tl1-phase-a-single','tl1-phase-a-preflight','54 mechanics cases','No balance conclusions') 'ScenarioRunner README'
    Assert-DocxContains '.\docs\Star_Cluster_Game_Concept_v0.3w.docx' @('TL1 Phase A mechanics corpus','Declared baseline hash','Safe energy output mode','D-240','END OF DRAFT v0.3w') 'Concept v0.3w'
    ...
    if ($RepositoryContractOnly) {
        Write-Host ''
        Write-Host 'Checkpoint 25 repository-contract preflight completed successfully.'
        Write-Host 'Manifest, parser, normalization, reference, documentation, CSV, JSON corpus, schema, test-source, and workbook contracts passed.'
        return
    }

    Write-Host '[3/8] Confirming Godot is closed and .NET SDK 8.0.423 is selected...'
    ...
    if ($godotProcesses) {
        ...
        throw "Close Godot before applying Checkpoint 25. Running process(es): $processNames"
    }
    $sdkVersion = dotnet --version
    Write-Host "       SDK: $sdkVersion"
    if ($sdkVersion -ne '8.0.423') { throw "Expected .NET SDK 8.0.423 from global.json, but dotnet selected $sdkVersion." }

    Write-Host '[4/8] Performing a clean compiler preflight with warnings as errors...'
    ...
    Write-Host '[5/8] Running 562 engine-independent tests...'
    $testOutput = & dotnet test '.\tests\StarCluster.Tests\StarCluster.Tests.csproj' --no-build --nologo 2>&1
    $testOutput | Write-Host
    if ($LASTEXITCODE -ne 0) { throw "dotnet test failed with exit code $LASTEXITCODE." }
    if (($testOutput | Out-String) -notmatch 'Passed:\s+562') { throw 'The complete suite did not report the expected 562 passed tests.' }

    Write-Host '[6/8] Running seven accepted deterministic moving-missile scenarios...'
    $legacyOutput = '.\out\checkpoint-25-deterministic'
    Remove-Item $legacyOutput -Recurse -Force -ErrorAction SilentlyContinue
    $legacyText = Invoke-Runner -Arguments @('run-all','--scenario-dir','.\src\StarCluster.ScenarioRunner\Scenarios','--output-dir',$legacyOutput) -Description 'Checkpoint 25 legacy deterministic corpus'
    ...
    Write-Host '[7/8] Running 12 TL1 Phase A documents and 54 mechanics cases...'
    $phaseAOutput = '.\out\checkpoint-25-tl1-phase-a'
    Remove-Item $phaseAOutput -Recurse -Force -ErrorAction SilentlyContinue
    $phaseAText = Invoke-Runner -Arguments @('tl1-phase-a','--scenario-dir','.\src\StarCluster.ScenarioRunner\Scenarios\TL1PhaseA','--baseline-file','.\docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_1.csv','--output-dir',$phaseAOutput) -Description 'Checkpoint 25 TL1 Phase A corpus'
    if ($phaseAText -notmatch 'TL1 Phase A preflight:\s+12 scenario documents, 54 mechanics cases, baseline 117 values; passed\.' -or $phaseAText -notmatch 'TL1 Phase A:\s+12 passed, 0 failed, 12 scenarios; 54 passed, 0 failed, 54 cases\.') { throw 'The TL1 Phase A corpus did not report 12 documents and 54 passing cases.' }

    Write-Host '[8/8] Running forty-six ScenarioRunner self-tests...'
    $selfTestText = Invoke-Runner -Arguments @('self-test','--scenario-file','.\src\StarCluster.ScenarioRunner\Scenarios\terminal-two-window-hit.json') -Description 'Checkpoint 25 runner self-tests'
    ...
    Write-Host ''
    Write-Host 'Checkpoint 25 completed successfully.'
    Write-Host 'Engine-independent tests passed: 562.'
    ...
    Write-Host 'No Monte Carlo calibration or mechanical Godot validation is required.'
}
