...
try {
    ...
    foreach ($priorFile in $priorFiles) {
        if (-not (Test-Path $priorFile)) {
            throw "Required prior-checkpoint file $priorFile was not found. Apply Checkpoints 02 through 04 first."
        }
    }
    ...
    Write-Host '[3/6] Verifying the Checkpoint 05 system-map source files...'
    ...
    foreach ($requiredFile in $requiredFiles) {
        if (-not (Test-Path $requiredFile)) {
            throw "$requiredFile was not found. Re-extract the Checkpoint 05 package into the repository root."
        }
    }
    ...
    if (-not (Test-Path '.\docs\checkpoints\Checkpoint_05_System_Map_Contents.md')) {
        throw 'The Checkpoint 05 documentation file was not found.'
    }
    ...
    Write-Host ''
    Write-Host 'Checkpoint 05 completed successfully.' -ForegroundColor Green
}
finally {
    Pop-Location
}
