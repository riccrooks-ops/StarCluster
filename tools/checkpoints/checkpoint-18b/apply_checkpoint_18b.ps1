...
function Assert-FileContains {
    ...
    if (-not (Test-Path $Path)) {
        throw "$Description file $Path was not found. Re-extract the complete Checkpoint 18b package."
    }
...
try {
    Write-Host '[1/12] Verifying repository and accepted Checkpoint 18a baseline...'
    ...
    foreach ($baselineFile in @(
        ...
        '.\docs\checkpoints\Checkpoint_18a_Application_And_PDS_Regression_Hotfix.md',
        ...
        if (-not (Test-Path $baselineFile)) {
            throw "Required Checkpoint 18a baseline file $baselineFile was not found."
        }
    }
    ...
    if ($godotProcesses) {
        ...
        throw "Close Godot before applying Checkpoint 18b. Running process(es): $processNames"
    }
    ...
    Write-Host '[4/12] Verifying terminal-only standard PDS policy...'
    ...
    Assert-FileContains $interceptionContext @(
        ...
        'Held direct-fire weapons remain the ordinary transit interceptor.') 'Checkpoint 18b PDS policy'
    Assert-FileNotContains $interceptionContext @(
        'MissileInterceptionOpportunity.Transit or',
        'MissileInterceptionOpportunity.Stationary or') 'Checkpoint 18b standard-PDS opportunity filter'
    ...
    foreach ($initializationFile in @(
        ...
        if (-not (Test-Path $initializationFile)) {
            throw "Required Checkpoint 18b initialization file $initializationFile was not found."
        }
    }
    Assert-FileContains '.\src\StarCluster.Core\Simulation\ScenarioInitializationService.cs' @(
        ...
        'TrackUpdateTrigger.SystemEntry') 'Checkpoint 18b initialization contract'
    ...
    foreach ($runnerFile in @(
        ...
        if (-not (Test-Path $runnerFile)) {
            throw "Required Checkpoint 18b runner file $runnerFile was not found."
        }
    }
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\StarCluster.ScenarioRunner.csproj' @(
        '<TargetFramework>net8.0</TargetFramework>',
        '<ProjectReference Include="..\StarCluster.Core\StarCluster.Core.csproj" />') 'Checkpoint 18b runner project'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\ScenarioExecutor.cs' @(
        ...
        'MissileAutonomousGuidanceService.AdvanceOnePhase(') 'Checkpoint 18b Core-service execution'
    ...
    foreach ($testFile in @(
        ...
        if (-not (Test-Path $testFile)) {
            throw "Required Checkpoint 18b test/support file $testFile was not found."
        }
    }
    ...
    Assert-FileContains '.\tests\StarCluster.Tests\Combat\Missiles\TerminalPointDefensePolicyTests.cs' @(
        ...
        'StandardPointDefenseReceivesEntryAndPreAttackWindows') 'Checkpoint 18b PDS-policy tests'
    ...
    foreach ($documentationFile in @(
        ...
        '.\docs\validation\Checkpoint_18b_Headless_Scenario_Runner_And_Deterministic_Combat_Validation.md',
        ...
        if (-not (Test-Path $documentationFile)) {
            throw "Required Checkpoint 18b documentation file $documentationFile was not found."
        }
    }
    Assert-FileContains '.\docs\validation\Checkpoint_18b_Headless_Scenario_Runner_And_Deterministic_Combat_Validation.md' @(
        '506/506',
        '7/7',
        'No separate mechanical Godot test is required.') 'Checkpoint 18b active validation runbook'
    ...
    $scenarioOutputDirectory = '.\out\checkpoint-18b-scenarios'
    ...
    if ($activeValidationFiles.Count -ne 1 -or
        $activeValidationFiles[0].Name -ne 'Checkpoint_18b_Headless_Scenario_Runner_And_Deterministic_Combat_Validation.md') {
        throw 'Checkpoint 18b must leave exactly one active validation runbook.'
    }

    Write-Host ''
    Write-Host 'Checkpoint 18b completed successfully.'
