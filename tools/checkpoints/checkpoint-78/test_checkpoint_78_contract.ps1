...
$normalRel = 'tools/calibration/checkpoints/checkpoint-78.json'
$deepRel = 'tools/calibration/checkpoints/checkpoint-78-deep-calibration.json'
$guardedPs = @(
    'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1',
    'tools/checkpoints/checkpoint-78/apply_checkpoint_78.ps1',
    'tools/checkpoints/checkpoint-78/test_checkpoint_78_contract.ps1',
...
Write-Host '       Validating Checkpoint 78 definitions and unchanged workload accounting...'
$normal = Read-Json $normalRel
$deep = Read-Json $deepRel
Assert-True ([string]$normal.checkpointId -eq '78' -and [string]$deep.checkpointId -eq '78') 'Checkpoint 78 definition ID mismatch.'
Assert-True ([string]$normal.manifestFile -eq 'CHECKPOINT_78_SHA256SUMS.txt' -and [string]$deep.manifestFile -eq 'CHECKPOINT_78_SHA256SUMS.txt') 'Checkpoint 78 manifest binding mismatch.'
Assert-True ([string]$normal.outputRoot -eq 'out/checkpoint-78' -and [string]$deep.outputRoot -eq 'out/checkpoint-78-deep-calibration') 'Checkpoint 78 output-root binding mismatch.'
Assert-True ((@($normal.nativeDependencyPrecheck.powerShellPaths) -join '|') -eq ($guardedPs -join '|')) 'Checkpoint 78 normal native-dependency PowerShell path binding mismatch.'
Assert-True ((@($deep.nativeDependencyPrecheck.powerShellPaths) -join '|') -eq ($guardedPs -join '|')) 'Checkpoint 78 deep native-dependency PowerShell path binding mismatch.'
Assert-True ((@($normal.nativeDependencyPrecheck.checkpointDefinitionPaths) -join '|') -eq ($guardedDefs -join '|')) 'Checkpoint 78 normal native-dependency definition binding mismatch.'
Assert-True ((@($deep.nativeDependencyPrecheck.checkpointDefinitionPaths) -join '|') -eq ($guardedDefs -join '|')) 'Checkpoint 78 deep native-dependency definition binding mismatch.'
Assert-True (@($normal.stages).Count -eq 8 -and @($deep.stages).Count -eq 30) 'Checkpoint 78 stage-count mismatch.'
Assert-True ([int]$normal.checkpointMetrics.stageCount -eq 8 -and [int]$normal.checkpointMetrics.monteCarloVariantCount -eq 0 -and [int]$normal.checkpointMetrics.trialsAtDefault -eq 0) 'Checkpoint 78 normal suite must contain no Monte Carlo workload.'
Assert-True ([int]$deep.checkpointMetrics.stageCount -eq 30 -and [int]$deep.checkpointMetrics.monteCarloVariantCount -eq 1598 -and [int]$deep.checkpointMetrics.trialsAtDefault -eq 15980000) 'Checkpoint 78 Deep Calibration workload mismatch.'
...
Assert-True (($stageIds -join '|') -eq ($expectedStages -join '|')) 'Checkpoint 78 normal stage ordering mismatch.'
$normalSelfTest = @($normal.stages | Where-Object { [string]$_.id -eq 'runner-self-tests' })
Assert-True ($normalSelfTest.Count -eq 1 -and [int]$normalSelfTest[0].metrics.selfTestCount -eq 47) 'Checkpoint 78 ScenarioRunner self-test count mismatch.'
foreach ($definition in @($normal, $deep)) {
    foreach ($docPath in @($definition.documentation)) {
        Assert-True (Test-Path -LiteralPath (Join-Path $repositoryRoot ([string]$docPath)) -PathType Leaf) "Checkpoint 78 definition references missing documentation/input '$docPath'."
    }
}
...
Assert-True ((Get-BaselineValue 'ecm_max') -eq 1 -and (Get-BaselineValue 'eccm_max') -eq 1) 'Checkpoint 78 must not change TL1 normal ECM/ECCM ceilings.'
...
Assert-True (-not $gameMain.Contains('allowsApproximateTrackFire: true')) 'Checkpoint 78 must not enable degraded fire on the Godot production/demo main weapon.'
...
Assert-True (@($productionApproximateEnable).Count -eq 0) 'Checkpoint 78 must not enable degraded fire in production Core/Game weapon construction.'
...
Assert-True ($technologyReadme.Contains('Historical `Player_TL1_TL9_Technology_Architecture_*`') -and $technologyReadme.Contains('not current authority')) 'Historical technology tables are not clearly separated from Matrix v1 authority.'
...
Assert-True ($tacticalComputerArchitecture.Contains('Technology_Architecture_Matrix_v1.md') -and $tacticalComputerArchitecture.Contains('holds the TL1 **-25 percentage-point** degraded-fire rating into the first TL2 candidate')) 'Tactical Computer architecture is not synchronized with Matrix v1.'
...
Assert-True ($validationFiles.Count -eq 1 -and $validationFiles[0].Name -eq 'Checkpoint_78_Technology_Architecture_Matrix_And_Concept_Synchronization.md') 'Exactly one Checkpoint 78 active validation runbook must remain.'
$rootTxt = @(Get-ChildItem -LiteralPath $repositoryRoot -File -Filter '*.txt')
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_78_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_78_SHA256SUMS.txt as .txt.'
...
Write-Host 'Checkpoint 78 contract validation passed.'
