...
$contractCheck = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-78\test_checkpoint_78_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-78-deep-calibration.json' } else { 'checkpoint-78.json' }
$definition = Join-Path $repositoryRoot ('tools\calibration\checkpoints\' + $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-78.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-78-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!78(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!78(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!78(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_78_Technology_Architecture_Matrix_And_Concept_Synchronization.md') {
...
foreach ($concept in @(Get-ChildItem -LiteralPath $docsRoot -File -Filter 'Star_Cluster_Game_Concept_v*.docx')) {
    if ($concept.Name -ne 'Star_Cluster_Game_Concept_v0.6q.docx') {
...
Write-Host '[3/4] Verifying Checkpoint 78 Concept and Technology Architecture Matrix contracts...'
& $contractCheck
