...
$contractCheck = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-80\test_checkpoint_80_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-80-deep-calibration.json' } else { 'checkpoint-80.json' }
$definition = Join-Path $repositoryRoot ('tools\calibration\checkpoints\' + $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-80.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-80-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!80(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!80(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!80(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_80_TL2_EW_Power_Pressure_And_Tall_Viability.md') {
...
foreach ($concept in @(Get-ChildItem -LiteralPath $docsRoot -File -Filter 'Star_Cluster_Game_Concept_v*.docx')) {
    if ($concept.Name -ne 'Star_Cluster_Game_Concept_v0.6r.docx') {
...
Write-Host '[3/4] Verifying Checkpoint 80 TL2 EW power-pressure and tall-viability contracts...'
& $contractCheck
