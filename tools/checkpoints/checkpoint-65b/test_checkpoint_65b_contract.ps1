...
$active = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-65b.json')
$deep = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-65b-deep-calibration.json')

Assert-True ([string]$active.checkpointId -eq '65b' -and [string]$deep.checkpointId -eq '65b') 'Checkpoint 65b definitions must identify checkpoint 65b.'
Assert-True ([string]$active.manifestFile -eq 'CHECKPOINT_65B_SHA256SUMS.txt' -and [string]$deep.manifestFile -eq 'CHECKPOINT_65B_SHA256SUMS.txt') 'Checkpoint 65b definitions must bind CHECKPOINT_65B_SHA256SUMS.txt.'
Assert-True ([string]$active.outputRoot -eq 'out/checkpoint-65b' -and [string]$deep.outputRoot -eq 'out/checkpoint-65b-deep-calibration') 'Checkpoint 65b output roots must be checkpoint-specific.'
Assert-True (@($active.documentation) -contains 'docs/validation/Checkpoint_65b_Native_Preflight_Dependency_Hotfix.md') 'Checkpoint 65b definition must reference the active runbook.'
...
$applyScript = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-65b\apply_checkpoint_65b.ps1'
Assert-True (Test-Path -LiteralPath $applyScript -PathType Leaf) 'Checkpoint 65b apply script is missing.'
$applySource = Get-Content -LiteralPath $applyScript -Raw
Assert-True ($applySource -match '&\s+\$contractCheck' -and $applySource -match '&\s+\$harness') 'Checkpoint 65b apply script must invoke the PowerShell contract and shared harness.'
Assert-True ($applySource -notmatch '(?im)^\s*(python|python3|py)(?:\.exe)?\b') 'Checkpoint 65b native acceptance must not require Python.'
$harnessSource = Get-Content -LiteralPath (Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1') -Raw
Assert-True ($harnessSource -notmatch '(?im)^\s*(python|python3|py)(?:\.exe)?\b') 'Shared checkpoint harness must not require Python.'

$validationFiles = @(Get-ChildItem -LiteralPath (Join-Path $repositoryRoot 'docs\validation') -File -Filter '*.md')
Assert-True ($validationFiles.Count -eq 1 -and $validationFiles[0].Name -eq 'Checkpoint_65b_Native_Preflight_Dependency_Hotfix.md') 'Exactly one Checkpoint 65b active validation runbook must remain.'
$rootTxt = @(Get-ChildItem -LiteralPath $repositoryRoot -File -Filter '*.txt')
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_65B_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_65B_SHA256SUMS.txt as a .txt file.'
...
Write-Host '       Checkpoint 65b design contract: balanced TL1 cruisers on the radius-5 map, 3x3 weapon families, 3 bilateral regimes, 2 movement orders = 54 variants.'
