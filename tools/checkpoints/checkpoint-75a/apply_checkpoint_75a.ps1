...
$contractCheck = Join-Path $PSScriptRoot 'test_checkpoint_75a_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-75a-deep-calibration.json' } else { 'checkpoint-75a.json' }
$definition = Join-Path $repositoryRoot (Join-Path 'tools\calibration\checkpoints' $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-75a.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-75a-deep-calibration.json'
...
# root-level generated checkpoint text artifacts that cannot coexist with CP75a.
$staleRootPatterns = @(
    '^CHECKPOINT_(?!75A(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!75A(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!75a(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_75a_Release_Gate_Block_Isolation_Hotfix.md') {
...
Write-Host '[3/4] Verifying Checkpoint 75a repository and frozen CP75 contracts...'
& $contractCheck
