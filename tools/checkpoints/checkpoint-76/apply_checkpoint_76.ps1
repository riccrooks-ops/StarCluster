...
$contractCheck = Join-Path $PSScriptRoot 'test_checkpoint_76_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-76-deep-calibration.json' } else { 'checkpoint-76.json' }
$definition = Join-Path $repositoryRoot (Join-Path 'tools\calibration\checkpoints' $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-76.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-76-deep-calibration.json'
...
# root-level generated checkpoint text artifacts that cannot coexist with CP76.
$staleRootPatterns = @(
    '^CHECKPOINT_(?!76(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!76(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!76(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_76_Degraded_Fire_ECCM_Value_Counterplay_And_Upgrade_Path_Guardrails.md') {
...
# Likewise keep only Concept v0.6o active under docs/. Older active Concepts are
...
foreach ($concept in @(Get-ChildItem -LiteralPath $docsRoot -File -Filter 'Star_Cluster_Game_Concept_v*.docx')) {
    if ($concept.Name -ne 'Star_Cluster_Game_Concept_v0.6o.docx') {
...
Write-Host '[3/4] Verifying Checkpoint 76 repository and CP76 contracts...'
& $contractCheck
