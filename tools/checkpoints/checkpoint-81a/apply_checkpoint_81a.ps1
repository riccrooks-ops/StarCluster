...
$contractCheck = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-81a\test_checkpoint_81a_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-81a-deep-calibration.json' } else { 'checkpoint-81a.json' }
$definition = Join-Path $repositoryRoot ('tools\calibration\checkpoints\' + $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-81a.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-81a-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!81a(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!81a(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!81a(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_81a_Reactive_EW_Study_Classification_Hotfix.md') {
...
Write-Host '[3/4] Verifying Checkpoint 81a reactive-EW study-classification contracts...'
& $contractCheck
