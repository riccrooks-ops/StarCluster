...
$contract = Join-Path $PSScriptRoot 'test_checkpoint_97_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definition = Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-97.json'
...
& $guard -RepositoryRoot $repositoryRoot -PowerShellPaths @(
    ...
    'tools/calibration/checkpoints/checkpoint-97.json',
    'tools/calibration/checkpoints/checkpoint-97-deep-calibration.json'
)
...
Write-Host '[3/4] Verifying Checkpoint 97 Encounter / Adaptive Engage AI contracts...'
