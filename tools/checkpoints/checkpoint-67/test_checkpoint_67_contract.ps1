...
$normalRel = 'tools/calibration/checkpoints/checkpoint-67.json'
$deepRel = 'tools/calibration/checkpoints/checkpoint-67-deep-calibration.json'
$powerShellPaths = @(
    'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1',
    'tools/checkpoints/checkpoint-67/apply_checkpoint_67.ps1',
    'tools/checkpoints/checkpoint-67/test_checkpoint_67_contract.ps1',
...
Assert-True ([string]$normal.checkpointId -eq '67' -and [string]$deep.checkpointId -eq '67') 'Checkpoint definitions must identify 67.'
Assert-True ([string]$normal.manifestFile -eq 'CHECKPOINT_67_SHA256SUMS.txt' -and [string]$deep.manifestFile -eq 'CHECKPOINT_67_SHA256SUMS.txt') 'Checkpoint 67 manifest binding mismatch.'
Assert-True ([string]$normal.outputRoot -eq 'out/checkpoint-67' -and [string]$deep.outputRoot -eq 'out/checkpoint-67-deep-calibration') 'Checkpoint 67 output roots mismatch.'
Assert-True ([int]$normal.checkpointMetrics.stageCount -eq 8 -and [int]$normal.checkpointMetrics.monteCarloVariantCount -eq 60 -and [int]$normal.checkpointMetrics.trialsAtDefault -eq 600000) 'Checkpoint 67 normal workload mismatch.'
Assert-True ([int]$deep.checkpointMetrics.stageCount -eq 26 -and [int]$deep.checkpointMetrics.monteCarloVariantCount -eq 1544 -and [int]$deep.checkpointMetrics.trialsAtDefault -eq 15440000) 'Checkpoint 67 Deep workload mismatch.'
Assert-True ([string]$normal.primaryStudy.id -eq 'tl1-bilateral-overload-ew-counterplay' -and [int]$normal.primaryStudy.variantCount -eq 60) 'Checkpoint 67 normal primaryStudy metadata mismatch.'
Assert-True ([string]$deep.primaryStudy.id -eq 'tl1-bilateral-overload-ew-counterplay' -and [int]$deep.primaryStudy.variantCount -eq 60) 'Checkpoint 67 Deep primaryStudy metadata mismatch.'
Assert-True (@($policy.mustAlwaysRunStageIds).Count -eq 8 -and @($policy.deepCalibrationStageIds).Count -eq 18) 'Checkpoint 67 validation-tier policy count mismatch.'
...
Assert-True (($normalMcStages | ForEach-Object { [int]$_.metrics.variantCount } | Measure-Object -Sum).Sum -eq 60) 'Checkpoint 67 normal Monte Carlo stage total must recompute to 60 variants.'
Assert-True (($deepMcStages | ForEach-Object { [int]$_.metrics.variantCount } | Measure-Object -Sum).Sum -eq 1544) 'Checkpoint 67 Deep Monte Carlo stage total must recompute to 1,544 variants.'
...
Assert-True ($normalPrimaryStage.Count -eq 1 -and $deepPrimaryStage.Count -eq 1) 'Checkpoint 67 primary study stage must exist exactly once in normal and Deep definitions.'
...
foreach ($stage in @($normalPrimaryStage[0],$deepPrimaryStage[0])) {
    ...
    Assert-True ($studyArg -ge 0 -and $studyArg + 1 -lt $args.Count -and [string]$args[$studyArg + 1] -eq $expectedStudyRel) 'Checkpoint 67 stage must bind the exact CP67 study file.'
    Assert-True ($baselineArg -ge 0 -and $baselineArg + 1 -lt $args.Count -and [string]$args[$baselineArg + 1] -eq $expectedBaselineRel) 'Checkpoint 67 stage must bind the exact v0.3 numerical baseline.'
    Assert-True ([int]$stage.metrics.variantCount -eq 60 -and [int]$stage.metrics.startingFuel -eq 100) 'Checkpoint 67 stage metrics must retain 60 variants and the 100-fuel baseline.'
}
foreach ($definition in @($normal,$deep)) {
    $pre = $definition.nativeDependencyPrecheck
    Assert-True ([bool]$pre.required -and [string]$pre.script -eq 'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1') 'Checkpoint 67 must require the shared no-Python dependency precheck.'
    Assert-True (@($pre.powerShellPaths) -contains 'tools/checkpoints/checkpoint-67/apply_checkpoint_67.ps1' -and @($pre.powerShellPaths) -contains 'tools/checkpoints/checkpoint-67/test_checkpoint_67_contract.ps1' -and @($pre.powerShellPaths) -contains 'tools/calibration/run_calibration_checkpoint.ps1') 'Checkpoint 67 dependency precheck path coverage mismatch.'
}
...
Assert-True ([int](Get-RequiredParameterRow $numericRows 'fuel_capacity').value -eq 100) 'Authoritative Checkpoint 67 fuel capacity must be 100.'
...
Assert-True ([string]$study.baselineSha256 -eq 'd3020245a1fe2d28f6795c96c3c331d905ed02ef5210f3ab661e3475f70cf5be') 'Checkpoint 67 study must bind numerical baseline v0.3 exactly.'
...
Assert-True ([string]$study.id -eq 'tl1-itc10-bilateral-overload-ew-counterplay' -and @($study.variants).Count -eq 60) 'Checkpoint 67 study identity/count mismatch.'
Assert-True (@($study.variants | Select-Object -ExpandProperty id -Unique).Count -eq 60) 'Checkpoint 67 variant IDs must be unique.'
Assert-True (@($study.builds).Count -eq 2) 'Checkpoint 67 requires exactly two construction fixtures.'
...
Assert-True (@($study.variants | Where-Object { [string]$_.sideAProfileId -ne 'tl1-production' -or [string]$_.sideBProfileId -ne 'tl1-production' }).Count -eq 0) 'Checkpoint 67 must use tl1-production on both sides.'
Assert-True (-not ($study.PSObject.Properties.Name -contains 'technologyProfileCatalog')) 'Checkpoint 67 must not reintroduce the stale historical technology-profile catalog binding.'
...
Assert-True ($stlVariants.Count -eq 48 -and $ewVariants.Count -eq 12) 'Checkpoint 67 48+12 study split mismatch.'
...
Assert-True (-not [string]::IsNullOrWhiteSpace($auxCatalogRel)) 'Checkpoint 67 study must name its auxiliary runtime-profile catalog.'
$auxCatalogPath = Join-Path $repositoryRoot $auxCatalogRel
Assert-True (Test-Path -LiteralPath $auxCatalogPath -PathType Leaf) 'Checkpoint 67 auxiliary runtime-profile catalog is missing.'
...
foreach ($fragment in $requiredRunnerFragments) {
    Assert-True ($runner.Contains($fragment)) "Checkpoint 67 cross-study integration missing runner fragment: $fragment"
}
...
$applySource = Get-Content -LiteralPath (Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-67\apply_checkpoint_67.ps1') -Raw
...
Assert-True ($guardCallIndex -ge 0 -and $guardCallIndex -lt $contractCallIndex -and $contractCallIndex -lt $harnessCallIndex) 'Checkpoint 67 apply script must run dependency guard before contract and harness.'
Assert-True ($applySource -notmatch '\$LASTEXITCODE') 'Checkpoint 67 apply script must not read LASTEXITCODE.'
...
Assert-True ($validationFiles.Count -eq 1 -and $validationFiles[0].Name -eq 'Checkpoint_67_Bilateral_Overload_EW_Counterplay_And_100_Fuel.md') 'Exactly one Checkpoint 67 active validation runbook must remain.'
$rootTxt = @(Get-ChildItem -LiteralPath $repositoryRoot -File -Filter '*.txt')
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_67_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_67_SHA256SUMS.txt as .txt.'
...
Write-Host '       Checkpoint 67 native dependency contract: PowerShell + pinned .NET only; active Python runtime dependencies are rejected before native work.'
...
Write-Host 'Checkpoint 67 contract validation passed.'
