...
$normalRel = 'tools/calibration/checkpoints/checkpoint-81.json'
$deepRel = 'tools/calibration/checkpoints/checkpoint-81-deep-calibration.json'
$guardedPs = @(
...
Write-Host '       Validating Checkpoint 81 definitions and workload accounting...'
$normal = Read-Json $normalRel
$deep = Read-Json $deepRel
Assert-True ([string]$normal.checkpointId -eq '81' -and [string]$deep.checkpointId -eq '81') 'Checkpoint 81 definition ID mismatch.'
Assert-True ([string]$normal.manifestFile -eq 'CHECKPOINT_81_SHA256SUMS.txt' -and [string]$deep.manifestFile -eq 'CHECKPOINT_81_SHA256SUMS.txt') 'Checkpoint 81 manifest binding mismatch.'
Assert-True ([string]$normal.outputRoot -eq 'out/checkpoint-81' -and [string]$deep.outputRoot -eq 'out/checkpoint-81-deep-calibration') 'Checkpoint 81 output-root binding mismatch.'
Assert-True ((@($normal.nativeDependencyPrecheck.powerShellPaths) -join '|') -eq ($guardedPs -join '|')) 'Checkpoint 81 normal native-dependency PowerShell path binding mismatch.'
Assert-True ((@($deep.nativeDependencyPrecheck.powerShellPaths) -join '|') -eq ($guardedPs -join '|')) 'Checkpoint 81 deep native-dependency PowerShell path binding mismatch.'
Assert-True ((@($normal.nativeDependencyPrecheck.checkpointDefinitionPaths) -join '|') -eq ($guardedDefs -join '|')) 'Checkpoint 81 normal native-dependency definition binding mismatch.'
Assert-True ((@($deep.nativeDependencyPrecheck.checkpointDefinitionPaths) -join '|') -eq ($guardedDefs -join '|')) 'Checkpoint 81 deep native-dependency definition binding mismatch.'
Assert-True (@($normal.stages).Count -eq 11 -and [int]$normal.checkpointMetrics.stageCount -eq 11) 'Checkpoint 81 normal stage-count mismatch.'
Assert-True ([int]$normal.checkpointMetrics.monteCarloVariantCount -eq 96 -and [int]$normal.checkpointMetrics.trialsAtDefault -eq 960000 -and [int]$normal.checkpointMetrics.smokeVariantExecutions -eq 96 -and [int]$normal.checkpointMetrics.smokeTrialsAtDefault -eq 96 -and [int]$normal.checkpointMetrics.totalTrialExecutionsAtDefault -eq 960096) 'Checkpoint 81 normal workload accounting mismatch.'
Assert-True (@($deep.stages).Count -eq 33 -and [int]$deep.checkpointMetrics.stageCount -eq 33 -and [int]$deep.checkpointMetrics.monteCarloVariantCount -eq 1694 -and [int]$deep.checkpointMetrics.trialsAtDefault -eq 16940000 -and [int]$deep.checkpointMetrics.smokeVariantExecutions -eq 150 -and [int]$deep.checkpointMetrics.smokeTrialsAtDefault -eq 150 -and [int]$deep.checkpointMetrics.totalTrialExecutionsAtDefault -eq 16940150) 'Checkpoint 81 Deep Calibration workload accounting mismatch.'
...
Assert-True ((@($normal.stages | ForEach-Object { [string]$_.id }) -join '|') -eq ($expectedStages -join '|')) 'Checkpoint 81 normal stage ordering mismatch.'
...
Assert-True ($normalSelfTestStages.Count -eq 1 -and [int]$normalSelfTestStages[0].metrics.selfTestCount -eq 48 -and $deepSelfTestStages.Count -eq 1 -and [int]$deepSelfTestStages[0].metrics.selfTestCount -eq 48) 'Checkpoint 81 ScenarioRunner self-test workload must declare 48 tests in normal and Deep Calibration definitions.'
Assert-True ([string]$normal.primaryStudy.id -eq 'tl2-itc08-tactical-computer-ew-integration-permutations' -and [int]$normal.primaryStudy.variantCount -eq 96) 'Checkpoint 81 primary-study binding mismatch.'
Assert-True ([string]$deep.primaryStudy.id -eq 'tl2-itc08-tactical-computer-ew-integration-permutations' -and [int]$deep.primaryStudy.variantCount -eq 96) 'Checkpoint 81 Deep Calibration primary-study binding mismatch.'
...
Assert-True ($validationFiles.Count -eq 1 -and $validationFiles[0].Name -eq 'Checkpoint_81_TL2_Tactical_Computer_EW_Integration_Permutation_Suite.md') 'Exactly one CP81 active validation runbook must remain.'
...
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_81_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_81_SHA256SUMS.txt as .txt.'

Write-Host '       CP81 isolation: +10/+12 Tactical Computer candidate paired across four EW packages at fixed 5 TP; -25 degraded fire and Evasive Compensation 0 held.'
Write-Host '       Normal workload: 11 stages / 96 substantive variants / 960,000 default substantive trials plus 96 smoke trials.'
Write-Host 'Checkpoint 81 contract validation passed.'
