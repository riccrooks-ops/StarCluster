...
$normalRel = 'tools/calibration/checkpoints/checkpoint-79a.json'
$deepRel = 'tools/calibration/checkpoints/checkpoint-79a-deep-calibration.json'
$guardedPs = @(
    'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1',
    'tools/checkpoints/checkpoint-79a/apply_checkpoint_79a.ps1',
    'tools/checkpoints/checkpoint-79a/test_checkpoint_79a_contract.ps1',
...
Write-Host '       Validating Checkpoint 79a definitions and workload accounting...'
$normal = Read-Json $normalRel
$deep = Read-Json $deepRel
Assert-True ([string]$normal.checkpointId -eq '79a' -and [string]$deep.checkpointId -eq '79a') 'Checkpoint 79a definition ID mismatch.'
Assert-True ([string]$normal.manifestFile -eq 'CHECKPOINT_79a_SHA256SUMS.txt' -and [string]$deep.manifestFile -eq 'CHECKPOINT_79a_SHA256SUMS.txt') 'Checkpoint 79a manifest binding mismatch.'
Assert-True ([string]$normal.outputRoot -eq 'out/checkpoint-79a' -and [string]$deep.outputRoot -eq 'out/checkpoint-79a-deep-calibration') 'Checkpoint 79a output-root binding mismatch.'
Assert-True ((@($normal.nativeDependencyPrecheck.powerShellPaths) -join '|') -eq ($guardedPs -join '|')) 'Checkpoint 79a normal native-dependency PowerShell path binding mismatch.'
Assert-True ((@($deep.nativeDependencyPrecheck.powerShellPaths) -join '|') -eq ($guardedPs -join '|')) 'Checkpoint 79a deep native-dependency PowerShell path binding mismatch.'
Assert-True ((@($normal.nativeDependencyPrecheck.checkpointDefinitionPaths) -join '|') -eq ($guardedDefs -join '|')) 'Checkpoint 79a normal native-dependency definition binding mismatch.'
Assert-True ((@($deep.nativeDependencyPrecheck.checkpointDefinitionPaths) -join '|') -eq ($guardedDefs -join '|')) 'Checkpoint 79a deep native-dependency definition binding mismatch.'
Assert-True (@($normal.stages).Count -eq 11 -and [int]$normal.checkpointMetrics.stageCount -eq 11) 'Checkpoint 79a normal stage-count mismatch.'
Assert-True ([int]$normal.checkpointMetrics.monteCarloVariantCount -eq 54 -and [int]$normal.checkpointMetrics.trialsAtDefault -eq 540000 -and [int]$normal.checkpointMetrics.smokeTrialsAtDefault -eq 54 -and [int]$normal.checkpointMetrics.totalTrialExecutionsAtDefault -eq 540054) 'Checkpoint 79a normal workload accounting mismatch.'
Assert-True (@($deep.stages).Count -eq 33 -and [int]$deep.checkpointMetrics.stageCount -eq 33 -and [int]$deep.checkpointMetrics.monteCarloVariantCount -eq 1652 -and [int]$deep.checkpointMetrics.trialsAtDefault -eq 16520000 -and [int]$deep.checkpointMetrics.smokeTrialsAtDefault -eq 108) 'Checkpoint 79a Deep Calibration workload accounting mismatch.'
...
Assert-True (($stageIds -join '|') -eq ($expectedStages -join '|')) 'Checkpoint 79a normal stage ordering mismatch.'
$primary = $normal.primaryStudy
Assert-True ([string]$primary.id -eq 'tl2-itc06-sensor-ew-discrimination-isolation' -and [int]$primary.variantCount -eq 54) 'Checkpoint 79a primary-study binding mismatch.'
...
Assert-True ($preflightStage.Count -eq 1 -and [string]$preflightStage[0].command -eq 'tl1-integrated-tactical-combat-preflight' -and [bool]$preflightStage[0].metrics.actualConsumerDeserializer -and [bool]$preflightStage[0].metrics.perSideSensorProfiles -and [bool]$preflightStage[0].metrics.normalEwRatingOverride) 'Checkpoint 79a actual-consumer preflight stage mismatch.'
Assert-True ($smokeStage.Count -eq 1 -and [int]$smokeStage[0].metrics.totalSmokeTrials -eq 54 -and [bool]$smokeStage[0].metrics.fullPipelineExecution) 'Checkpoint 79a full-pipeline smoke stage mismatch.'
Assert-True ($primaryStage.Count -eq 1 -and [int]$primaryStage[0].metrics.variantCount -eq 54 -and -not [bool]$primaryStage[0].metrics.balanceTargetsBlocking -and -not [bool]$primaryStage[0].metrics.tl2ComputerTargetingCandidateApplied -and -not [bool]$primaryStage[0].metrics.sensorRangeChanged -and -not [bool]$primaryStage[0].metrics.sensorOverloadChanged -and -not [bool]$primaryStage[0].metrics.ewOverloadChanged) 'Checkpoint 79a primary-stage isolation metrics mismatch.'
$normalSelfTest = @($normal.stages | Where-Object { [string]$_.id -eq 'runner-self-tests' })
Assert-True ($normalSelfTest.Count -eq 1 -and [int]$normalSelfTest[0].metrics.selfTestCount -eq 47) 'Checkpoint 79a ScenarioRunner self-test count mismatch.'
foreach ($definition in @($normal, $deep)) {
    foreach ($docPath in @($definition.documentation)) {
        Assert-True (Test-Path -LiteralPath (Join-Path $repositoryRoot ([string]$docPath)) -PathType Leaf) "Checkpoint 79a definition references missing documentation/input '$docPath'."
    }
}
...
Assert-Sha256 'src/StarCluster.ScenarioRunner/Scenarios/TL1Calibration/tl2-itc06-sensor-ew-discrimination-isolation.json' 'fb42b496c33b8699391ad366d81a5202ec2f008f7c0ba3438d5cb9a4e4f461b3'
...
Assert-Sha256 'docs/design/player_technology/TL2_Sensor_EW_Discrimination_Isolation_Study_v0_1.md' 'e410b27ec05599ab509fd23bedd94f54f6cfc82b851dc4aca3688b0f64f61e03'
Assert-Sha256 'docs/design/testing/Checkpoint_79_Validation_Tiers.md' '78449d1eecf5de0ef9d6a138865c063ac074d860d888c8aef4647d72a3330e86'
Assert-Sha256 'docs/design/testing/checkpoint_79_validation_suite_policy_v0_1.json' 'af93a0766f449aa9ab6ae136c1bee628257c35378c901620760dad91e211a25e'
Assert-Sha256 'src/StarCluster.ScenarioRunner/TL1Calibration/Tl1IntegratedTacticalCombatDocuments.cs' '1f621592e39510869075eb3f23683cdac7187cfad83459c372a8fda4622f4203'

Write-Host '       Auditing CP79a actual-consumer runtime integration and shared policy telemetry...'
...
$cp79PolicyMentions = [regex]::Matches($policyGateText, 'Tl2SensorEwDiscriminationIsolationStudyId').Count
Assert-True ($cp79PolicyMentions -eq 2) 'CP79/79a must be classified exactly twice inside the shared policy-telemetry gate: once in the pass predicate and once in the diagnostic-message branch.'
Assert-True ($policyGateText.Contains('Sensor/EW')) 'The shared policy-telemetry diagnostic branch must identify Sensor/EW screening studies explicitly.'
...
Assert-True ($validationFiles.Count -eq 1 -and $validationFiles[0].Name -eq 'Checkpoint_79a_Policy_Telemetry_Study_Classification_Hotfix.md') 'Exactly one CP79a active validation runbook must remain.'
Assert-True (Test-Path -LiteralPath (Join-Path $repositoryRoot 'docs\validation\archive\Checkpoint_78a_Historical_Authority_Contract_Hotfix.md') -PathType Leaf) 'Checkpoint 78a validation runbook must remain archived during CP79a normalization.'
$rootTxt = @(Get-ChildItem -LiteralPath $repositoryRoot -File -Filter '*.txt')
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_79a_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_79a_SHA256SUMS.txt as .txt.'
...
Write-Host 'Checkpoint 79a hotfix contract validation passed.'
