...
$contractCheck = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-79a\test_checkpoint_79a_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-79a-deep-calibration.json' } else { 'checkpoint-79a.json' }
$definition = Join-Path $repositoryRoot ('tools\calibration\checkpoints\' + $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-79a.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-79a-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!79a(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!79a(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!79a(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_79a_Policy_Telemetry_Study_Classification_Hotfix.md') {
...
Write-Host '[3/4] Verifying Checkpoint 79a policy-telemetry classification hotfix contracts...'
& $contractCheck
