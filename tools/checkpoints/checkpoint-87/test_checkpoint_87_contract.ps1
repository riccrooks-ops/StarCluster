...
$normalRel = 'tools/calibration/checkpoints/checkpoint-87.json'
$deepRel = 'tools/calibration/checkpoints/checkpoint-87-deep-calibration.json'
$guardedPs = @(
    'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1',
    'tools/checkpoints/checkpoint-87/apply_checkpoint_87.ps1',
    'tools/checkpoints/checkpoint-87/test_checkpoint_87_contract.ps1',
...
Write-Host '       Validating Checkpoint 87 definitions and workload accounting...'
$normal = Read-Json $normalRel
$deep = Read-Json $deepRel
Assert-True ([string]$normal.checkpointId -eq '87' -and [string]$deep.checkpointId -eq '87') 'Checkpoint 87 definition ID mismatch.'
Assert-True ([string]$normal.manifestFile -eq 'CHECKPOINT_87_SHA256SUMS.txt' -and [string]$deep.manifestFile -eq 'CHECKPOINT_87_SHA256SUMS.txt') 'Checkpoint 87 manifest binding mismatch.'
Assert-True ([string]$normal.outputRoot -eq 'out/checkpoint-87' -and [string]$deep.outputRoot -eq 'out/checkpoint-87-deep-calibration') 'Checkpoint 87 output-root binding mismatch.'
Assert-True (@($normal.stages).Count -eq 13 -and [int]$normal.checkpointMetrics.stageCount -eq 13) 'Checkpoint 87 normal stage-count mismatch.'
Assert-True ([int]$normal.checkpointMetrics.monteCarloVariantCount -eq 192 -and [int]$normal.checkpointMetrics.trialsAtDefault -eq 1920000 -and [int]$normal.checkpointMetrics.smokeVariantExecutions -eq 192 -and [int]$normal.checkpointMetrics.smokeTrialsAtDefault -eq 192 -and [int]$normal.checkpointMetrics.totalTrialExecutionsAtDefault -eq 1920192) 'Checkpoint 87 normal workload mismatch.'
Assert-True (@($deep.stages).Count -eq 14 -and [int]$deep.checkpointMetrics.stageCount -eq 14) 'Checkpoint 87 Deep Calibration stage-count mismatch.'
Assert-True ([int]$deep.checkpointMetrics.monteCarloVariantCount -eq 480 -and [int]$deep.checkpointMetrics.trialsAtDefault -eq 4800000 -and [int]$deep.checkpointMetrics.smokeVariantExecutions -eq 192 -and [int]$deep.checkpointMetrics.totalTrialExecutionsAtDefault -eq 4800192) 'Checkpoint 87 Deep Calibration workload mismatch.'
Assert-True ([string]$normal.primaryStudy.id -eq 'tl2-itc13-cross-tl-build-permutation-screening' -and [int]$normal.primaryStudy.variantCount -eq 192) 'Checkpoint 87 primary-study metadata mismatch.'
$expectedNormal = @(
...
Assert-True (($normalIds -join '|') -eq ($expectedNormal -join '|')) 'Checkpoint 87 normal stage order/identity mismatch.'
$deepIds = @($deep.stages | ForEach-Object { [string]$_.id })
Assert-True (($deepIds -contains 'cp86-weapon-penetration-regression') -and $deepIds[-1] -eq 'runner-self-tests') 'Checkpoint 87 Deep Calibration must add the accepted CP86 penetration regression before final retained stages.'
...
Assert-True ($generationStage.Count -eq 1 -and [string]$generationStage[0].command -eq 'cross-tl-build-permutation') 'Checkpoint 87 is missing the native generator stage.'
Assert-True ($consumerPreflight.Count -eq 1 -and [string]$consumerPreflight[0].command -eq 'tl1-integrated-tactical-combat-preflight') 'Checkpoint 87 is missing generated-study actual-consumer preflight.'
Assert-True ($consumerSmoke.Count -eq 1 -and [string]$consumerSmoke[0].command -eq 'tl1-integrated-tactical-combat') 'Checkpoint 87 is missing generated-study full-pipeline smoke.'
Assert-True ($consumerStudy.Count -eq 1 -and [string]$consumerStudy[0].command -eq 'tl1-integrated-tactical-combat') 'Checkpoint 87 is missing the substantive generated-study consumer stage.'
...
Assert-True ($selfTestStage.Count -eq 1 -and [int]$selfTestStage[0].metrics.selfTestCount -eq 52) 'Checkpoint 87 must require 52 ScenarioRunner self-tests.'
...
Assert-True ([regex]::Matches($integrated,[regex]::Escape($studyId)).Count -ge 10) 'Integrated combat runner has incomplete CP87 study-ID integration.'
...
Require-Contains $rootReadme 'Checkpoint 87' 'Root README must identify Checkpoint 87.'
...
Require-Contains $todo 'Checkpoint 87' 'Prototype TODO must identify Checkpoint 87.'
...
Assert-True ($activeValidation.Count -eq 1 -and $activeValidation[0].Name -eq 'Checkpoint_87_Cross_TL_Legal_Build_Permutation_Foundation.md') 'Exactly one CP87 active validation runbook must remain.'
...
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_87_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_87_SHA256SUMS.txt as .txt.'

Write-Host '       CP87 foundation: 512 legal TL1/TL2 working-envelope builds; 64 ordered representative pairings; 192 generated integrated variants.'
...
Write-Host 'Checkpoint 87 contract validation passed.'
