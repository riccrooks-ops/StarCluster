...
$contractCheck = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-87\test_checkpoint_87_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-87-deep-calibration.json' } else { 'checkpoint-87.json' }
$definition = Join-Path $repositoryRoot ('tools\calibration\checkpoints\' + $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-87.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-87-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!87(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!87(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!87(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_87_Cross_TL_Legal_Build_Permutation_Foundation.md') {
...
Write-Host '[3/4] Verifying Checkpoint 87 cross-TL legal-build/permutation contracts...'
& $contractCheck
