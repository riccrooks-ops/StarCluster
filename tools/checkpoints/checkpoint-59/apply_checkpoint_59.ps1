...
$contractCheck = Join-Path $PSScriptRoot 'test_checkpoint_59_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-59-deep-calibration.json' } else { 'checkpoint-59.json' }
