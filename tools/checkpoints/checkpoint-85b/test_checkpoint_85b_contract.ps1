...
$normalRel='tools/calibration/checkpoints/checkpoint-85b.json'
$deepRel='tools/calibration/checkpoints/checkpoint-85b-deep-calibration.json'
$guardedPs=@('tools/checkpoints/Test-NativeAcceptanceDependencies.ps1','tools/checkpoints/checkpoint-85b/apply_checkpoint_85b.ps1','tools/checkpoints/checkpoint-85b/test_checkpoint_85b_contract.ps1','tools/calibration/run_calibration_checkpoint.ps1')
...
Write-Host '       Validating Checkpoint 85b definitions and unchanged CP85 workload accounting...'
$normal=Read-Json $normalRel; $deep=Read-Json $deepRel
Assert-True ([string]$normal.checkpointId -eq '85b' -and [string]$deep.checkpointId -eq '85b') 'Checkpoint 85b definition ID mismatch.'
Assert-True ([string]$normal.manifestFile -eq 'CHECKPOINT_85b_SHA256SUMS.txt' -and [string]$deep.manifestFile -eq 'CHECKPOINT_85b_SHA256SUMS.txt') 'Checkpoint 85b manifest binding mismatch.'
Assert-True ([string]$normal.outputRoot -eq 'out/checkpoint-85b' -and [string]$deep.outputRoot -eq 'out/checkpoint-85b-deep-calibration') 'Checkpoint 85b output-root binding mismatch.'
...
$requiredVariantProperties=@('id','comparisonGroup','profileLabel','sideAReactorOutputOverride','sideBReactorOutputOverride','sideBShieldCapacityOverride','sideBPrimaryArmorProtectionOverride','sideBPrimaryArmorIntegrityOverride','sideAShieldCapacityOverride','sideAPrimaryArmorProtectionOverride','sideAPrimaryArmorIntegrityOverride','sideAFamily','sideBFamily')
for ($variantIndex=0; $variantIndex -lt $variants.Count; $variantIndex++) { $v=$variants[$variantIndex]; foreach ($propertyName in $requiredVariantProperties) { Assert-True ($null -ne $v.PSObject.Properties[$propertyName]) "CP85 variant index $variantIndex is missing required property '$propertyName'." } }
Assert-True (@($variants | Where-Object {$null -ne $_.PSObject.Properties['label']}).Count -eq 0) 'CP85 schema drift: legacy label property must not replace profileLabel.'
    ...
    foreach ($env in @('firm-reference','tall-dr1-eccm1')) {
        foreach ($shield in @(2,3)) {
            foreach ($ap in @(0,1)) { foreach ($ai in @(4,5)) {
                ...
                $m=@($g.Group | Where-Object {[string]$_.profileLabel -eq $label -and [int]$_.sideAShieldCapacityOverride -eq $shield -and [int]$_.sideAPrimaryArmorProtectionOverride -eq $ap -and [int]$_.sideAPrimaryArmorIntegrityOverride -eq $ai})
...
Assert-True ($suite.Contains('Technology Integration Permutation Suite Architecture v0.5') -and $suite.Contains('Armor')) 'Standing suite v0.5 Armor architecture is missing.'
...
$cp84ManifestRecord=Read-Manifest $cp84ManifestRel
$cp84Manifest=$cp84ManifestRecord.Entries
...
Assert-True ([string]$prov.acceptanceSummary.checkpointManifestSha256 -eq '108579f8b097186bd4d00ea80482d7d9111109a1115795cc9f3605ab316145a0') 'CP84 provenance manifest SHA-256 is not the accepted CP84 value.'
Assert-True ((Hash-Rel $cp84ManifestRel) -eq [string]$prov.acceptanceSummary.checkpointManifestSha256) 'Embedded CP84 evidence manifest bytes do not match accepted native provenance.'
Assert-True ([int]$cp84ManifestRecord.PhysicalLineCount -eq 1622 -and [int]$cp84ManifestRecord.EntryCount -eq 1622) 'Accepted CP84 evidence manifest must contain exactly 1,622 unique entries.'
Write-Host ("       Accepted CP84 evidence manifest: {0} physical lines / {1} unique entries; SHA-256 matched provenance." -f $cp84ManifestRecord.PhysicalLineCount,$cp84ManifestRecord.EntryCount)
...
Assert-True ($activeValidation.Count -eq 1 -and $activeValidation[0].Name -eq 'Checkpoint_85b_CP85_Native_Contract_Provenance_Manifest_Hotfix.md') 'Exactly one CP85b active validation runbook must remain.'
...
Assert-True ($rootTxt.Count -eq 1 -and $rootTxt[0].Name -eq 'CHECKPOINT_85b_SHA256SUMS.txt') 'Repository root must contain only CHECKPOINT_85b_SHA256SUMS.txt as .txt.'
...
Write-Host 'Checkpoint 85b contract validation passed (CP85 study unchanged; profileLabel and CP84 provenance-manifest contracts corrected).'
