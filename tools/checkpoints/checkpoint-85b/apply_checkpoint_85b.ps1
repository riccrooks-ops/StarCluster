...
$contractCheck = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-85b\test_checkpoint_85b_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-85b-deep-calibration.json' } else { 'checkpoint-85b.json' }
$definition = Join-Path $repositoryRoot ('tools\calibration\checkpoints\' + $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-85b.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-85b-deep-calibration.json'
$guardedPowerShellPaths = @(
    'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1',
    'tools/checkpoints/checkpoint-85b/apply_checkpoint_85b.ps1',
    'tools/checkpoints/checkpoint-85b/test_checkpoint_85b_contract.ps1',
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!85b(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!85b(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!85b(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_85b_CP85_Native_Contract_Provenance_Manifest_Hotfix.md') {
...
Write-Host '[3/4] Verifying Checkpoint 85b CP85 native-contract/provenance hotfix and Armor study contracts...'
& $contractCheck
