...
function Test-NormalizedRelativePathCompatibility {
    $testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-31-relative-path-test-{0}" -f ([guid]::NewGuid().ToString('N')))
...
function Ensure-ActiveConceptDocument {
    ...
    if ($activeConcepts.Count -ne 1 -or $activeConcepts[0].Name -ne $ExpectedFileName) {
        ...
        throw "Exactly the Checkpoint 31 concept document must remain active. Found: $activeNames"
    }
    Write-Host "       Active concept document: $ExpectedFileName"
}

function Test-ActiveConceptDocumentNormalization {
    $testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-31-concept-test-{0}" -f ([guid]::NewGuid().ToString('N')))
    $docsDirectory = Join-Path $testRoot 'docs'
    $archiveDirectory = Join-Path $docsDirectory 'archive'
    $expectedName = 'Star_Cluster_Game_Concept_v0.4c.docx'
...
function Assert-RepositoryManifest {
    ...
    foreach ($line in Get-Content -LiteralPath $ManifestPath) {
        ...
        if ($relativePath -eq 'CHECKPOINT_31_SHA256SUMS.txt') {
    ...
    if ($verified -ne 640) { throw "Repository manifest verified $verified files; the Checkpoint 31 archive requires exactly 640 files." }
...
function Ensure-ActiveValidationRunbook {
    ...
    if ($activeRunbooks.Count -ne 1 -or $activeRunbooks[0].Name -ne $ExpectedFileName) {
        ...
        throw "Exactly the Checkpoint 31 validation runbook must remain active. Found: $activeNames"
    }
    Write-Host "       Active validation runbook: $ExpectedFileName"
}

function Test-ActiveValidationRunbookNormalization {
    $testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-31-runbook-test-{0}" -f ([guid]::NewGuid().ToString('N')))
    $validationDirectory = Join-Path $testRoot 'validation'
    $archiveDirectory = Join-Path $validationDirectory 'archive'
    $expectedName = 'Checkpoint_31_TL1_Layered_Defensive_Systems_Calibration.md'
...
function Assert-TechnologyCsvContracts {
    ...
    if ($baselineHash -ne '624c46e991022b187fb01804d6e094389fcce5996d2b91589277d0bde94c55f5') { throw "TL1 baseline SHA-256 is $baselineHash, not the accepted TL1 baseline hash." }
    $requiredValues = @{
...
function Assert-Tl1ScenarioCorpusContracts {
    ...
    $expectedHash = '624c46e991022b187fb01804d6e094389fcce5996d2b91589277d0bde94c55f5'
...
function Assert-PdsStudyContracts {
    ...
    $baselinePath = '.\docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_1.csv'
    $expectedBaselineHash = '624c46e991022b187fb01804d6e094389fcce5996d2b91589277d0bde94c55f5'
...
function Assert-WorkbookContract {
    $path = '.\docs\design\player_technology\StarCluster_Player_TL_Framework_Draft_v0_11.xlsx'
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw 'Technology workbook v0.11 was not found.' }
    Add-Type -AssemblyName System.IO.Compression.FileSystem
    $zip = [System.IO.Compression.ZipFile]::OpenRead((Resolve-Path -LiteralPath $path).Path)
    try {
        ...
        $requiredSheets = @('Overview','TL1 Baseline','TL1 Loadouts','TL1 Test Matrix','Phase A Runtime','TL1 Phase B','TL1 Calibration','Checkpoint 28 Energy','Checkpoint 29 Matrix','Checkpoint 30 PDS','Checkpoint 31 Defense','Component Schema','Checkpoint 25 Plan','TL Matrix','Components','Compatibility Profiles','Adaptation Rules','Reference Library','Reference Insights','Design Reconciliation','Level Themes','Design Decisions','Sources Used')
        ...
        foreach ($marker in @('D-257','Checkpoint 29 - Complete TL1 Weapon Matrix','48 variants x 10,000 trials','D-261','Checkpoint 30 PDS Control - Corrected by Checkpoint 31','59 variants x 10,000 trials','D-269','Checkpoint 31 - TL1 Layered Defensive Systems Calibration','171 variants x 10,000 trials','Ready Package','Targeting Computer assistance')) {
            if (-not $allXml.Contains($marker)) { throw "Workbook does not contain required marker $marker." }
        }
    }
    finally { $zip.Dispose() }
    Write-Host '       Workbook OOXML: retained Checkpoint 29 Matrix, corrected Checkpoint 30 PDS, new Checkpoint 31 Defense, Decisions through D-269, and no structured table parts verified.'
}
...
function Assert-NewUnitTestContracts {
    ...
    if ($directFacts -ne 10 -or $directTheories -ne 3 -or $kineticFacts -ne 8 -or $energyFacts -ne 10 -or $matrixFacts -ne 8 -or $pdsFacts -ne 15 -or $defenseFacts -ne 10 -or $weaponFacts -ne 14) {
        throw "Unit-test source counts differ from the Checkpoint 31 contract: direct $directFacts/$directTheories, kinetic $kineticFacts, energy $energyFacts, matrix $matrixFacts, PDS $pdsFacts, defense $defenseFacts, weapon $weaponFacts."
    }
    Assert-FileContains $pdsPath @('Operational_targeting_computer_assists_pds_by_ten_points','Degraded_targeting_computer_assists_pds_by_five_points','Own_evm_does_not_reduce_amm_interception_chance','Kinetic_pds_consumes_and_reloads_one_ready_package_on_an_intercept') 'Checkpoint 31 PDS tests'
    ...
    Assert-FileContains $weaponPath @('AmmunitionFedWeaponStartsWithOneReadyPackage','AutomaticLoaderKeepsOnePackageReadyUntilTheLastShot') 'Checkpoint 31 Ready Package tests'
    Write-Host '       Checkpoint 31 unit-test sources: 15 PDS facts, 10 defensive facts, Ready Package coverage, and retained direct-fire/kinetic/energy/matrix contracts present.'
}
...
try {
    Write-Host '[1/14] Verifying complete Checkpoint 31 repository and parser contracts...'
    foreach ($requiredFile in @(
        '.\StarCluster.sln',
        '.\global.json',
        '.\CHECKPOINT_31_SHA256SUMS.txt',
        '.\Checkpoint_31_Readme.txt',
        '.\README.md',
        '.\docs\Star_Cluster_Game_Concept_v0.4c.docx',
        '.\docs\archive\Star_Cluster_Game_Concept_v0.4b.docx',
        '.\docs\checkpoints\Checkpoint_31_TL1_Layered_Defensive_Systems_Calibration.md',
        '.\docs\validation\Checkpoint_31_TL1_Layered_Defensive_Systems_Calibration.md',
        '.\docs\validation\archive\Checkpoint_30_TL1_PDS_And_Missile_Interception_Calibration.md',
        '.\docs\design\player_technology\StarCluster_Player_TL_Framework_Draft_v0_11.xlsx',
        '.\docs\design\player_technology\TL1_Layered_Defensive_Systems_Calibration_Plan_v0_1.md',
        '.\docs\design\player_technology\tl1_defensive_calibration_schema_v0_1.json',
        '.\docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_1.csv',
        '.\src\StarCluster.Core\Combat\Weapons\AmmunitionFeedState.cs',
        '.\src\StarCluster.Core\Combat\DirectFire\Tl1WeaponMatrixSimulator.cs',
        '.\src\StarCluster.ScenarioRunner\Program.cs',
        '.\src\StarCluster.ScenarioRunner\TL1Calibration\Tl1DefensiveCalibrationDocuments.cs',
        '.\src\StarCluster.ScenarioRunner\TL1Calibration\Tl1DefensiveCalibrationRunner.cs',
        '.\src\StarCluster.ScenarioRunner\Scenarios\TL1Calibration\tl1-ds01-layered-defensive-systems-study.json',
        '.\tests\StarCluster.Tests\Combat\DirectFire\Tl1DefensiveSystemsCalibrationTests.cs',
        '.\tools\checkpoints\checkpoint-31\static_preflight_checkpoint_31.py',
        '.\tools\checkpoints\checkpoint-31\build_checkpoint_31_release.ps1',
        '.\tools\checkpoints\checkpoint-31\validate_checkpoint_31_release.ps1',
        ...
        if (-not (Test-Path -LiteralPath $requiredFile -PathType Leaf)) { throw "Required Checkpoint 31 file $requiredFile was not found." }
    }

    Ensure-ActiveValidationRunbook -ValidationDirectory '.\docs\validation' -ExpectedFileName 'Checkpoint_31_TL1_Layered_Defensive_Systems_Calibration.md' -ArchiveDirectory '.\docs\validation\archive'
    Ensure-ActiveConceptDocument -DocsDirectory '.\docs' -ExpectedFileName 'Star_Cluster_Game_Concept_v0.4c.docx' -ArchiveDirectory '.\docs\archive'
    Test-NormalizedRelativePathCompatibility
    Test-RepositoryManifestLocalArtifactPolicy
    Assert-RepositoryManifest '.\CHECKPOINT_31_SHA256SUMS.txt'
    Assert-PowerShellScriptsParse '.\tools'
    Test-ActiveValidationRunbookNormalization
    Test-ActiveConceptDocumentNormalization

    Write-Host '[2/14] Verifying Concept, reference library, exact data, schemas, corpora, tests, and workbook contracts...'
    Assert-FileContains '.\README.md' @('Checkpoint 31','v0.4c','171 layered defensive-system variants','642 engine-independent tests') 'Repository README'
    Assert-FileContains '.\docs\checkpoints\Checkpoint_31_TL1_Layered_Defensive_Systems_Calibration.md' @('171 variants','Ready Package','Targeting Computer','AMM','ECM','ECCM','Shield Hardener','Shield Battery') 'Checkpoint 31 documentation'
    Assert-FileContains '.\docs\validation\Checkpoint_31_TL1_Layered_Defensive_Systems_Calibration.md' @('642 engine-independent tests','171 layered defensive-system variants','zero failed gates','idempotent') 'Checkpoint 31 validation runbook'
    Assert-FileContains '.\docs\Prototype_TODO.md' @('Checkpoint 31 status','Targeting Computer assistance','Shield Battery','reactor envelope') 'Prototype TODO'
    Assert-FileContains '.\docs\design\player_technology\README.md' @('TL1_Layered_Defensive_Systems_Calibration_Plan_v0_1.md','tl1_defensive_calibration_schema_v0_1.json','StarCluster_Player_TL_Framework_Draft_v0_11.xlsx') 'Player-technology README'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\README.md' @('tl1-defensive-calibration-preflight','tl1-defensive-calibration','checkpoint-31-tl1-defensive-calibration') 'ScenarioRunner README'
    Assert-DocxContains '.\docs\Star_Cluster_Game_Concept_v0.4c.docx' @(
        ...
        'D-262','D-269','END OF DRAFT v0.4c') 'Concept v0.4c'
    Assert-FileContains '.\src\StarCluster.Core\Combat\Weapons\AmmunitionFeedState.cs' @('ReadyPackages','ReservePackages','Consume') 'Ready Package state'
    Assert-FileContains '.\src\StarCluster.Core\Combat\DirectFire\Tl1WeaponMatrixSimulator.cs' @('SensorTrackGateEnabled','PdsInterceptionChance','PdsReadyAmmunitionA','FirmTrackTurnsA','ShieldBatteryChargesUsedA','TacticalShieldRechargePower') 'TL1 layered-defense simulator'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\TL1Calibration\Tl1DefensiveCalibrationRunner.cs' @('RequiredVariantCount = 171','accepted-control','sensor-ew-boundary','side-swap-track','variants.csv') 'TL1 defensive runner'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\Program.cs' @('tl1-defensive-calibration','tl1-defensive-calibration-preflight','checkpoint-31-tl1-defensive-calibration') 'ScenarioRunner defensive command routing'
    ...
    Assert-DefensiveStudyContracts
    Assert-NewUnitTestContracts
    Assert-WorkbookContract

    if ($RepositoryContractOnly) {
        Write-Host ''
        Write-Host 'Checkpoint 31 repository-contract preflight completed successfully.'
        Write-Host 'Manifest, parser, normalization, reference, documentation, CSV, Phase A/B/kinetic/energy/no-counter/PDS/defensive corpus, schema, test-source, semantic, and workbook contracts passed.'
        return
    }

    Write-Host '[3/14] Confirming Godot is closed and .NET SDK 8.0.423 is selected...'
    ...
    if ($godotProcesses) {
        ...
        throw "Close Godot before applying Checkpoint 31. Running process(es): $processNames"
    }
    $sdkVersion = dotnet --version
    Write-Host "       SDK: $sdkVersion"
    if ($sdkVersion -ne '8.0.423') { throw "Expected .NET SDK 8.0.423 from global.json, but dotnet selected $sdkVersion." }

    Write-Host '[4/14] Performing a clean compiler preflight with warnings as errors...'
    ...
    $buildText = Invoke-DotnetCapture -Arguments @('build','.\StarCluster.sln','--nologo','-warnaserror') -Description 'Clean dotnet build'

    Write-Host '[5/14] Running 642 engine-independent tests...'
    ...
    if ($testText -notmatch 'Passed:\s+642') { throw 'The complete suite did not report the expected 642 passed tests.' }

    Write-Host '[6/14] Running seven accepted deterministic moving-missile scenarios...'
    $legacyOutput = '.\out\checkpoint-31-deterministic'
    Remove-Item $legacyOutput -Recurse -Force -ErrorAction SilentlyContinue
    $legacyText = Invoke-Runner -Arguments @('run-all','--scenario-dir','.\src\StarCluster.ScenarioRunner\Scenarios','--output-dir',$legacyOutput) -Description 'Checkpoint 31 legacy deterministic corpus'
    ...
    Write-Host '[7/14] Running 12 TL1 Phase A documents and 54 mechanics cases...'
    $phaseAOutput = '.\out\checkpoint-31-tl1-phase-a'
    Remove-Item $phaseAOutput -Recurse -Force -ErrorAction SilentlyContinue
    $phaseAText = Invoke-Runner -Arguments @('tl1-phase-a','--scenario-dir','.\src\StarCluster.ScenarioRunner\Scenarios\TL1PhaseA','--baseline-file','.\docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_1.csv','--output-dir',$phaseAOutput) -Description 'Checkpoint 31 TL1 Phase A corpus'
    ...
    Write-Host '[8/14] Running seven TL1 Phase B documents and 36 direct-fire cases...'
    $phaseBOutput = '.\out\checkpoint-31-tl1-phase-b'
    Remove-Item $phaseBOutput -Recurse -Force -ErrorAction SilentlyContinue
    $phaseBText = Invoke-Runner -Arguments @('tl1-phase-b','--output-dir',$phaseBOutput) -Description 'Checkpoint 31 TL1 Phase B corpus'
    ...
    Write-Host '[9/14] Running 29 TL1 kinetic-interaction calibration variants at 10,000 trials each...'
    $phaseCOutput = '.\out\checkpoint-31-tl1-kinetic-calibration'
    Remove-Item $phaseCOutput -Recurse -Force -ErrorAction SilentlyContinue
    $phaseCText = Invoke-Runner -Arguments @('tl1-kinetic-calibration','--study-file','.\src\StarCluster.ScenarioRunner\Scenarios\TL1Calibration\tl1-kc01-kinetic-interaction-study.json','--baseline-file','.\docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_1.csv','--trials','10000','--jobs','24','--output-dir',$phaseCOutput) -Description 'Checkpoint 31 TL1 kinetic calibration study'
    ...
    Write-Host '[10/14] Running 31 TL1 energy-interaction calibration variants at 10,000 trials each...'
    $phaseDOutput = '.\out\checkpoint-31-tl1-energy-calibration'
    Remove-Item $phaseDOutput -Recurse -Force -ErrorAction SilentlyContinue
    $phaseDText = Invoke-Runner -Arguments @('tl1-energy-calibration','--study-file','.\src\StarCluster.ScenarioRunner\Scenarios\TL1Calibration\tl1-ec01-energy-interaction-study.json','--baseline-file','.\docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_1.csv','--trials','10000','--jobs','24','--output-dir',$phaseDOutput) -Description 'Checkpoint 31 TL1 energy calibration study'
    ...
    Write-Host '[12/14] Running 59 corrected TL1 PDS/interception variants at 10,000 trials each...'
    $phaseFOutput = '.\out\checkpoint-31-tl1-pds-calibration'
    Remove-Item $phaseFOutput -Recurse -Force -ErrorAction SilentlyContinue
    $phaseFText = Invoke-Runner -Arguments @('tl1-pds-calibration','--study-file','.\src\StarCluster.ScenarioRunner\Scenarios\TL1Calibration\tl1-pds01-interception-study.json','--baseline-file','.\docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_1.csv','--trials','10000','--jobs','24','--output-dir',$phaseFOutput) -Description 'Checkpoint 31 corrected TL1 PDS calibration study'
    ...
    Write-Host '[13/14] Running 171 TL1 layered defensive-system variants at 10,000 trials each...'
    $phaseGOutput = '.\out\checkpoint-31-tl1-defensive-calibration'
    Remove-Item $phaseGOutput -Recurse -Force -ErrorAction SilentlyContinue
    $phaseGText = Invoke-Runner -Arguments @('tl1-defensive-calibration','--study-file','.\src\StarCluster.ScenarioRunner\Scenarios\TL1Calibration\tl1-ds01-layered-defensive-systems-study.json','--baseline-file','.\docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_1.csv','--trials','10000','--jobs','24','--output-dir',$phaseGOutput) -Description 'Checkpoint 31 TL1 layered defensive calibration study'
    ...
    Write-Host '[14/14] Running forty-six ScenarioRunner self-tests...'
    $selfTestText = Invoke-Runner -Arguments @('self-test','--scenario-file','.\src\StarCluster.ScenarioRunner\Scenarios\terminal-two-window-hit.json') -Description 'Checkpoint 31 runner self-tests'
    ...
    Write-Host ''
    Write-Host 'Checkpoint 31 completed successfully.'
    Write-Host 'Engine-independent tests passed: 642.'
    ...
    Write-Host 'TL1 layered defensive-system calibration completed; no mechanical Godot validation is required.'
}
