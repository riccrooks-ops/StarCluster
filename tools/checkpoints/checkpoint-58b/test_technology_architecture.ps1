...
$checkpoint = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-58b.json')
...
Assert-True ([string]$checkpoint.checkpointId -eq '58b') 'Checkpoint 58b definition ID mismatch.'
Assert-True (@($checkpoint.stages).Count -eq 56 -and [int]$checkpoint.checkpointMetrics.stageCount -eq 56) 'Checkpoint 58b must contain 56 stages.'
Assert-True ([int]$checkpoint.checkpointMetrics.monteCarloVariantCount -eq 14746 -and [int]$checkpoint.checkpointMetrics.trialsAtDefault -eq 147460000) 'Checkpoint 58b workload metrics mismatch.'
Assert-True ([string]$checkpoint.primaryStudy.id -eq 'tl4-itc04-single-main-axis-screening' -and [int]$checkpoint.primaryStudy.variantCount -eq 144) 'Checkpoint 58b primary study metadata mismatch.'
...
# Checkpoint 58b release-manifest contract: retained repository ZIPs are repository-owned files,
# not packaging artifacts, and therefore must be present exactly once in the top-level manifest.
$manifestPath = Join-Path $repositoryRoot 'CHECKPOINT_58B_SHA256SUMS.txt'
Assert-True (Test-Path -LiteralPath $manifestPath -PathType Leaf) 'Checkpoint 58b repository manifest is missing.'
...
foreach ($relative in $retainedZipPaths) {
    ...
    $matches = @($manifestLines | Where-Object { $_ -match ("^[0-9a-fA-F]{64}  {0}$" -f $escaped) })
    Assert-True ($matches.Count -eq 1) "Retained repository ZIP must appear exactly once in Checkpoint 58b manifest: $relative"
...
$applySource = Get-Content -LiteralPath (Join-Path $PSScriptRoot 'apply_checkpoint_58b.ps1') -Raw
Assert-True (-not ($applySource -match '(?im)^\s*&\s*(python|python3|py)(\.exe)?\b')) 'Checkpoint 58b launcher must not require Python.'
Assert-True ($applySource.Contains('checkpoint-58b.json')) 'Checkpoint 58b launcher must use the Checkpoint 58b definition.'
...
Write-Host '       Checkpoint 58b manifest hotfix: retained nested repository ZIPs are SHA-256 locked.'
Write-Host '       Checkpoint 58b hotfix: 56 stages; 14,746 Monte Carlo variants; 147.46 million default trials.'
