...
OUT=ROOT/'out/checkpoint-58b-static-preflight.txt'; OUT.parent.mkdir(parents=True,exist_ok=True); checks=[]
...
required=[
 ROOT/'Checkpoint_58b_Readme.txt',ROOT/'README.md',ROOT/'docs/README.md',ROOT/'docs/Prototype_TODO.md',
 ROOT/'CHECKPOINT_58B_SHA256SUMS.txt',
 ...
 ROOT/'docs/validation/Checkpoint_58b_Repository_Manifest_Nested_ZIP_Hotfix.md',
 ...
 ROOT/'tools/calibration/checkpoints/checkpoint-58b.json',ROOT/'tools/checkpoints/checkpoint-58b/apply_checkpoint_58b.ps1',ROOT/'tools/checkpoints/checkpoint-58b/test_technology_architecture.ps1',ROOT/'tools/checkpoints/checkpoint-58b/static_preflight_checkpoint_58b.py',ROOT/'tools/checkpoints/checkpoint-58b/checkpoint_57a_scenario_hashes.txt',ROOT/'tools/checkpoints/checkpoint-58b/checkpoint_57a_critical_hashes.txt',
...
validate_hash_list(ROOT/'tools/checkpoints/checkpoint-58b/checkpoint_57a_scenario_hashes.txt',95,'frozen 57a scenario')
validate_hash_list(ROOT/'tools/checkpoints/checkpoint-58b/checkpoint_57a_critical_hashes.txt',498,'frozen 57a critical')
...
apply=read(ROOT/'tools/checkpoints/checkpoint-58b/apply_checkpoint_58b.ps1'); ok('launcher no Python',re.search(r'(?im)^\s*&\s*(python|python3|py)(\.exe)?\b',apply) is None); ok('launcher checkpoint58b definition','checkpoint-58b.json' in apply)

# Checkpoint 58b hotfix contract: canonical weapon power property is powerCost.
ps58b=read(ROOT/'tools/checkpoints/checkpoint-58b/test_technology_architecture.ps1')
ok('58a validator rejects legacy .power access', all(x not in ps58b for x in ['$output.weapons.kinetic.power -eq 2','$output.weapons.energy.power -eq 3','$output.weapons.missile.power -eq 1']))
ok('58a validator uses kinetic powerCost', "Get-OptionalProperty $output.weapons.kinetic 'powerCost' -1" in ps58b)
ok('58a validator uses energy powerCost', "Get-OptionalProperty $output.weapons.energy 'powerCost' -1" in ps58b)
ok('58a validator uses missile powerCost', "Get-OptionalProperty $output.weapons.missile 'powerCost' -1" in ps58b)
...
# Checkpoint 58b release manifest must lock every repository-owned file, including nested ZIPs.
manifest_path=ROOT/'CHECKPOINT_58B_SHA256SUMS.txt'
manifest_entries={}
...
lines=[f'Checkpoint 58b static preflight: {passed}/{len(checks)} checks passed; {len(failed)} failed.']
