...
$contractCheck = Join-Path $repositoryRoot 'tools\checkpoints\checkpoint-78a\test_checkpoint_78a_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-78a-deep-calibration.json' } else { 'checkpoint-78a.json' }
$definition = Join-Path $repositoryRoot ('tools\calibration\checkpoints\' + $definitionName)
$normalDefinition = 'tools/calibration/checkpoints/checkpoint-78a.json'
$deepDefinition = 'tools/calibration/checkpoints/checkpoint-78a-deep-calibration.json'
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!78A(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!78A(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!78a(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_78a_Historical_Authority_Contract_Hotfix.md') {
...
Write-Host '[3/4] Verifying Checkpoint 78a historical-authority and Matrix contracts...'
& $contractCheck
