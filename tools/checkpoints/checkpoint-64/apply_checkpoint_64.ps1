...
$contractCheck = Join-Path $PSScriptRoot 'test_checkpoint_64_contract.ps1'
$harness = Join-Path $repositoryRoot 'tools\calibration\run_calibration_checkpoint.ps1'
$definitionName = if ($DeepCalibration) { 'checkpoint-64-deep-calibration.json' } else { 'checkpoint-64.json' }
...
$staleRootPatterns = @(
    '^CHECKPOINT_(?!64(?:_|$)).*_SHA256SUMS\.txt$',
    '^Checkpoint_(?!64(?:_|$)).*_Readme\.txt$',
    '^checkpoint-(?!64(?:-|$)).*-static-preflight\.txt$'
)
...
if (Test-Path -LiteralPath $validationRoot -PathType Container) {
    foreach ($file in @(Get-ChildItem -LiteralPath $validationRoot -File -Filter 'Checkpoint_*.md')) {
        if ($file.Name -ne 'Checkpoint_64_TL1_Track_Aware_Movement_And_Acquisition.md') {
            Remove-Item -LiteralPath $file.FullName -Force
        }
    }
}
