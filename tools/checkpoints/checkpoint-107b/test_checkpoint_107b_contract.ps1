...
Assert-ObjectJsonEqual -Left $matrix.tiers -Right $frozenMatrix.tiers -Message 'CP107b must not change matrix tiers.'
...
Assert-True (-not (Test-Path -LiteralPath (Get-RepositoryPath 'tools/calibration/checkpoints/checkpoint-107b.json'))) 'CP107b must not create a calibration definition.'
...
Write-Host '       Validating CP107 design-content freeze and CP107b acceptance hotfix...'
...
Assert-True ((Get-RepositoryHash 'docs/validation/evidence/checkpoint-107a-unaccepted/CHECKPOINT_107A_SHA256SUMS.txt') -eq 'c5ed76d833daa3447eeb355c6c21360437aca88dcc48f5c943e3f0d6481e332a') 'Unaccepted CP107a manifest evidence hash drifted.'
$definition = Read-RepositoryJson 'tools/checkpoints/checkpoint-107b/checkpoint_107b_architecture_definition.json'
Assert-True ([string]$definition.checkpointId -eq '107b' -and [string]$definition.acceptedBaseline -eq '106' -and [string]$definition.contentCheckpoint -eq '107') 'CP107b definition identity drifted.'
Assert-True (-not [bool]$definition.numericalTlTableChanged -and -not [bool]$definition.newTl4Tl9BalanceValuesAssigned -and -not [bool]$definition.simulationOrCalibrationRun -and [int]$definition.declaredTrials -eq 0) 'CP107b must remain architecture-only.'
Assert-True (-not [bool]$definition.hotfix.designContentChanged -and [bool]$definition.hotfix.precheckHardened -and [bool]$definition.hotfix.schemaSafeOptionalProperties) 'CP107b must remain a pure acceptance-infrastructure hotfix.'
Assert-True (@($definition.stages).Count -eq 0) 'CP107b architecture definition must normalize stages as an explicit empty array.'
...
Assert-True ($guardText.IndexOf('Get-OptionalPropertyValue',[StringComparison]::Ordinal) -ge 0) 'Native precheck optional-property helper is missing.'
Assert-True ($guardText.IndexOf("-Name 'stages'",[StringComparison]::Ordinal) -ge 0) 'Native precheck must read stages through the optional-property helper.'
$guardPath = Get-RepositoryPath 'tools/checkpoints/Test-NativeAcceptanceDependencies.ps1'
$selfTestDirectory = Get-RepositoryPath 'out/checkpoint-107b'
$selfTestPath = Join-Path $selfTestDirectory 'native_precheck_alias_collision_fixture.ps1'
$selfTestRelativePath = 'out/checkpoint-107b/native_precheck_alias_collision_fixture.ps1'
...
try {
    & $guardPath -RepositoryRoot $repositoryRoot -PowerShellPaths @($selfTestRelativePath) -CheckpointDefinitionPaths @('tools/checkpoints/checkpoint-107b/checkpoint_107b_architecture_definition.json')
}
catch {
...
# Regression-test checkpoint-definition schema variants under StrictMode.
$schemaFixtureDirectory = Get-RepositoryPath 'out/checkpoint-107b'
$schemaPowerShellPath = Join-Path $schemaFixtureDirectory 'native_precheck_safe_fixture.ps1'
$schemaPowerShellRelativePath = 'out/checkpoint-107b/native_precheck_safe_fixture.ps1'
$missingStagesPath = Join-Path $schemaFixtureDirectory 'definition_missing_stages.json'
$missingStagesRelativePath = 'out/checkpoint-107b/definition_missing_stages.json'
$emptyStagesPath = Join-Path $schemaFixtureDirectory 'definition_empty_stages.json'
$emptyStagesRelativePath = 'out/checkpoint-107b/definition_empty_stages.json'
$blockedStagePath = Join-Path $schemaFixtureDirectory 'definition_blocked_stage.json'
$blockedStageRelativePath = 'out/checkpoint-107b/definition_blocked_stage.json'
New-Item -ItemType Directory -Path $schemaFixtureDirectory -Force | Out-Null
[IO.File]::WriteAllText($schemaPowerShellPath, "Write-Output 'native-precheck-fixture'`r`n")
[IO.File]::WriteAllText($missingStagesPath, '{"checkpointId":"fixture-missing-stages"}')
[IO.File]::WriteAllText($emptyStagesPath, '{"checkpointId":"fixture-empty-stages","stages":[]}')
[IO.File]::WriteAllText($blockedStagePath, '{"checkpointId":"fixture-blocked-stage","stages":[{"id":"blocked-python","command":"python"}]}')
try {
    & $guardPath -RepositoryRoot $repositoryRoot -PowerShellPaths @($schemaPowerShellRelativePath) -CheckpointDefinitionPaths @($missingStagesRelativePath)
    & $guardPath -RepositoryRoot $repositoryRoot -PowerShellPaths @($schemaPowerShellRelativePath) -CheckpointDefinitionPaths @($emptyStagesRelativePath)
    $blockedStageRejected = $false
    try {
        & $guardPath -RepositoryRoot $repositoryRoot -PowerShellPaths @($schemaPowerShellRelativePath) -CheckpointDefinitionPaths @($blockedStageRelativePath)
    }
    catch {
        $blockedStageRejected = $_.Exception.Message.IndexOf("blocked interpreter 'python'",[StringComparison]::OrdinalIgnoreCase) -ge 0
    }
    Assert-True $blockedStageRejected 'Native precheck schema self-test did not reject a blocked interpreter in an explicit stage.'
}
finally {
    Remove-Item -LiteralPath $schemaPowerShellPath,$missingStagesPath,$emptyStagesPath,$blockedStagePath -Force -ErrorAction SilentlyContinue
}
...
foreach ($relativePath in $cp107ContentHashes.Keys) {
    Assert-True ((Get-RepositoryHash $relativePath) -eq $cp107ContentHashes[$relativePath]) "CP107b acceptance hotfix changed frozen CP107 design content '$relativePath'."
}
...
foreach ($relativePath in @('README.md','CHAT_README.md','docs/README.md','docs/design/player_technology/README.md','docs/design/testing/README.md','docs/validation/README.md')) {
    ...
    Assert-True ($text.IndexOf('107b',[StringComparison]::OrdinalIgnoreCase) -ge 0) "Active document '$relativePath' must recognize CP107b."
}

Write-Host '       Validating full repository manifest...'
$manifestPath = Get-RepositoryPath 'CHECKPOINT_107B_SHA256SUMS.txt'
Assert-True (Test-Path -LiteralPath $manifestPath) 'CP107b manifest is missing.'
$manifestEntries = @{}
...
foreach ($file in Get-ChildItem -LiteralPath $repositoryRoot -Recurse -File -Force) {
    ...
    if ($relativePath -eq 'CHECKPOINT_107B_SHA256SUMS.txt') { continue }
...
Write-Host ("       CP107b contract verified: {0} repository-owned files; CP107 design content frozen; 10 disciplines / 32 source lineages / 214 beats / 136 ideas / 20 foundation domains; zero hard gates; zero trials; zero numerical TL4-TL9 values." -f $repositoryOwnedFiles.Count)
