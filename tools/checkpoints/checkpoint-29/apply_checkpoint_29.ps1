...
function Ensure-ActiveConceptDocument {
    ...
    if ($activeConcepts.Count -ne 1 -or $activeConcepts[0].Name -ne $ExpectedFileName) {
        ...
        throw "Exactly the Checkpoint 29 concept document must remain active. Found: $activeNames"
    }
...
function Test-ActiveConceptDocumentNormalization {
    ...
    $expectedName = 'Star_Cluster_Game_Concept_v0.4a.docx'
...
function Assert-RepositoryManifest {
    ...
    foreach ($line in Get-Content -LiteralPath $ManifestPath) {
        ...
        if ($relativePath -eq 'CHECKPOINT_29_SHA256SUMS.txt') {
    ...
    if ($verified -ne 611) { throw "Repository manifest verified $verified files; the Checkpoint 29 archive requires exactly 611 files." }
...
function Ensure-ActiveValidationRunbook {
    ...
    if ($activeRunbooks.Count -ne 1 -or $activeRunbooks[0].Name -ne $ExpectedFileName) {
        ...
        throw "Exactly the Checkpoint 29 validation runbook must remain active. Found: $activeNames"
    }
...
function Test-ActiveValidationRunbookNormalization {
    ...
    $expectedName = 'Checkpoint_29_Revised_Evasive_Maneuvering_And_Complete_TL1_Weapon_Matrix.md'
...
function Assert-TechnologyCsvContracts {
    ...
    if ($recon.Count -ne 32) { throw "Expected 32 Checkpoint 29 reconciliation rows; found $($recon.Count)." }
    if (($recon[0].PSObject.Properties.Name -join ',') -ne 'topic,current_direction,reference_influence,checkpoint_25_decision,status,next_action') { throw 'Checkpoint 29 reconciliation header is invalid.' }
    ...
    if ($baselineHash -ne '11913725247f43c7a11c4d5fd06a2182586ca0d91d49abd9236af5b398088dcb') { throw "TL1 baseline SHA-256 is $baselineHash, not the accepted Checkpoint 29 hash." }
    $requiredValues = @{
...
function Assert-Tl1ScenarioCorpusContracts {
    ...
    if ($implemented.Count -ne 12) { throw "Expected 12 implemented Checkpoint 29 matrix rows; found $($implemented.Count)." }
...
function Assert-WorkbookContract {
    $path = '.\docs\design\player_technology\StarCluster_Player_TL_Framework_Draft_v0_9.xlsx'
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw 'Technology workbook v0.9 was not found.' }
    Add-Type -AssemblyName System.IO.Compression.FileSystem
    $zip = [System.IO.Compression.ZipFile]::OpenRead((Resolve-Path -LiteralPath $path).Path)
    try {
        ...
        foreach ($sheetName in @('Overview','TL1 Baseline','TL1 Loadouts','TL1 Test Matrix','Phase A Runtime','TL1 Phase B','TL1 Calibration','Checkpoint 28 Energy','Checkpoint 29 Matrix','Component Schema','Checkpoint 25 Plan','TL Matrix','Components','Compatibility Profiles','Adaptation Rules','Reference Library','Reference Insights','Design Reconciliation','Level Themes','Design Decisions','Sources Used')) {
        ...
        foreach ($marker in @('D-257','Checkpoint 29 - Complete TL1 Weapon Matrix','48 variants x 10,000 trials','Accuracy / Guidance')) {
            if (-not $allXml.Contains($marker)) { throw "Workbook does not contain required Checkpoint 29 marker $marker." }
        }
    }
    finally { $zip.Dispose() }
    Write-Host '       Workbook OOXML: Checkpoint 29 Matrix sheet and Decision D-257 present; no structured table parts.'
}

function Assert-NewUnitTestContracts {
    $path = '.\tests\StarCluster.Tests\Combat\DirectFire\Tl1DirectFireAccuracyTests.cs'
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Checkpoint 29 direct-fire unit-test file was not found." }
    ...
    if ($facts -ne 10 -or $theories -ne 3) { throw "Expected 10 Fact methods and 3 Theory methods in the Checkpoint 29 direct-fire test file; found $facts and $theories." }
    Assert-FileContains $path @('CalculatorMatchesAcceptedTl1Examples','SimultaneousWindowAllowsMutualDestruction','KineticMirrorAllHitsMutuallyDestroysOnTurnNine') 'Checkpoint 29 direct-fire tests'
    $calibrationPath = '.\tests\StarCluster.Tests\Combat\DirectFire\Tl1KineticDuelCalibrationTests.cs'
    if (-not (Test-Path -LiteralPath $calibrationPath -PathType Leaf)) { throw 'Checkpoint 29 calibration test file was not found.' }
    ...
    Assert-FileContains $calibrationPath @('RevisedBaselineAllHitsMutuallyDestroysOnTurnNine','ArmorProtectionTwoWithoutApenCanStallDamageThree','ApenTwoRestoresProgressAgainstArmorProtectionTwo') 'Checkpoint 29 calibration tests'
    Write-Host '       Checkpoint 29 unit-test source: 10 direct-fire facts, 3 theories, and 8 calibration facts present.'
}
...
try {
    Write-Host '[1/12] Verifying complete Checkpoint 29 repository and parser contracts...'
    foreach ($requiredFile in @(
        '.\StarCluster.sln',
        '.\global.json',
        '.\CHECKPOINT_29_SHA256SUMS.txt',
        ...
        '.\docs\Star_Cluster_Game_Concept_v0.4a.docx',
        '.\docs\design\player_technology\TL1_Core_Combat_Test_Plan_v0_3.md',
        '.\docs\design\player_technology\StarCluster_Player_TL_Framework_Draft_v0_9.xlsx',
        '.\docs\checkpoints\Checkpoint_29_Revised_Evasive_Maneuvering_And_Complete_TL1_Weapon_Matrix.md',
        '.\docs\validation\Checkpoint_29_Revised_Evasive_Maneuvering_And_Complete_TL1_Weapon_Matrix.md',
        ...
        '.\src\StarCluster.ScenarioRunner\TL1Calibration\Tl1WeaponMatrixDocuments.cs',
        ...
        '.\docs\design\player_technology\tl1_weapon_matrix_schema_v0_1.json',
        '.\docs\design\player_technology\TL1_Complete_Weapon_Matrix_Plan_v0_1.md',
        '.\tests\StarCluster.Tests\Combat\DirectFire\Tl1WeaponMatrixTests.cs',
        ...
        if (-not (Test-Path -LiteralPath $requiredFile -PathType Leaf)) { throw "Required Checkpoint 29 file $requiredFile was not found." }
    }
    Ensure-ActiveValidationRunbook -ValidationDirectory '.\docs\validation' -ExpectedFileName 'Checkpoint_29_Revised_Evasive_Maneuvering_And_Complete_TL1_Weapon_Matrix.md' -ArchiveDirectory '.\docs\validation\archive'
    Ensure-ActiveConceptDocument -DocsDirectory '.\docs' -ExpectedFileName 'Star_Cluster_Game_Concept_v0.4a.docx' -ArchiveDirectory '.\docs\archive'
    Test-NormalizedRelativePathCompatibility
    Test-RepositoryManifestLocalArtifactPolicy
    Assert-RepositoryManifest '.\CHECKPOINT_29_SHA256SUMS.txt'
    Assert-PowerShellScriptsParse '.\tools'
    Test-ActiveValidationRunbookNormalization
    Test-ActiveConceptDocumentNormalization

    Write-Host '[2/12] Verifying Concept, reference library, exact data, schema, corpus, tests, and workbook contracts...'
    Assert-FileContains '.\README.md' @('Checkpoint 29','v0.4a','48 complete weapon-matrix','615 engine-independent tests') 'Repository README'
    Assert-FileContains '.\docs\checkpoints\Checkpoint_29_Revised_Evasive_Maneuvering_And_Complete_TL1_Weapon_Matrix.md' @('48-variant','missile','side swaps','ECM') 'Checkpoint 29 documentation contract'
    ...
    Assert-FileContains '.\docs\Prototype_TODO.md' @('Checkpoint 29 status','48 kinetic/energy/missile','10,000 trials','PDS') 'Prototype TODO'
    ...
    Assert-DocxContains '.\docs\Star_Cluster_Game_Concept_v0.4a.docx' @('Checkpoint 29 - Revised Evasive Maneuvering and Complete TL1 Weapon Matrix','D-254','D-257','kinetic, energy, and missile mirrors','END OF DRAFT v0.4a') 'Concept v0.4a'
    ...
    if ($phaseC.baselineSha256.ToLowerInvariant() -ne '11913725247f43c7a11c4d5fd06a2182586ca0d91d49abd9236af5b398088dcb') { throw 'Kinetic calibration baseline hash is invalid.' }
    ...
    if ($energyStudy.baselineSha256.ToLowerInvariant() -ne '11913725247f43c7a11c4d5fd06a2182586ca0d91d49abd9236af5b398088dcb') { throw 'Energy calibration baseline hash is invalid.' }
    ...
    Assert-FileContains '.\src\StarCluster.Core\Combat\DirectFire\Tl1WeaponMatrixSimulator.cs' @('Tl1WeaponMatrixSimulator','MissileGuidance','RangeExhaustedA','TargetMovePerTurn') 'TL1 weapon matrix simulator'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\TL1Calibration\Tl1WeaponMatrixRunner.cs' @('TL1 Weapon Matrix preflight','mirror-side-bias','side-swap','variants.csv') 'TL1 weapon matrix runner'
    ...
    if ($weaponStudy.schemaVersion -ne 'star-cluster-tl1-weapon-matrix-v1') { throw 'Unexpected TL1 weapon matrix schema.' }
    if (@($weaponStudy.variants).Count -ne 48) { throw "Expected 48 weapon matrix variants; found $(@($weaponStudy.variants).Count)." }
    $weaponTestText = Get-Content -LiteralPath '.\tests\StarCluster.Tests\Combat\DirectFire\Tl1WeaponMatrixTests.cs' -Raw
    $weaponFacts = ([regex]::Matches($weaponTestText, '\[Fact\]')).Count
    if ($weaponFacts -ne 8) { throw "Expected 8 weapon matrix Fact methods; found $weaponFacts." }
    ...
    if ($RepositoryContractOnly) {
        Write-Host ''
        Write-Host 'Checkpoint 29 repository-contract preflight completed successfully.'
        Write-Host 'Manifest, parser, normalization, reference, documentation, CSV, Phase A/B/kinetic/energy/complete-weapon-matrix corpus, schema, test-source, and workbook contracts passed.'
        return
    }

    Write-Host '[3/12] Confirming Godot is closed and .NET SDK 8.0.423 is selected...'
    ...
    if ($godotProcesses) {
        ...
        throw "Close Godot before applying Checkpoint 29. Running process(es): $processNames"
    }
    $sdkVersion = dotnet --version
    Write-Host "       SDK: $sdkVersion"
    if ($sdkVersion -ne '8.0.423') { throw "Expected .NET SDK 8.0.423 from global.json, but dotnet selected $sdkVersion." }

    Write-Host '[4/12] Performing a clean compiler preflight with warnings as errors...'
    ...
    Write-Host '[5/12] Running 615 engine-independent tests...'
    $testOutput = & dotnet test '.\tests\StarCluster.Tests\StarCluster.Tests.csproj' --no-build --nologo 2>&1
    $testOutput | Write-Host
    if ($LASTEXITCODE -ne 0) { throw "dotnet test failed with exit code $LASTEXITCODE." }
    if (($testOutput | Out-String) -notmatch 'Passed:\s+615') { throw 'The complete suite did not report the expected 615 passed tests.' }

    Write-Host '[6/12] Running seven accepted deterministic moving-missile scenarios...'
    $legacyOutput = '.\out\checkpoint-29-deterministic'
    Remove-Item $legacyOutput -Recurse -Force -ErrorAction SilentlyContinue
    $legacyText = Invoke-Runner -Arguments @('run-all','--scenario-dir','.\src\StarCluster.ScenarioRunner\Scenarios','--output-dir',$legacyOutput) -Description 'Checkpoint 29 legacy deterministic corpus'
    ...
    Write-Host '[7/12] Running 12 TL1 Phase A documents and 54 mechanics cases...'
    $phaseAOutput = '.\out\checkpoint-29-tl1-phase-a'
    Remove-Item $phaseAOutput -Recurse -Force -ErrorAction SilentlyContinue
    $phaseAText = Invoke-Runner -Arguments @('tl1-phase-a','--scenario-dir','.\src\StarCluster.ScenarioRunner\Scenarios\TL1PhaseA','--baseline-file','.\docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_1.csv','--output-dir',$phaseAOutput) -Description 'Checkpoint 29 TL1 Phase A corpus'
    ...
    Write-Host '[8/12] Running seven TL1 Phase B documents and 36 direct-fire cases...'
    $phaseBOutput = '.\out\checkpoint-29-tl1-phase-b'
    ...
    Write-Host '[9/12] Running 29 TL1 kinetic-interaction calibration variants at 10,000 trials each...'
    $phaseCOutput = '.\out\checkpoint-29-tl1-kinetic-calibration'
    Remove-Item $phaseCOutput -Recurse -Force -ErrorAction SilentlyContinue
    $phaseCText = Invoke-Runner -Arguments @('tl1-kinetic-calibration','--study-file','.\src\StarCluster.ScenarioRunner\Scenarios\TL1Calibration\tl1-kc01-kinetic-interaction-study.json','--baseline-file','.\docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_1.csv','--trials','10000','--jobs','24','--output-dir',$phaseCOutput) -Description 'Checkpoint 29 TL1 kinetic calibration study'
    ...
    Write-Host '[10/12] Running 31 TL1 energy-interaction calibration variants at 10,000 trials each...'
    $phaseDOutput = '.\out\checkpoint-29-tl1-energy-calibration'
    Remove-Item $phaseDOutput -Recurse -Force -ErrorAction SilentlyContinue
    $phaseDText = Invoke-Runner -Arguments @('tl1-energy-calibration','--study-file','.\src\StarCluster.ScenarioRunner\Scenarios\TL1Calibration\tl1-ec01-energy-interaction-study.json','--baseline-file','.\docs\archive\player_technology\pre-cp165-active\tl1_core_combat_numerical_baseline_v0_1.csv','--trials','10000','--jobs','24','--output-dir',$phaseDOutput) -Description 'Checkpoint 29 TL1 energy calibration study'
    ...
    Write-Host '[11/12] Running 48 complete TL1 weapon-matrix variants at 10,000 trials each...'
    $phaseEOutput = '.\out\checkpoint-29-tl1-weapon-matrix'
    Remove-Item -LiteralPath $phaseEOutput -Recurse -Force -ErrorAction SilentlyContinue
    ...
    Write-Host '[12/12] Running forty-six ScenarioRunner self-tests...'
    $selfTestText = Invoke-Runner -Arguments @('self-test','--scenario-file','.\src\StarCluster.ScenarioRunner\Scenarios\terminal-two-window-hit.json') -Description 'Checkpoint 29 runner self-tests'
    ...
    Write-Host ''
    Write-Host 'Checkpoint 29 completed successfully.'
    ...
    Write-Host 'TL1 complete weapon matrix passed: 48 variants / 10,000 trials each / 0 failed gates.'
    Write-Host 'Runner self-tests passed: 46.'
    Write-Host 'Complete stripped-down TL1 weapon matrix completed; no mechanical Godot validation is required.'
}
