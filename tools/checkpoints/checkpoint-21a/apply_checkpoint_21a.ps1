...
function Assert-FileContains {
    ...
    if (-not (Test-Path $Path)) {
        throw "$Description file $Path was not found. Re-extract the complete Checkpoint 21a package."
    }
...
try {
    Write-Host '[1/13] Verifying repository and Checkpoint 21 overlay baseline...'
    ...
    Get-ChildItem '.\docs\validation' -File -ErrorAction SilentlyContinue |
        Where-Object { $_.Name -ne 'Checkpoint_21a_Full_Flight_Opportunity_Movement_Horizon_And_24_Worker_Scheduler_Repair.md' } |
        Remove-Item -Force
    ...
    if ($godotProcesses) {
        ...
        throw "Close Godot before applying Checkpoint 21a. Running process(es): $processNames"
    }
    ...
    Assert-FileContains '.\src\StarCluster.Core\Combat\Missiles\MissileInterceptionPhaseContext.cs' @(
        ...
        'MissileInterceptionOpportunity.PreTerminalAttack;') 'Checkpoint 21a interception policy'
    Assert-FileContains '.\src\StarCluster.Core\Simulation\ScenarioInitializationService.cs' @(
        'restoredGuidancePhaseCount = Math.Max(',
        'definition.RetainedDatalink.ReceivedGuidancePhase') 'Checkpoint 21a shared initialization'
    Assert-FileContains '.\src\StarCluster.Core\Combat\Missiles\MissileAutonomousGuidanceService.cs' @(
        ...
        'MissileRoutePlanner.FindRoute(') 'Checkpoint 21a authoritative guidance path'
    Assert-FileContains '.\src\StarCluster.Core\Combat\Missiles\MissileTerminalResolutionService.cs' @(
        'MissileTerminalOutcome.Dud',
        'MissileTerminalOutcome.CriticalHit') 'Checkpoint 21a terminal contract'
    ...
    foreach ($runnerFile in @(
        ...
        if (-not (Test-Path $runnerFile)) {
            throw "Required Checkpoint 21a runner file $runnerFile was not found."
        }
    }
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\ScenarioTerminalOpportunity.cs' @(
        ...
        'StationarySearchRetry') 'Checkpoint 21a opportunity source model'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\ScenarioExecutor.cs' @(
        ...
        'data: Data(("source", source.ToString()))') 'Checkpoint 21a opportunity authority'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\MonteCarloTrialResult.cs' @(
        ...
        'result.TerminalOpportunities.Count') 'Checkpoint 21a trial classifications'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\MonteCarloBatchRunner.cs' @(
        'if (options.KeepTrialJournal || options.Resume)',
        'AppendTrials(trialsPath, ordered);') 'Checkpoint 21a discarded-journal optimization'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\FullFlightCalibrationModel.cs' @(
        ...
        'ScenarioPreflightValidator.Validate') 'Checkpoint 21a full-flight materialization'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\FullFlightCalibrationRunner.cs' @(
        ...
        'PeakActiveWorkers') 'Checkpoint 21a bounded scheduler and reporting'
    Assert-FileContains '.\src\StarCluster.ScenarioRunner\ScenarioRunnerSelfTests.cs' @(
        ...
        'variant scheduler honors the 24-worker ceiling') 'Checkpoint 21a runner self-tests'
    ...
    foreach ($path in @($catalogPath, $pursuitPath)) {
        if (-not (Test-Path $path)) {
            throw "Required Checkpoint 21a study file $path was not found."
        }
    }
    try {
        ...
        throw "Checkpoint 21a study JSON could not be parsed: $($_.Exception.Message)"
    }
    if ($pursuit.schemaVersion -ne 2 -or
        ...
        throw 'Checkpoint 21a study requires schema 2, 1,000 trials, minimum 8 turns, safety buffer 4, PDS TL 4, and target ECM TL 4.'
    }
    $variantCount = (
    ...
    if ($variantCount -ne 288) {
        throw "Checkpoint 21a pursuit matrix contains $variantCount variants, expected 288."
    }
    ...
    if ($policyNames.Count -ne 4 -or
        ...
        throw 'Checkpoint 21a movement policies must be stationary, straight-retreat, crossing-weave, and turnback.'
    }

    Write-Host '[7/13] Verifying synchronized Checkpoint 21a documentation...'
    foreach ($documentationFile in @(
        ...
        '.\docs\validation\Checkpoint_21a_Full_Flight_Opportunity_Movement_Horizon_And_24_Worker_Scheduler_Repair.md',
        ...
        if (-not (Test-Path $documentationFile)) {
            throw "Required Checkpoint 21a documentation file $documentationFile was not found."
        }
    }
    Assert-FileContains '.\docs\validation\Checkpoint_21a_Full_Flight_Opportunity_Movement_Horizon_And_24_Worker_Scheduler_Repair.md' @(
        '506/506',
        ...
        'No mechanical Godot validation is required') 'Checkpoint 21a active validation runbook'
    Assert-FileContains '.\docs\checkpoints\Checkpoint_21a_Full_Flight_Opportunity_Movement_Horizon_And_24_Worker_Scheduler_Repair.md' @(
        'TargetEnteredMissileHex',
        'crossing-weave',
        'OperationalTimeout',
        'at most 24 variants execute concurrently',
        '288,000 main-study trials',
        'No mechanical Godot validation is required') 'Checkpoint 21a implementation record'
    $activeValidationFiles = @(Get-ChildItem '.\docs\validation' -File)
    if ($activeValidationFiles.Count -ne 1 -or
        $activeValidationFiles[0].Name -ne 'Checkpoint_21a_Full_Flight_Opportunity_Movement_Horizon_And_24_Worker_Scheduler_Repair.md') {
        throw 'Exactly the Checkpoint 21a validation runbook must remain active.'
    }
    ...
    $deterministicOutputDirectory = '.\out\checkpoint-21a-deterministic'
    ...
    foreach ($jobs in @(1, 24)) {
        $outputDirectory = ".\out\checkpoint-21a-repro-j$jobs"
    ...
    $schedulerHashes = @{}
    foreach ($jobs in @(1, 24)) {
        $outputDirectory = ".\out\checkpoint-21a-scheduler-repro-j$jobs"
        Remove-Item -Recurse -Force $outputDirectory -ErrorAction SilentlyContinue
    ...
    $pursuitOutputDirectory = '.\out\checkpoint-21a-full-flight-pursuit-calibration'
    ...
    Write-Host ''
    Write-Host 'Checkpoint 21a completed successfully.'
    ...
    Write-Host 'Preserve out\checkpoint-21a-full-flight-pursuit-calibration for review.'
}
finally {
    Pop-Location
}
