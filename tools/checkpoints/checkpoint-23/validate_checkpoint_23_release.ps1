...
$stagingRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("star-cluster-checkpoint-23-release-{0}" -f ([guid]::NewGuid().ToString('N')))
...
function Assert-ArchiveContract {
    ...
    try {
        ...
        $manifestEntry = $zip.GetEntry('CHECKPOINT_23_SHA256SUMS.txt')
        ...
        foreach ($line in ($manifestText -split '\r?\n')) {
            ...
            if ($name -eq 'CHECKPOINT_23_SHA256SUMS.txt') { throw 'Archive manifest must not contain itself.' }
            if ($manifest.ContainsKey($name)) { throw "Duplicate archive manifest path: $name" }
            $manifest[$name] = $m.Groups[1].Value.ToLowerInvariant()
        }
        if ($manifest.Count -ne 466) { throw "Checkpoint 23 Revision 3 archive manifest must contain exactly 466 files; found $($manifest.Count)." }
        $actualNames = @($entries | ForEach-Object { $_.FullName.Replace('\','/') } | Where-Object { $_ -ne 'CHECKPOINT_23_SHA256SUMS.txt' })
...
try {
    ...
    $checkpointScript = Join-Path $stagingRoot 'tools\checkpoints\checkpoint-23\apply_checkpoint_23.ps1'
    if (-not (Test-Path -LiteralPath $checkpointScript -PathType Leaf)) { throw "Checkpoint 23 application script was not found in $resolvedArchive." }
    ...
    $archivedRunbook = Join-Path $validationDirectory 'archive\Checkpoint_22d_Accepted_Baseline_Closure_And_Checkpoint_23_Handoff.md'
    $staleActiveRunbook = Join-Path $validationDirectory 'Checkpoint_22d_Accepted_Baseline_Closure_And_Checkpoint_23_Handoff.md'
    if (-not (Test-Path -LiteralPath $archivedRunbook -PathType Leaf)) { throw 'The packaged Checkpoint 22d archived validation runbook was not found.' }
    Copy-Item -LiteralPath $archivedRunbook -Destination $staleActiveRunbook -Force

    $archivedConcept = Join-Path $stagingRoot 'docs\archive\Star_Cluster_Game_Concept_v0.3s.docx'
    $staleActiveConcept = Join-Path $stagingRoot 'docs\Star_Cluster_Game_Concept_v0.3s.docx'
    if (-not (Test-Path -LiteralPath $archivedConcept -PathType Leaf)) { throw 'The packaged Concept v0.3s archive copy was not found.' }
    ...
    $localArtifacts = @{
        ...
        'out\checkpoint-23-release-test\result.txt' = 'local output'
    ...
    if ($activeRunbooks.Count -ne 1 -or $activeRunbooks[0].Name -ne 'Checkpoint_23_Player_Technology_Reference_Mining_And_Framework_Foundation.md') { throw 'Release validation left an invalid active-runbook set.' }
    ...
    Write-Host ''
    Write-Host 'Checkpoint 23 Revision 3 release validation completed successfully.'
