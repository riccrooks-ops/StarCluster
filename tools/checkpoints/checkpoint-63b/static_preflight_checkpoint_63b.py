...
required = [
    ...
    "docs/validation/Checkpoint_63b_Manifest_Binding_Hotfix.md",
    "docs/validation/archive/Checkpoint_63a_Passive_Core_Gate_Hotfix.md",
    ...
    "tools/calibration/checkpoints/checkpoint-63b.json",
    "tools/calibration/checkpoints/checkpoint-63b-deep-calibration.json",
    "tools/checkpoints/checkpoint-63b/apply_checkpoint_63b.ps1",
    "tools/checkpoints/checkpoint-63b/test_checkpoint_63b_contract.ps1",
    "tools/checkpoints/checkpoint-63b/static_preflight_checkpoint_63b.py",
    "CHECKPOINT_63B_SHA256SUMS.txt",
]
for rel in required:
...
active = load("tools/calibration/checkpoints/checkpoint-63b.json")
deep = load("tools/calibration/checkpoints/checkpoint-63b-deep-calibration.json")
schema = load(
    "docs/design/player_technology/tl1_integrated_tactical_combat_schema_v0_9.json"
)

# Hotfix identity and harness-definition binding.  These checks specifically
# prevent the Checkpoint 63a packaging regression where the current root
# manifest existed but the active definition still referenced Checkpoint 63.
ok(active.get("checkpointId") == "63b", "active checkpoint id 63b")
ok(deep.get("checkpointId") == "63b", "deep checkpoint id 63b")
ok(active.get("manifestFile") == "CHECKPOINT_63B_SHA256SUMS.txt", "active manifest binding")
ok(deep.get("manifestFile") == "CHECKPOINT_63B_SHA256SUMS.txt", "deep manifest binding")
ok(active.get("outputRoot") == "out/checkpoint-63b", "active output root")
ok(deep.get("outputRoot") == "out/checkpoint-63b-deep-calibration", "deep output root")
ok("docs/validation/Checkpoint_63b_Manifest_Binding_Hotfix.md" in active.get("documentation", []), "active runbook binding")
ok("docs/validation/Checkpoint_63b_Manifest_Binding_Hotfix.md" in deep.get("documentation", []), "deep runbook binding")
...
hotfix_doc = (ROOT / "docs/validation/Checkpoint_63b_Manifest_Binding_Hotfix.md").read_text(encoding="utf-8")
ok("CHECKPOINT_63B_SHA256SUMS.txt" in hotfix_doc and "checkpoint-63b.json" in hotfix_doc, "CP63b manifest binding documented")

# Packaging hygiene and active runbook status.
root_txt = sorted(path.name for path in ROOT.glob("*.txt"))
ok(root_txt == ["CHECKPOINT_63B_SHA256SUMS.txt"], f"root txt hygiene {root_txt}")
...
ok(
    ...
    and active_runbooks[0].name == "Checkpoint_63b_Manifest_Binding_Hotfix.md",
    "single active validation runbook",
)
ok(
    (ROOT / "docs/validation/archive/Checkpoint_63a_Passive_Core_Gate_Hotfix.md").is_file(),
    "superseded CP63a runbook archived",
)
ok(
...
apply_script = (ROOT / "tools/checkpoints/checkpoint-63b/apply_checkpoint_63b.ps1").read_text(encoding="utf-8")
ok("Checkpoint_*.md" in apply_script and "Checkpoint_63b_Manifest_Binding_Hotfix.md" in apply_script, "overlay-safe stale active runbook cleanup")

# Manifest format and independent content hash verification.
manifest = ROOT / "CHECKPOINT_63B_SHA256SUMS.txt"
if manifest.is_file():
    entries = []
    ...
    ok("CHECKPOINT_63B_SHA256SUMS.txt" not in paths, "manifest excludes itself")
...
print(f"Checkpoint 63b static preflight: {checks - len(errors)}/{checks} checks passed.")
if errors:
