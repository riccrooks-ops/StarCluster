...
$checkpoint = Read-Json (Join-Path $repositoryRoot 'tools\calibration\checkpoints\checkpoint-58c.json')
...
Assert-True ([string]$checkpoint.checkpointId -eq '58c') 'Checkpoint 58c definition ID mismatch.'
Assert-True (@($checkpoint.stages).Count -eq 56 -and [int]$checkpoint.checkpointMetrics.stageCount -eq 56) 'Checkpoint 58c must contain 56 stages.'
Assert-True ([int]$checkpoint.checkpointMetrics.monteCarloVariantCount -eq 14746 -and [int]$checkpoint.checkpointMetrics.trialsAtDefault -eq 147460000) 'Checkpoint 58c workload metrics mismatch.'
Assert-True ([string]$checkpoint.primaryStudy.id -eq 'tl4-itc04-single-main-axis-screening' -and [int]$checkpoint.primaryStudy.variantCount -eq 144) 'Checkpoint 58c primary study metadata mismatch.'
...
# Checkpoint 58c release-manifest contract: retained repository ZIPs are repository-owned files,
# not packaging artifacts, and therefore must be present exactly once in the top-level manifest.
$manifestPath = Join-Path $repositoryRoot 'CHECKPOINT_58C_SHA256SUMS.txt'
Assert-True (Test-Path -LiteralPath $manifestPath -PathType Leaf) 'Checkpoint 58c repository manifest is missing.'
...
foreach ($relative in $retainedZipPaths) {
    ...
    Assert-True ($matches.Count -eq 1) "Retained repository ZIP must appear exactly once in Checkpoint 58c manifest: $relative"
...
$applySource = Get-Content -LiteralPath (Join-Path $PSScriptRoot 'apply_checkpoint_58c.ps1') -Raw
Assert-True (-not ($applySource -match '(?im)^\s*&\s*(python|python3|py)(\.exe)?\b')) 'Checkpoint 58c launcher must not require Python.'
Assert-True ($applySource.Contains('checkpoint-58c.json')) 'Checkpoint 58c launcher must use the Checkpoint 58c definition.'
...
Write-Host '       Checkpoint 58c PowerShell manifest hotfix: retained ZIP entries are parsed without regex format-string interpolation.'
Write-Host '       Checkpoint 58c hotfix: 56 stages; 14,746 Monte Carlo variants; 147.46 million default trials.'
