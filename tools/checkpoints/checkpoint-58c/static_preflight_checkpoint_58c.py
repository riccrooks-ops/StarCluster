...
OUT=ROOT/'out/checkpoint-58c-static-preflight.txt'; OUT.parent.mkdir(parents=True,exist_ok=True); checks=[]
...
required=[
 ROOT/'Checkpoint_58c_Readme.txt',ROOT/'README.md',ROOT/'docs/README.md',ROOT/'docs/Prototype_TODO.md',
 ROOT/'CHECKPOINT_58C_SHA256SUMS.txt',
 ...
 ROOT/'docs/validation/Checkpoint_58c_PowerShell_Manifest_Format_Hotfix.md',
 ...
 ROOT/'tools/calibration/checkpoints/checkpoint-58c.json',ROOT/'tools/checkpoints/checkpoint-58c/apply_checkpoint_58c.ps1',ROOT/'tools/checkpoints/checkpoint-58c/test_technology_architecture.ps1',ROOT/'tools/checkpoints/checkpoint-58c/static_preflight_checkpoint_58c.py',ROOT/'tools/checkpoints/checkpoint-58c/checkpoint_57a_scenario_hashes.txt',ROOT/'tools/checkpoints/checkpoint-58c/checkpoint_57a_critical_hashes.txt',
...
validate_hash_list(ROOT/'tools/checkpoints/checkpoint-58c/checkpoint_57a_scenario_hashes.txt',95,'frozen 57a scenario')
validate_hash_list(ROOT/'tools/checkpoints/checkpoint-58c/checkpoint_57a_critical_hashes.txt',498,'frozen 57a critical')
...
apply=read(ROOT/'tools/checkpoints/checkpoint-58c/apply_checkpoint_58c.ps1'); ok('launcher no Python',re.search(r'(?im)^\s*&\s*(python|python3|py)(\.exe)?\b',apply) is None); ok('launcher checkpoint58c definition','checkpoint-58c.json' in apply)

# Checkpoint 58c hotfix contract: canonical weapon power property is powerCost.
ps58c=read(ROOT/'tools/checkpoints/checkpoint-58c/test_technology_architecture.ps1')
ok('58c manifest check avoids regex format-string collision', '"^[0-9a-fA-F]{64}  {0}$" -f' not in ps58c)
ok('58c manifest check parses manifest entries', "[regex]::Match($_, '^([0-9a-fA-F]{64})  (.+)$')" in ps58c)
ok('58a validator rejects legacy .power access', all(x not in ps58c for x in ['$output.weapons.kinetic.power -eq 2','$output.weapons.energy.power -eq 3','$output.weapons.missile.power -eq 1']))
ok('58a validator uses kinetic powerCost', "Get-OptionalProperty $output.weapons.kinetic 'powerCost' -1" in ps58c)
ok('58a validator uses energy powerCost', "Get-OptionalProperty $output.weapons.energy 'powerCost' -1" in ps58c)
ok('58a validator uses missile powerCost', "Get-OptionalProperty $output.weapons.missile 'powerCost' -1" in ps58c)
...
# Checkpoint 58c release manifest must lock every repository-owned file, including nested ZIPs.
manifest_path=ROOT/'CHECKPOINT_58C_SHA256SUMS.txt'
manifest_entries={}
...
lines=[f'Checkpoint 58c static preflight: {passed}/{len(checks)} checks passed; {len(failed)} failed.']
