# Checkpoint 19a Validation - Reproducible Monte Carlo Guard Hotfix

## Required action

Close Godot, extract the complete Checkpoint 19a overlay into the repository
root with overwrite enabled, and run:

```powershell
powershell -ExecutionPolicy Bypass -File .\tools\checkpoints\checkpoint-19a\apply_checkpoint_19a.ps1
```
...
- the 2,000-trial reproducibility study passes at worker counts 1, 12, and 24;
...
The apply script invokes the same reproducibility sweep with these literal
worker settings:
...
These runs must produce byte-identical canonical `sweep-summary.json` files.

## Output directories

```text
out\checkpoint-19-deterministic
...
requests the statistical results. Normal successful handoff requires the
