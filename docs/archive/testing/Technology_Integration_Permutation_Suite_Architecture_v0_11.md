# Technology Integration Permutation Suite Architecture v0.11

## Authority

This is the current standing integration-suite architecture. It extends accepted v0.10 without changing the legal construction envelope, gameplay rules, component values, Technology Architecture, AI doctrine, or reference-mining authority. Its purpose is to improve **sampling quality and analysis fidelity** before the cross-TL study is scaled further.

Version 0.11 preserves the complete deterministic **82,944 raw / 22,592 legal** build envelope and its **255,187,936 unordered distinct** legal-pair population. It does not promote any technology candidate.

## Why v0.11 exists

Native Checkpoint 93 proved the matched/readiness/Space/population pipeline, but it also exposed two analysis limitations:
...
CP93 also showed that mover order can have a small median effect but a very large tail for some pairings. Version 0.11 therefore improves **within-cell representation**, **individual observed-engagement diagnosis**, and **mover-order-neutral reporting** before any broad component tuning or much larger Monte Carlo expansion.
...
Version 0.11 replaces the uniform one-pair-per-cell statistical slice with **192 unordered distinct statistical base pairs**. Every cell receives at least one base pair. Remaining representatives are allocated deterministically in proportion to the square root of cell population, with a maximum of five statistical representatives per cell. This deliberately gives more representation to high-population cells without allowing the largest cells to consume the entire sample.
...
Primary population cells do not directly stratify by weapon-family pairing or information-control gap. Those dimensions remain important comparability metadata, so v0.11 adds a separate diagnostic overlay rather than corrupting the statistical weights.
...
The CP94 bounded screen therefore contains:
...
Version 0.11 additionally records the **maximum ready range in hexes** at which the side can simultaneously obtain the required Firm track and satisfy physical weapon range against that opponent under the declared reference Sensor/EW capability.
...
Aggregate family/context activity remains useful, but it may hide a dead individual pairing inside a healthy cohort. Version 0.11 therefore classifies every statistical/diversity matched result independently.
...
This distinction directly prevents the CP93 readiness/doctrine mismatch from being mislabeled as build illegality.

## Mover-order-neutral matched reporting

Both existing TrackAware movement-order bounds remain authoritative diagnostic contexts. No production initiative rule is chosen by v0.11.
...
Version 0.11 keeps three questions separate rather than collapsing them into one aggregate:
...
## CP94 bounded proving workload

Checkpoint 94 deliberately improves the **number and diversity of sampled pairs** while slightly reducing per-variant trials from CP93's 2,000 to **1,500**. This keeps the pass focused on sampling quality rather than simply buying narrower Monte Carlo error bars on a weak sample.

Default CP94 workload:
...
Deep Calibration is not applicable to CP94. A larger/high-trial study should wait until native evidence confirms that the adaptive sample, diversity overlay, individual engagement diagnostics, and mover-neutral reports behave as intended.
