# Weapon Ammunition and Warhead Architecture v0.2

**Checkpoint:** 116  
**Status:** Architecture refinement candidate; CP115a accepted evidence incorporated; no numerical promotion
...
- **General-purpose energetic maturation and specialist penetration are separate axes.** A normal GP warhead may gain raw energetic yield as fission/fusion/antimatter engineering advances, but SPEN/APEN do not automatically rise just because DAM rises. Any penetration improvement requires an explicit technological/warhead rationale and must be evaluated for whether it turns GP into a hidden specialist.
- Specialist warheads mature with the contemporary energetic generation rather than remaining frozen at TL1-sized packets. Their role-specific effect is purchased by giving up some combination of yield, the opposite penetration axis, generality, or another meaningful capability.
- A GP warhead is allowed to be merely competent rather than optimal against strongly specialized defenses. Missile flexibility comes from access to specialist payloads, not from making GP simultaneously anti-Shield and anti-Armor.
...
| 5 | Fusion microcharge general-purpose warhead | automatic_compatible_upgrade | high_energy_payload_bus; antimatter_hardened_payload_bus | Power TL2 | High-energy GP maturation. Because an otherwise superior fusion payload would be an obvious choice, it should replace the older GP package rather than create loadout clutter. | bounded increase in general-purpose energetic yield; penetration changes are separate explicit candidates rather than automatic companions to higher DAM; strategic resupply/resource cost may differ later; no per-type tactical inventory | CP109 D8/SP2/AP3 is not retained as an active numerical candidate; exact GP improvement requires calibration. |
...
| 7 | Antimatter general-purpose warhead | automatic_compatible_upgrade | antimatter_hardened_payload_bus | Power TL5 | Antimatter-era GP maturation. Tactical subtype bookkeeping remains abstract; containment/resource consequences belong to compatibility, hazards, resupply and campaign economy. | large but bounded GP energetic-yield increase; SPEN/APEN do not automatically rise with antimatter yield; containment/signature/hazard traits; strategic resource burden | CP109 D10/SP3/AP4 branch value is suspended; exact contemporary GP stats require calibration against defenses. |
...
## CP116 role orthogonality clarification

CP115a demonstrated that the prior characteristic-space GP candidates gained raw yield **and** penetration together. That made it impossible to tell whether improved performance came from energetic maturation or from silently giving the default payload anti-Shield/anti-Armor specialization. CP116 therefore separates these dimensions.

- **Pure GP controls:** increase DAM by energetic generation while holding the existing GP SPEN/APEN baseline constant.
- **Single-axis controls:** at matched DAM, add only SPEN or only APEN to measure the contribution of each penetration axis.
- **Bundled-penetration controls:** at matched DAM, add both penetration axes. These are intentionally diagnostic controls, not preferred GP designs.
- **Generational specialists:** start from contemporary fission/fusion/antimatter yield and then sacrifice something meaningful for an Armor- or Shield-focused effect.
- **Static-specialist controls:** preserve CP115-style small fixed specialist packets so the study can measure artificial obsolescence directly.

This does **not** lock the final GP penetration baseline at SPEN1/APEN2. It locks only the design principle that higher energetic yield does not automatically grant unrelated specialist penetration.
...
Run CP116 role-orthogonality/generational-scaling diagnostics before selecting any final Missile GP or specialist profile. Use pure-yield, single-axis, bundled-penetration, and generation-relative specialist controls to distinguish energetic progression from hidden specialization. No result promotes automatically.
