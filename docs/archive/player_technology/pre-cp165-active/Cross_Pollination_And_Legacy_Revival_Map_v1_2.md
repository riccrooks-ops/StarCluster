# Cross-Pollination and Legacy Revival Map v1.2
...
The storyboard's `relatedResearch` field is descriptive and **non-gating**. A hard external prerequisite exists only when a later component record separately names the precise external TL or capability, explains the causal science, and shows why the owning discipline cannot supply a useful native route. CP109 established sparse candidate hard prerequisites. CP113 preserves that model and makes the ammunition examples explicit: the TL4 Maneuvering/Programmable Smart Projectile may require Computing / Fire Control TL4; the TL5 Fusion GP missile warhead may require Power TL2; and the TL7 Antimatter GP warhead may require Power TL5. These unlock automatically when the last prerequisite is satisfied and do not block the owning discipline’s other useful progression.
...
| Projectile Weapons | electromagnetic launch, smart/maneuvering projectiles, specialist penetrators | Power, Computing | automatic compatible ammunition maturation or a true tradeoff mode |
| Energy Weapons | beam PDS maturation, particle beams | Power, Computing | specialist interception or new beam behavior |
| Missile Weapons | cooperative seekers, high-energy GP maturation, shield/armor specialist warheads | Sensors/EW, Computing, Power, Propulsion | new delivery/guidance plus mission-specific payload choices without subtype inventory bookkeeping |
