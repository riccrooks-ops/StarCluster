# Technology Family Storyboard v1.3

**Checkpoint:** 113  
**Status:** Ammunition/warhead-reconciled qualitative architecture; no balance calibration

CP113 preserves the 10-discipline / 32-lineage Storyboard foundation while refining the Kinetic-ammunition and Missile-warhead expressions. Strictly better compatible ammunition is automatic; only genuine tradeoffs remain selectable. Normal payload variants share broad magazine pools; rare/exotic munitions may be tracked individually.
...
**Identity:** Projectile engineering progresses independently of the launcher. Strict improvements become automatic compatible ammunition maturation; selectable projectile modes exist only where a real tradeoff creates a tactical choice.
...
| 5 | Dense/graded penetrator package | candidate / cross_pollinated_derivative | `payload_variant` | Advanced materials create a selectable armor-penetration package only because it trades APEN against raw damage and/or shield performance. | Armor | — | SD-SW-061 | Must remain non-dominating versus the contemporary general projectile; uses the same generic Kinetic magazine. |
| 6 | Micro-submunition/saturation package | candidate / branch | `payload_variant` | A selectable multi-body/proximity package trades per-packet penetration/effect for saturation or broader effective damage volume while one Ammo point still represents the attack package. | Computing / Fire Control | — | RM-THEME-011 | Uses the same generic Kinetic magazine; exact packet count/effect awaits calibration. |
...
| 5 | Cooperative swarm/bus architecture | candidate / branch | `installed_component` | A Missile Flight may represent coordinated submunitions/vehicles with one bounded attack package and richer PDS interactions. | Computing / Fire Control | — | SD-SW-005; SD-SW-059 | — |
...
**Identity:** Missile payload technology is modular. General-purpose warheads may mature automatically when a superior compatible package has no tactical downside; armor- or shield-specialist warheads remain selectable only when they sacrifice another capability.

| TL | Beat | Status / role | Player expression | Story | Related research | Hard prerequisites | References | Boundary |
|---:|---|---|---|---|---|---|---|---|
| 1 | Conventional general-purpose warhead | base / core_family | `automatic_capability` | Ordinary impact/blast packages provide the balanced default when target defenses are unknown. | Projectile Weapons | — | SD-SW-051 | Normal warhead types share the generic Missile Flight magazine; no pre-battle subtype allocation. |
| 3 | Shaped/advanced penetrator warhead | candidate / maturation | `payload_variant` | A shaped/advanced penetrator becomes a selectable armor specialist only if increased APEN costs raw damage and/or shield effectiveness. | Armor | — | SD-SW-052 | Warhead is chosen at launch and cannot change in flight; it must not strictly dominate the GP payload. |
| 4 | Nuclear directed-pulse / shield-disruption warhead | candidate / cross_pollinated_derivative | `payload_variant` | Directed nuclear/high-energy payload engineering enables a specialist attack intended to create cumulative shield pressure through shield-facing damage, field coupling, or bounded recharge disruption rather than universal raw damage escalation. | Power | — | SD-SW-052; TI shaped-nuclear inspiration | Must sacrifice Armor/Hull performance; exact anti-shield mechanism is a calibration question. Uses generic Missile Flight inventory. |
