# Player TL1-TL9 Technology Architecture v0.9

## Purpose

Checkpoint 57 preserves the accepted three-generation capacity cadence and refines TL3 as the mature low-tech cruiser. TL1/TL2 and all accepted Checkpoint 55b evidence remain frozen. The working TL3 numerical base is the modest offense-only refinement; Hull, Armor Integrity, and Shield Capacity are tested one point at a time before any defensive change is promoted.
...
## Checkpoint 57 runtime evidence

Checkpoint 57 adds six studies totaling 1,995 Monte Carlo variants:
...
All 79 ScenarioRunner JSON files present in accepted Checkpoint 55b are SHA-256 frozen before the six new studies are added. The complete Checkpoint 57 workload is 45 stages and 12,691 Monte Carlo variants at the default trial count. TL4-TL9 runtime generation remains deferred.


## Checkpoint 57 accepted TL3 / provisional TL4 bridge

- TL3 is frozen at one main weapon and two AUX Capacity with the offense-only maturity profile: Targeting +1 over TL2, Kinetic accuracy +1, Energy accuracy +1, Missile guidance +2, and no base Hull/AI/Shield increment.
- TL3 Combat Battery is B4G1 (4 charges x +1 TP); TL3 Power Capacitor is C2D1 (capacity 2, charge 1, discharge +1 TP). Per-installed-AUX independence remains authoritative.
- TL4 foundation screening uses two unrestricted main weapons, two AUX Capacity, and otherwise exact TL3 production component statistics.
- New TL4 studies use natural weapon/AUX power demand only. No synthetic background Tactical Power commitment is used.
- If mature specialized TL3 ships become broadly irrelevant against a naked TL4 merely because of the second weapon, the preferred fallback is one main weapon throughout TL1-TL9 rather than artificial fire-control restrictions.
